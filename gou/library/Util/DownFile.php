...
class Util_DownFile {
	 ...
	 * @return stream
	 */
	public static function outputFile($sContent, $sAlias = '')
	{
		if (! strlen($sContent)) return;

        $a_file_mime = array('application/octet-stream');

        $i_file_size = strlen($sContent);
        $s_file_name = substr(strrchr('/' . $sAlias, '/'), 1);
        $s_file_ext = strtolower(substr(strrchr($sAlias, '.'), 1));
    ...
    public static function downloadFile($file, $sAlias = '') {
        if (! strlen($file)) return;

        $a_file_mime = array('application/octet-stream');

        $i_file_size = filesize($file);
        $s_file_name = substr(strrchr('/' . $file, '/'), 1);
        $s_file_ext = strtolower(substr(strrchr($file, '.'), 1));
	...
	//定义文件MIME类型数组
	public static function file_mime($mine) {
		$file_mime = array(
			'323' => array('text/h323'),
