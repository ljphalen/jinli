 ...
 * @version 1.0.0.0
 */
class Api_Baidu_Channel extends Api_Baidu_Base
{
	/**
	 * 可选参数的KEY
	 * 
	...
	public function setApiKey ( $apiKey )
	{
		...
		try
		{
			...
			{
				throw new Api_Baidu_Api_Baidu_ChannelException ( "invaid apiKey ( ${apiKey} ), which must be a 1 - 64 length string", self::CHANNEL_SDK_INIT_FAIL );
			}
		}
		catch ( Exception $ex )
		{
	...
	public function setSecretKey ( $secretKey )
	{
		...
		try
		{
			...
			{
				throw new Api_Baidu_ChannelException ( "invaid secretKey ( ${secretKey} ), which must be a 1 - 64 length string", self::CHANNEL_SDK_INIT_FAIL );
			}
		}
		catch ( Exception $ex )
		{
	...
	public function setCurlOpts($arr_curlOpts)
	{
		...
		try {
			if (is_array($arr_curlOpts)) {
				...
				throw new Api_Baidu_ChannelException( 'invalid param - arr_curlOpts is not an array ['
	...
	public function setHost ( $host )
	{
		...
		try
		{
			...
			{
				throw new Api_Baidu_ChannelException ( "invaid host ( ${host} ), which must be a 1 - 1024 length string", self::CHANNEL_SDK_INIT_FAIL );
			}
		}
		catch ( Exception $ex )
		{
	...
	*/
			...
			switch($pushType)
			{
				case self::PUSH_TO_USER:
					if ( !array_key_exists(self::USER_ID, $arrArgs) || empty($arrArgs[self::USER_ID])){
						throw new Api_Baidu_ChannelException("userId should be specified in optional[] when pushType is PUSH_TO_USER", self::CHANNEL_SDK_PARAM);
					}
					break;
	
				case self::PUSH_TO_TAG:
					if (!array_key_exists(self::TAG_NAME, $arrArgs) || empty($arrArgs[self::TAG_NAME])){
						throw new Api_Baidu_ChannelException("tag should be specified in optional[] when pushType is PUSH_TO_TAG", self::CHANNEL_SDK_PARAM);
					}
					break;
				...
				default:
					throw new Api_Baidu_ChannelException("pushType($pushType) must be in range[1,3]", self::CHANNEL_SDK_PARAM);
			}
     ...
     * @throws Api_Baidu_ChannelException 如果出错，则抛出异常，异常号是self::CHANNEL_SDK_INIT_FAIL
    */
	public function __construct ($apiKey = NULL, $secretKey = NULL, $arr_curlOpts = array())
	{
		if($this->_checkString($apiKey, 1, 64)){
			$this->_apiKey = $apiKey;
			 ...
			 throw new Api_Baidu_ChannelException("invalid param - apiKey[$apiKey],"
		...
		if($this->_checkString($secretKey, 1, 64)){
			...
			throw new Api_Baidu_ChannelException("invalid param - secretKey[$secretKey],"
                    . "which must be a 1 - 64 length string",
                    self::CHANNEL_SDK_INIT_FAIL );
		}

		if (!is_array($arr_curlOpts)) {
			throw new Api_Baidu_ChannelException('invalid param - arr_curlopt is not an array ['
                    . print_r($arr_curlOpts, true) . ']',
                    self::CHANNEL_SDK_INIT_FAIL);
		}
        $this->_curlOpts = $this->_curlOpts + $arr_curlOpts;
     ...
     * @throws Api_Baidu_ChannelException 如果出错，则抛出Api_Baidu_ChannelException异常，异常类型为self::CHANNEL_SDK_PARAM
     * 
     * @version 1.0.0.0
     */
            ...
            $throw = true)
	{
        ...
        if (isset($opt[$opt_key])) {
            if (!$this->_checkString($opt[$opt_key], $min, $max)) {
                throw new Api_Baidu_ChannelException ( 'invalid ' . $dis[$opt_key] . ' in $optinal ('
        ...
        if (isset($$g_key)) {
            if (!$this->_checkString($$g_key, $min, $max)) {
                throw new Api_Baidu_ChannelException('invalid ' . $g_key . ' in global area ('
        ...
        if (false !== getenv($env_key)) {
            if (!$this->_checkString(getenv($env_key), $min, $max)) {
                throw new Api_Baidu_ChannelException( 'invalid ' . $env_key . ' in environment variable ('
        ...
        if ($throw) {
            throw new Api_Baidu_ChannelException('no param (' . $dis[$opt_key] . ') was found',
	 ...
	 * @access protected
	 * @param array $opt 参数数组
	 * @throws Api_Baidu_ChannelException 如果出错，则抛出异常，异常号为 self::CHANNEL_SDK_PARAM
	 * 
	 * @version 1.0.0.0
	 */
	protected function _adjustOpt(&$opt)
	{
		if (!isset($opt) || empty($opt) || !is_array($opt)) {
			throw new Api_Baidu_ChannelException('no params are set',self::CHANNEL_SDK_PARAM);
		}
	...
	{
		if ( !is_array($params)) {
			throw new Api_Baidu_ChannelException('no params',self::CHANNEL_SDK_PARAM);
		}
		foreach($params as $key => $value) {
			switch($key)
			{
				case self::USER_ID:
					if( !is_string($value)){
						throw new Api_Baidu_ChannelException("USER_ID($value) is not string", 
							self::CHANNEL_SDK_PARAM);
					}
					break;
				case self::CHANNEL_ID:
					if( !is_numeric($value)) {
						throw new Api_Baidu_ChannelException("CHANNEL_ID($value) is not numeric", 
							self::CHANNEL_SDK_PARAM);
					}
					break;
				case self::DEVICE_TYPE:
					if( !is_numeric($value) || $value < 0 || $value > 5 ) {
						throw new Api_Baidu_ChannelException( "invalid DEVICE_TYPE($value)",
 							self::CHANNEL_SDK_PARAM);
					}
					break;
				case self::TAG_NAME:
					if( !is_string($value) || strlen($value) > 128 ){
						throw new Api_Baidu_ChannelException( "TAG_NAME($value) must be a string and strlen <= 128",
							self::CHANNEL_SDK_PARAM);
					}
					break;
				case self::MESSAGE_TYPE:
					if( !is_numeric($value) || $value < 0 || $value > 1) {
						throw new Api_Baidu_ChannelException( "invalid MESSAGE_TYPE($value) must be 0 or 1",
 							self::CHANNEL_SDK_PARAM);
					}
					break;
				case self::NAME:
					if( !is_string($value) || strlen($value) > 128 ){
						throw new Api_Baidu_ChannelException( "IOS_CERT_NAME($value) must be a string and strlen <= 128",
							self::CHANNEL_SDK_PARAM);
					}
					break;
				case self::DESCRIPTION:
					if( !is_string($value) || strlen($value) > 256 ){
						throw new Api_Baidu_ChannelException( "IOS_CERT_DESCRIPTION($value) must be a string and strlen <= 256",
							self::CHANNEL_SDK_PARAM);
					}
					break;
			}
	...
	*/
	 ...
	 * @access protected
	 * @param array $opt 参数数组
	 * @throws Api_Baidu_ChannelException 如果出错，则抛出异常，错误号为self::CHANNEL_SDK_SYS
	 * 
	 * @version 1.0.0.0
	 */
	protected function _baseControl($opt)
	{
		...
		$request = new Api_Baidu_RequestCore($url);
	 ...
	 * @throws Api_Baidu_ChannelException 如果出错，则抛出异常
	 * 
	 * @version 1.0.0.0
	 */
	protected function _commonProcess($paramOpt = NULL)
	{
		...
		if (empty($ret)) {
			throw new Api_Baidu_ChannelException('base control returned empty object',
                    self::CHANNEL_SDK_SYS);
		}
		if ($ret->isOK()) {
			$result = json_decode($ret->body, true);
			if (is_null($result)) {
				throw new Api_Baidu_ChannelException($ret->body,
		...
		$result = json_decode($ret->body,true);
		if (is_null($result)) {
			throw new Api_Baidu_ChannelException('ret body:' . $ret->body,
                    self::CHANNEL_SDK_HTTP_STATUS_ERROR_AND_RESULT_ERROR);
		}
		$this->_requestId = $result['request_id'];
		throw new Api_Baidu_ChannelException($result['error_msg'], $result['error_code']);
	}
	 ...
	 * @throws Api_Baidu_ChannelException 如果出错，则抛出异常，异常号为self::Channel_SDK_PARAM 
	 * 
	 * @version 1.0.0.0
	 */
