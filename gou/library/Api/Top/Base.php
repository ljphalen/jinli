...
class Api_Top_Base extends Api_Top_Client {
	 ...
	 * @return array
	 */
	public function _getMallInfo($nick) {
	 ...
	 * @return array
	 */
	public function _getShopInfo($nick) {
	 ...
	 *  @return array
	 */
	public function _getShopCatsList() {
	    echo 'afsfasfa';exit;
	 ...
	 *  @return array
	 */
	public function _getTaobaokeShops(array $params) {
		$params = array_merge(
				$params,
				array(
	 ...
	 * @return array
	 */
	public function _getUserInfo($nick) {
	 ...
	 * @return array
	 */
	public function _getSellerInfo($nick) {
		if (strpos($nick, '*') !== false) return array();
	 ...
	 * @return array
	 */
	public function _getUserInfos(array $nicks) {
		$filtedNicks = array();
	 ...
	 * @return array
	 */
	public function _searchItems(array $params) {
	 ...
	 * @return array
	 */
	public function _getItemInfo($numIid) {
		$fields = array(
			/**
			 * 必选字段
			 */
			...
			'is_prepay','ww_status','wap_desc','wap_detail_url','after_sale_id','cod_postage_id','sell_promise','promotion_price'
		);
	 ...
	 * @param array $numIids
	 */
	public function _getItemInfos($numIids) {
		$fields = array(
				'detail_url','num_iid','title'/* ,'desc' */,'sku','props_name','props','cid','pic_url','num','volume',
				'price','post_fee','express_fee','ems_fee','has_discount','freight_payer','postage_id',
				'nick','score','created','location','item_img','prop_imgs','product_id','second_kill','is_virtual',
	 ...
	 */
	public function _getTaokeItemInfo($params) {
		$params = array_merge(
				$params,
				array(
	 ...
	 * @param unknown_type $params
	 */
	public function _getTbkItemInfo($params) {
		$params = array_merge(
				$params,
				array(
						'fields' => 'num_iid,nick,title,price,item_location,seller_credit_score,click_url,mall_click_url,pic_url,commission_rate,commission,commission_num,commission_volume,volume',
	 ...
	 * 获取某件商品的淘宝客的链接地址
	 */
	public function _getTaobaokeClickUrl($numIid) {
	 ...
	 */
	public function _getTaobaokes($params) {
		$params = array_merge(
				$params,
				array(
	 ...
	 * @param array $numIids 淘宝客商品数字id串
	 */
	public function _getTaobaokeItemDetail($params) {
		$params = array_merge(
				$params,
				array(
						'fields' => 'commission_rate,iid,num_iid,title,nick,pic_url,price,click_url,commission,commission_num,commission_volume,mall_click_url,seller_credit_score,item_location,volume,taobaoke_cat_click_url,keyword_click_url,promotion_price',
		...
		$items = $this->taobao_taobaoke_items_detail_get($params);
	
		if (!isset($items['taobaoke_item_details'])) return array();
		return $items['taobaoke_item_details']['taobaoke_item_detail'];
	}
	 ...
	 * @param array $params
	 */
	public function _getTaokeReport($params) {
	 ...
	 */
	public function _getTaobaoke($params) {
	 ...
	 * 淘宝店铺转换
	 */
	public function _TaobaokeShopsConvert($params) {
		$params = array_merge(
				$params,
				array(
						'fields' => 'seller_nick,user_id,shop_title,click_url,commission_rate,seller_credit,shop_type,total_auction,auction_count',
	 ...
	 * 淘宝店铺转换
	 */
	public function _TaobaokeMobileShopsConvert($params) {
		$params = array_merge(
				$params,
				array(
						'fields' => 'seller_nick,user_id,shop_title,click_url,commission_rate,seller_credit,shop_type,total_auction,auction_count',
		...
		$shops = $this->taobao_taobaoke_mobile_shops_convert($params);
		$shop_list = $shops['taobaoke_shops']['taobaoke_shop'];
		if (!isset($shop_list)) return array();
		if (isset($shop_list['auction_count'])) {
			$shop_list['credit_img'] = $this->getLevelImg($shop_list['seller_credit']);
	 ...
	 * 商品关联推荐
	 */
	public function _taobaokeItemsRelate($params) {
		$params = array_merge(
				$params,
				array(
	 ...
	 * 无线商品关联推荐
	 */
	public function _taobaokeMobileItemsRelate($params) {
		$params = array_merge(
				$params,
				array(
						'fields' => 'num_iid,seller_id,nick,title,price,item_location,seller_credit_score,click_url,shop_click_url,pic_url,taobaoke_cat_click_url,keyword_click_url,coupon_rate,coupon_price	,coupon_start_time,coupon_end_time,commission_rate,commission,commission_num,commission_volume,volume,shop_type,promotion_price',
		...
		$items = $this->taobao_taobaoke_mobile_items_relate_get($params);
		return isset($items['taobaoke_items']['taobaoke_item']) ? $items['taobaoke_items']['taobaoke_item'] : array();
	}
	 ...
	 * 无线淘客商品转换
	 */
	public function _tbkMobileItemsConvert($params) {
		$params = array_merge(
				$params,
				array(
		...
		return isset($items['tbk_items']['tbk_item']) ? $items['tbk_items']['tbk_item'] : array();
	}
	 ...
	 * @return array
	 */
	protected function _findTaobaokes(array $params) {
	 ...
	 * @return array
	 */
	protected function _findItems(array $params) {
	 ...
	 * 天猫折扣精选商品库
	 *
	 * @param array tmall.items.discount.search 参数
	 * @return array
	 */
	protected function _tmallItemsDiscountSearch(array $params) {
		/*$params = array_merge(
				$params,
				array(
						'fields' => 'total_results,total_page,page_size,page,param_value,item_list,cat_list,brand_list,minisite_list,search_url',
				)
		);*/
	 ...
	 * @param $pageSize 每页显示的条数，允许值：5、10、20、40
	 */
	protected function _getTraderatesDetail($numIid, $page = 1, $pageSize = 20) {
	 ...
	 * @return array
	 */
	protected function _getItemInfoDetail($numIid) {
	 ...
	 * @return unknown
	 */
	public function _getTaoBaoItemCats($cid = 0) {
		$params = array (
	 ...
	 * @return array
	 */
	public function _getTaobaokeItems($nick, array $params) {
	 ...
	 * @return array
	 */
	public function _taobaoTbkItemsGet(array $params) {
		if (!array_key_exists('keyword', $params) && !array_key_exists('cid', $params)) return array();
		$params = array_merge(
				$params,
				array(
	 ...
	 * 返利授权查询
	 *
	 * @param array taobao.taobaoke.rebate.auth.get参数
	 * @return array
	 */
	protected function getTaobaoTaobaokeRebateAuth(array $params) {
		$items = $this->taobao_taobaoke_rebate_auth_get($params);
		if (!isset($items['results']['taobaoke_authorize'])) return array();
	
		if (isset($items['results']['taobaoke_authorize']['param'])) {
			$items['results']['taobaoke_authorize'] = array($items['results']['taobaoke_authorize']);
		}
		return $items['results']['taobaoke_authorize'];
	 ...
	 * @param array $params
	 */
	public function _getTaokeRebateReport($params) {
		$params = array_merge($params, array(
				'fields' => 'trade_parent_id,trade_id,real_pay_fee,commission_rate,commission,app_key,outer_code,create_time,pay_time,pay_price,num_iid,item_title,item_num,category_id,category_name,shop_title,seller_nick',
		));
		$response = $this->taobao_taobaoke_rebate_report_get($params);
		if (!isset($response['taobaoke_payments']['taobaoke_payment'])) return array();
		if (isset($response['taobaoke_payments']['taobaoke_payment']['category_id'])) {
				$response['taobaoke_payments']['taobaoke_payment'] = array($response['taobaoke_payments']['taobaoke_payment']);
		}
		return $response['taobaoke_payments']['taobaoke_payment'];
	}
	 ...
	 * @return array
	 */
	public function _getTbkShopInfo(array $params) {
	    $params = array_merge(
    	    $params,
    	    array(
	    ...
	    $items = $this->taobao_tbk_shops_detail_get($params);
	    if (!isset($items['tbk_shops']['tbk_shop'])) return array();
	    
	    return $items['tbk_shops']['tbk_shop'];;
	}
	
	
	/**
	 * 获取授权访问令牌
	 * @param 	alipay.system.oauth.token 参数
	 */
	public function _alipaySystemOauthToken($params) {
		$params = array_merge($params, array(
				'request_method' => 'https'
		));
		$response = $this->alipay_system_oauth_token($params);
		print_r($response);
	}
