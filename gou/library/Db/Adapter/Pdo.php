...
class Db_Adapter_Pdo {
	...
	public static function quote($string, $parameter_type = null) {
		//return is_numeric($string)?$string:self::getPDO()->quote($string, $parameter_type);
	...
	public static function _where($field, $op, $value) {
		...
		switch ($op) {
			...
			case "IN":
				...
				break;
			case "LIKE":
				$str .= sprintf("%s LIKE %s", self::fieldMeta($field), self::quote("%" . self::_filterLike($value) . "%"));
				break;
			case "=" :
	 ...
	 * @return string
	 */
	public static function _filterLike($keyWord) {
		$search = array('[','%', '_', '/');
		$replace = array('[[]', '[%]', '[_]', '[/]');
     ...
     * @return bool|
     */
    private static function _getPDOLog($stmt){
        $log_switch = Common::getConfig('siteConfig', 'pdoLogSwitch');
        if(!$log_switch) return false;

        $log_file = Common::getConfig('siteConfig', 'pdoLog');
        ob_start();
        $stmt->debugDumpParams();
        $pdo_log = ob_get_contents();
        $pdo_log = preg_replace('/\nParams:.*\n/isU', '', $pdo_log);
        Common::log($pdo_log, $log_file);
        ob_end_clean();
//        register_shutdown_function(array('Db_Adapter_Pdo', 'webPDOLog'), $pdo_log);
    }

    public static function webPDOLog($log){
        header('Content-type:text/html');
        echo "<p style=\"background: #e5e5e5; margin: 0; margin-top: 5px; color: #555555; font-size: 14px; padding: 5px;\">".$log."</p>";
    }
}