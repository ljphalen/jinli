...
class Client_Service_Category{
	 ...
	 */
	public function getCanUseCategorys($page, $limit, $params = array()) {
		$params = self::_cookData($params);
		if(intval($page) < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::_getDao()->getCanUseCategorys(intval($start), intval($limit), $params);
		$total = self::_getDao()->getCanUseCategoryCount($params);
		return array($total, $ret);
	}
