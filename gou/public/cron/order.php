...
$session = Gou_Service_Config::getValue('GOU_ADMIN_TOKEN');
...
// 			Common::sms($mobiles, '[' . ENV . ']' . '淘宝客session已失效,请及时登录后台.');
		...
		if ($total) {
			...
			foreach($result as $key=>$value) {
				//查询日志，如果已经返还，不再返还
				$log = Gou_Service_TaokeCoinLog::getCoinLogByTradeId($value['trade_id']);
				if ($log) continue;
				
				//计算返银币数量
				$fanli_rate = Gou_Service_Config::getValue('gou_silver_fanli_rate');
				$coin = Common::money($value['commission'] * $rate * 2);
				if ($coin == 0) continue;
...
// 				$uid = 1;
				...
				$return = Api_Gionee_Pay::coinAdd(array(
						...
						'msg'=>"购买淘客商品，奖励".Common::money($coin)."银币。"));
				Gou_Service_TaokeCoinLog::addCoinLog(array(
					'uid'=>$uid,
					'out_uid'=>$user['out_uid'],
