 ...
 *
 */
class TejiaController extends Front_BaseController {
	...
	public function redirectAction(){
        ...
        //需要特殊处理
        if ($channel_type == 6) {
            $goodsUrl = str_replace('gionee3', 'gionee8', html_entity_decode($goodsInfo['link']));
        }

        $configs = Common::getConfig('tejiaConfig');
        $config = $configs[$channel_type];
        $goodsUrl = sprintf($config['formart'], $goodsUrl);
