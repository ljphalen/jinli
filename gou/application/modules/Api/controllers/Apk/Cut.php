 ...
 *
 */
class Apk_CutController extends Api_BaseController {
	...
	public function cutAction() {
	    ...
	    $status['is_client'] = $android_uid ? true : false;
		...
		$ret['is_score'] = false;
		if($android_uid){
			$s_ret = Gou_Service_ScoreLog::score(2, $android_uid);
			if($s_ret) {
	    ...
	    $this->output(0, '砍掉'.$ret['range'].'元，再找朋友一起来吧', $ret);
	}
	...
	public function getStatusAction() {
	    ...
	    $buy_url = Common::getWebRoot().'/cut/buy?id='.$goods['id'];
	    $cut_url = Common::getWebRoot().'/api/apk_cut/cut?id='.$goods['id'].'&fuid='.$fuid;
