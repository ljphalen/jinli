 ...
 *
 */
class Apk_CodController extends Api_BaseController {
	...
	public function indexAction() {
		...
		foreach ($types as $key=>$value) {
			...
			$data[$key]['link'] = Cod_Service_Type::getShortUrl(Stat_Service_Log::V_APK, $value);
			
			//pic
			$img_guides[$key] = Cod_Service_Guide::getCanUseImgGuides(0, 1, array('ptype'=>$value['id'], 'channel_id'=>2));
			...
			$data[$key]['img_data']['link'] =  Cod_Service_Guide::getShortUrl(Stat_Service_Log::V_APK, $img_guides[$key][0]);
			
			//text
			$text_guides[$key] = Cod_Service_Guide::getCanUseTextGuides(0, 10, array('ptype'=>$value['id'], 'channel_id'=>2));
			$text_data = array();
			$n = 1;
			foreach ($text_guides[$key] as $k=>$val) {
				$text_data[$k]['title'] = Util_String::substr($val['title'], 0, 10, true);
				$text_data[$k]['link'] =  Cod_Service_Guide::getShortUrl(Stat_Service_Log::V_APK, $val);
	...
	public function searchAction() {
		...
		//hash
		$apk_cod_search =  json_decode(Gou_Service_Config::getValue('apk_cod_search'), true);
		$action = Common::tjurl(Stat_Service_Log::URL_SEARCH, Stat_Service_Log::V_APK, $apk_cod_search['module_id'], $apk_cod_search['channel_id'], 0, $apk_cod_search['url'], 'apk货到付款搜索', $apk_cod_search['channel_code']);
