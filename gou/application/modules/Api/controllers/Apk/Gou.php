 ...
 *
 */
class Apk_GouController extends Api_BaseController {
	...
	public $perpage = 4;
	public $cacheKey = 'Apk_Index_index';
	...
	public function adAction() {
		list(, $ads) = Gou_Service_Ad::getCanUseAds(1, 3, array('ad_type'=>1, 'channel_id'=>2));
		$webroot = Common::getWebRoot();
		$tjUrl = $webroot.$this->actions['tjUrl'];
		$data = $imgs = array();
		foreach ($ads as $key=>$value) {
			$data[$key]['title'] = Util_String::substr($value['title'], 0, 18, '', true);
			$imgs[] = $data[$key]['img'] = Common::getAttachPath() . $value['img'];
			$data[$key]['link'] = Gou_Service_Ad::getShortUrl(Stat_Service_Log::V_APK, $value);
		}
		$this->cache($imgs, 'ad');
		$this->output(0, '', $data);
	}
	...
	public function linkAction() {
		$webroot = Common::getWebRoot();
		$tjUrl = $webroot.$this->actions['tjUrl'];
		list(, $ad1) = Gou_Service_Ad::getCanUseAds(1, 2, array('ad_type'=>3, 'channel_id'=>2));
		list(, $ad2) = Gou_Service_Ad::getCanUseAds(1, 2, array('ad_type'=>4, 'channel_id'=>2));
		...
		foreach ($ad1 as $key=>$value) {
		    ...
		    $dataAd1[$key]['link'] = Gou_Service_Ad::getShortUrl(Stat_Service_Log::V_APK, $value);
		}
		...
		foreach ($ad2 as $key=>$value) {
		    ...
		    $dataAd2[$key]['link'] = Gou_Service_Ad::getShortUrl(Stat_Service_Log::V_APK, $value);
		}
	...
	public function convenientAction() {
		...
		if ($version <= $server_version){
			$webroot = Common::getWebRoot();
			$tjUrl = $webroot.$this->actions['tjUrl'];
			list(, $ad) = Gou_Service_Ad::getCanUseAds(1, 4, array('ad_type'=>5, 'channel_id'=>2));
			$dataAd = $imgs = array();
			if($ad) {
	...
	public function recommendAction() {
		list(, $recommend) = Gou_Service_Channel::getCanUseChanels(0, 4, array('type_id'=>2, 'channel_id'=>2));
		$webroot = Common::getWebRoot();
		$tjUrl = $webroot.$this->actions['tjUrl'];
		$data = $imgs = array();
		foreach ($recommend as $key=>$value) {
			$imgs[] = $data[$key]['img'] = Common::getAttachPath() . $value['img'];
			$data[$key]['link'] = Gou_Service_Channel::getShortUrl(Stat_Service_Log::V_APK, $value);
		}
	...
	public function mallAction() {
		list(, $mall) = Gou_Service_Channel::getCanUseChanels(0, 8, array('type_id'=>1, 'channel_id'=>2));
		$webroot = Common::getWebRoot();
		$tjUrl = $webroot.$this->actions['tjUrl'];
		$data = $imgs = array();
	...
	public function helperAction() {
		list(, $mall) = Gou_Service_Channel::getCanUseChanels(0, 4, array('type_id'=>3, 'channel_id'=>2));
		$webroot = Common::getWebRoot();
		$tjUrl = $webroot.$this->actions['tjUrl'];
		$data = $imgs = array();
		foreach ($mall as $key=>$value) {
	...
	public function themeAction() {
		list($total, $mall) = Gou_Service_Channel::getCanUseChanels(0, 8, array('type_id'=>4, 'channel_id'=>2));
		$webroot = Common::getWebRoot();
		$tjUrl = $webroot.$this->actions['tjUrl'];
		$data = $imgs = array();
		foreach ($mall as $key=>$value) {
		...
		if(strpos($ua, 'E6 ') || strpos($ua, 'GiONEE-E6')) {
			$data[$count - 1] = array('name'=>'限时抢','link'=>Common::tjurl($tjUrl, 82, 'CHANNEL', 'http://12650.mmb.cn/wap/touch/groupRate.do?type=2', 13, $count));
	...
	public function tuanAction() {
		...
		list(, $mall) = Gou_Service_Channel::getCanUseChanels(0, 4, array('type_id'=>5, 'channel_id'=>2));
		$webroot = Common::getWebRoot();
		$tjUrl = $webroot.$this->actions['tjUrl'];
		$data = $imgs = array();
		foreach ($mall as $key=>$value) {
