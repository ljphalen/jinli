 ...
 *
 */
class TejiaController extends Api_BaseController {
		...
		if ($version != $server_version){
			...
			if($goodsList) {
				foreach ($goodsList as $key=>$val){
					...
					'link'=>Client_Service_Channelgoods::getShortUrl(Stat_Service_Log::V_H5, $val),
					'img'=>$img
				);
					if($val['sale_price']!=$val['market_price']){
						$data[$key]['market_price']=$val['market_price'];
					}
				}
			}
