 ...
 *
 */
class App_CodController extends Api_BaseController {
	...
	public function indexAction() {
		...
		foreach ($types as $key=>$value) {
			...
			$data[$key]['link'] = Cod_Service_Type::getShortUrl(Stat_Service_Log::V_APP, $value);
			
			//pic
			$img_guides[$key] = Cod_Service_Guide::getCanUseImgGuides(0, 1, array('ptype'=>$value['id'], 'channel_id'=>5));
			...
			$data[$key]['img_data']['link'] =  Cod_Service_Guide::getShortUrl(Stat_Service_Log::V_APP, $img_guides[$key][0]);
			
			//text
			$text_guides[$key] = Cod_Service_Guide::getCanUseTextGuides(0, 10, array('ptype'=>$value['id'], 'channel_id'=>5));
			$text_data = array();
			$n = 1;
			foreach ($text_guides[$key] as $k=>$val) {
				$text_data[$k]['title'] = Util_String::substr($val['title'], 0, 10, true);
				$text_data[$k]['link'] =  Cod_Service_Guide::getShortUrl(Stat_Service_Log::V_APP, $val);
	...
	public function searchAction() {
		...
		//hash
		$app_cod_search =  json_decode(Gou_Service_Config::getValue('app_cod_search'), true);
		$action = Common::tjurl(Stat_Service_Log::URL_SEARCH, Stat_Service_Log::V_APP, $app_cod_search['module_id'], $app_cod_search['channel_id'], 0, $app_cod_search['url'], 'APP货到付款搜索', $app_cod_search['channel_code']);
