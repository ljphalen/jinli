...
class SubjectController extends Front_BaseController {
	...
	public function goodsAction() {
		...
		foreach ($goods as $key=>$value) {
			...
			$goods[$key]['img'] = strpos($value['img'], 'http://') === false ? $attachsPath .$value['img'] : $value['img'].'_200x200.'.end(explode(".",  $value['img']));
	...
	public function moreAction() {
		...
		foreach($goods as $key=>$value) {
			...
			if(strpos($value['img'], 'http://') === false) {
				$temp[$key]['img'] = $attachsPath . $value['img'];
			}else{
