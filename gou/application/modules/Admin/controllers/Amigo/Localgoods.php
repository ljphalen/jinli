 ...
 *
 */
class Amigo_LocalGoodsController extends Admin_BaseController {
	
	public $actions = array(
		'listUrl' => '/Admin/Amigo_LocalGoods/index',
		'addUrl' => '/Admin/Amigo_LocalGoods/add',
		'addPostUrl' => '/Admin/Amigo_LocalGoods/add_post',
		'editUrl' => '/Admin/Amigo_LocalGoods/edit',
		'editPostUrl' => '/Admin/Amigo_LocalGoods/edit_post',
		'deleteUrl' => '/Admin/Amigo_LocalGoods/delete',
		'uploadUrl' => '/Admin/Amigo_LocalGoods/upload',
		'uploadPostUrl' => '/Admin/Amigo_LocalGoods/upload_post',
		'uploadImgUrl' => '/Admin/Amigo_LocalGoods/uploadImg',
		'deleteImgUrl' => '/Admin/Amigo_LocalGoods/deleteimg',
	);
	...
	public function editAction() {
		...
		list(,$goods_imgs) = Gou_Service_LocalGoodsImg::getList(0, 100, array('goods_id'=>intval($id)), array('id'=>'ASC'));
	...
	public function add_postAction() {
		$info = $this->getPost(array('sort', 'title', 'short_desc', 'img', 'gold_coin',
				'silver_coin', 'price', 'supplier', 'start_time', 'end_time', 
				'iscash', 'is_sale','is_praise', 'stock_num', 'limit_num','purchase_num', 'show_type',
				'is_new_user','isrecommend','status','descrip', 'goods_type','keywords', 'img_thumb'));
		
		if (!$info['img']) $this->output('-1', '至少上传一张商品图片.');
		
		$simg = $this->getPost('simg');
		if (!$simg[1]) $this->output('-1', '至少上传一张商品详情图片.');
	...
	public function edit_postAction() {
		$info = $this->getPost(array('id', 'sort', 'title', 'short_desc', 'img', 'gold_coin',
				'silver_coin', 'price', 'supplier', 'start_time', 
				'end_time', 'iscash','is_sale','is_praise', 'stock_num', 'limit_num',
				'purchase_num', 'is_new_user','isrecommend','show_type',
				'status','descrip', 'goods_type','keywords', 'img_thumb'));
		$info = $this->_cookData($info);
		
		$upImgs = $this->getPost('upImg');
		$simg = $this->getPost('simg');
		if (!$simg && !$upImgs) 
			$this->output('-1', '至少上传一张预览图片.');
	...
	private function _cookData($info) {
		...
		if (!$info['short_desc']) $this->output(-1, '商品简介不能为空.');
