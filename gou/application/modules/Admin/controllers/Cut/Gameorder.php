 ...
 *
 */
class Cut_GameorderController extends Admin_BaseController {
	
	public $actions = array(
		'indexUrl' =>'/admin/Cut_gameorder/index',
		'editUrl'=>'/admin/Cut_gameorder/edit',
		'editPostUrl'=>'/admin/Cut_gameorder/edit_post',
		'addUrl'=>'/admin/Cut_gameorder/add',
		'exportUrl'=>'/admin/Cut_gameorder/export',
		'addPostUrl'=>'/admin/Cut_gameorder/add_post',
		'syncUrl'=>'/admin/Cut_gameorder/sync',
		'editOrderAjax'=>'/admin/Cut_gameorder/editorder',
	   'refundUrl'=>'/admin/Cut_gameorder/refund',
	);
	
	// 0.Cut商城使用，1.积分换购使用, 2.话费充值, 3.砍价, 4.游戏版砍价
	public $show_type = 4;
	public $perpage = 20;
	...
	public function indexAction() {
		...
		$params  = $this->getInput(array('buyer_name', 'status', 'phone', 
				'trade_no', 'goods_title', 'start_time', 'end_time',
				'order_by', 'order_type'));
		if ($page < 1) $page = 1;
		...
		if ($params['goods_title']) {
		    list($total, $goods) = Cut_Service_Goods::getsBy(array('title'=>array('LIKE', $params['goods_title'])));
		    if($total) {
		        $goods = Common::resetKey($goods, 'id');
		        $ids = array_keys($goods);
		        $search['goods_id'] = array('IN', $ids);
		    }
		...
		$goods = Cut_Service_Goods::getGoodsByIds(array_unique($goods_ids));
		$goods = Common::resetKey($goods, 'id');
	...
	public function editorderAction() {
		...
		if ($data['isaddress'] == 0){
			...
			if ($editdata['cloumn'] == 'number' && $orderInfo['number'] != $editdata['value']){
				$goods = Cut_Service_Goods::getGoods($orderInfo['goods_id']);
				$updateData = array(
...
//		}
		...
		if($order['status'] != 4 && ($status == 4)) {
			...
			if($express_code[0] && $express_code[1]) {
				Common::sms($order['phone'], '亲，您好。您在购物大厅参加【拼速度，赢大奖】活动中赢取的奖品已经发货，物流公司:'.$express_code[0].'，运单号：'.$express_code[1].'，打开购物大厅，进入我的获奖记录可查看物流 ~感谢亲对购物大厅的支持！');
			}
		}
	...
	public function exportAction() {
		...
		list($total, $order) = Gou_Service_Order::getsBy($search, array($order_by=>'DESC'));
	...
	public function refundAction() {
	    $id = $this->getInput('id');
	    $order = Gou_Service_Order::getOrder($id);
	    if(!$order) $this->redirect($this->actions['indexUrl']);
	    Cut_Service_Order::refundOrder($order['trade_no']);
	}
}
