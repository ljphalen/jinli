...
class WantlogController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		//get logs
		list($total, $logs) = Gou_Service_WantLog::getWantLogsByTime($page, $this->perpage, $search);
	...
	public function orderfreeAction() {
		...
		if ($params['goods_id']) {
			...
			//get users
			$users = Gou_Service_User::getUserByUids($uids);
	...
	public function editAction() {
		$id = $this->getInput('id');
		$info = Gou_Service_WantLog::getWantLog(intval($id));
	...
	public function edit_postAction() {
		...
		$ret = Gou_Service_WantLog::updateWantLog(array('order_id'=>$info['order_id'], 'status'=>1), intval($info['id']));
	...
	public function orderfree_postAction() {
		...
		list($total, $logs) = Gou_Service_WantLog::getByGoodsId($info['goods_id']);
		if ($info['number'] > $total) $this->output(-1, '中奖人数不能大于喜欢总数.');
		//上次的编号
		$OrderFreeNumber = Gou_Service_OrderFreeNumber::getLast();
		$number = $OrderFreeNumber['number'] + 1;
		//插入期数
		$ret = Gou_Service_OrderFreeNumber::addOrderFreeNumber(array('number'=>$number));
		if(!$ret) $this->output(-1, '抽奖失败.'); 
		
		//商品信息
		$goods = Gou_Service_Goods::getGoods($info['goods_id']);		
		...
		$result = Gou_Service_OrderFreeLog::batchAddOrderFreeLog($data);
		//更新会员免单数
		$ret = Gou_Service_User::updateFreeNumberByIds($uids);
