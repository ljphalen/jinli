...
//获取所有优惠券的批次
function getCouponBatch($id){
	$coupon = D("Coupon://CouponBatch")->where(array('is_delete'=>0))->field(array('id','title','rate'))->order("id desc")->select();
	if (!empty($coupon)) {
		$select = '<select id="'.$id.'" name="'.$id.'" style="width:130px;">';
