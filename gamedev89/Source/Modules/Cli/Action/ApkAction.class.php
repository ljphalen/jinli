...
class ApkAction extends CliBaseAction
{
	...
	protected function bsdiff()
	{
		...
		foreach ($tmp as $app_id => $apks)
		{
			...
			foreach($apks as $apk_id)
			{
				...
				$map = array("status"=>array("in", "-2,2,3"), "file_size"=>array("lt", C('APK.MAX_BSDIFF_SIZE')), "app_id"=>$app_id, "id"=>array("lt", $apk_id));
				...
				foreach ($apk as $small)
				{
					...
					$this->printf("%d_%d.patch start", $_apk_id, $apk_id);
					...
					$this->printf("%d_%d.patch done", $_apk_id, $apk_id);
				}
			}
		}
	}
	...
	protected function Signature($apk_id, $apk)
	{
		...
		if(!is_file($save_apk))
		{
			Log::write("apk sign apk not exist", Log::EMERG);
			Log::write($save_apk, Log::EMERG);
		...
		{
			Log::write("apk sign shell failure", Log::EMERG);
			Log::write($shell, Log::EMERG);
			Log::write($result, Log::EMERG);
			return array("id"=>$apk_id, "sign"=>-1);
		}
		...
		{
			Log::write("apk signed file not exist", Log::EMERG);
			Log::write($save_apk, Log::EMERG);
			Log::write($signed_apk_path, Log::EMERG);
			return array("id"=>$apk_id, "sign"=>-1);
		}
	
		return array("id"=>$apk_id, "sign"=>1, "file_path"=>$signed, "apk_md5"=>md5_file($signed_apk_path));
	}
	...
	protected function patch(string $old, string $new, string $patchfile)
	{
		...
		Log::write($cmd, Log::EMERG);
		Log::write($new, Log::EMERG);
		Log::write($patchfile, Log::EMERG);
