		...
		if (!empty($list))
		{
			foreach ($list as $key => $val)
			{
				...
				if($val['receiver_account'] == MessageModel::RECEIVE_ALL)  //所有用户
				{
					...
					//首先把状态改为发送中
				 	$message_update = array('state' => MessageModel::STATE_LOAD,
				 							   'send_time' => time() 
				 								);
				 	$message_m->where('id='.$val['id'])->save($message_update);
					while (1)
					{
						$user_map = array(
							'status' => AccountsModel::STATUS_PASS,
							'id' => array('GT',$account_id),
						  );
						 $user_list = $accounts_m->where($user_map)->order('id asc')->limit(0,100)->select();  
						 if (!empty($user_list))
						 {
					 ...
					 if (!empty($user_list))
						 {
						 	...
						 	{
						 		$user_val = $accounts_m->find($user_id);
						 		$this->_sendMessage($val, $user_val);
						 	}
						 }
				}
				 ...
				 */
				//更新发送到id
			 	$message_update = array('state' => MessageModel::STATE_COMPLATE,
			 							   'now_account' => $user_val['id'], 
			 							   'send_time' => time() 
			 								);
			 	$message_m->where('id='.$val['id'])->save($message_update);
