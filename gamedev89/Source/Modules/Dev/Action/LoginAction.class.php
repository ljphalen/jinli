...
class LoginAction extends BaseAction {
	...
	public function login() {
		...
		if ($uid) {
			$this->redirect ( $this->firstPage );
			exit;
		}
		
		//读取首页封面
		$file = sprintf("%s%s", helper("Apk")->get_path("image"), "index-background-slide.png");
		if(is_file($file))
			//$this->assign("slideimage", sprintf("%s%s?%s", helper("Apk")->get_url("image"), "index-background-slide.png", time()));
			$this->assign("slideimage", sprintf("%s%s", helper("Apk")->get_url("image"), "index-background-slide.png"));
	...
	public function loginsub() {
		...
		if (empty ( $email )) {
			...
			$this->error ( '请输入密码！' );
		} elseif (empty ( $_SESSION ['verify'] ) || $_SESSION ['verify'] != md5 ( strtoupper ( $_POST ['authcode'] ) )) {
			$_SESSION ['verify'] = "";
			$this->error ( '验证码错误！' );
		}
		
		// 是否保存用户名
		$this->saveAccount = (isset ( $_POST ['saveAccount'] ) && $_POST ['saveAccount'] == 'Y') ? true : false;
		// 是否自动登录
		$this->autoLogin = (isset ( $_POST ['autoLogin'] ) && $_POST ['autoLogin'] == 'Y') ? true : false;
	...
	public function regsub() {
		...
		if ($authId) {
			...
			if ($res) {
				...
				$this->redirect ( "Login/confirm", array (
						'email' => $email 
				) );
			}
		}
...
//		}
	...
	public function prefectsub() {
		//$authId = $this->uid?$this->uid:cookie ( 'authId' );
	...
	function _checkEmail() {
		$email = $this->_request ( "email", "trim", "" );
