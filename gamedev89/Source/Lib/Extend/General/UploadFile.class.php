...
{//类定义开始
    ...
    // 文件名称保存方式(1:上传时的名称;2:time();3:自定义)
    public $saveFileName = 1;
    ...
    private function save($file)
    {
        ...
        if($this->charset !== "utf-8"){
        	$destination = auto_charset($filename,'utf-8',$this->charset);
        }
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                if(function_exists($this->hashType)) {
                    $fun =  $this->hashType;
                    $saveName = ($this->charset!=="utf-8") ? auto_charset($file['savepath'].$file['savename'],'utf-8',$this->charset) : $file['savepath'].$file['savename'];
                    $file['hash']   =  $fun($saveName);
                }
                //上传成功后保存文件信息，供其他地方调用
