...
class AdController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $ads) = Games_Service_Ad::getList($page, $perpage, array('ad_type'=>1));
	...
	public function editAction() {
		$id = $this->getInput('id');
		$info = Games_Service_Ad::getAd(intval($id));
		$this->assign('ad_types', $this->ad_types);
	...
	public function add_postAction() {
		$info = $this->getPost(array('sort', 'title', 'ad_type', 'ptype', 'gameid', 'link', 'img', 'start_time', 'end_time', 'status'));
		$info = $this->_cookData($info);
		$result = Games_Service_Ad::addAd($info);
	...
	public function edit_postAction() {
		$info = $this->getPost(array('id', 'sort', 'title', 'ad_type', 'ptype', 'gameid', 'link', 'img', 'start_time', 'end_time', 'status'));
		$info = $this->_cookData($info);
		$ret = Games_Service_Ad::updateAd($info, intval($info['id']));
	...
	private function _cookData($info) {
		...
		if ($info['ptype'] == 1 && !$info['gameid']) {
			$this->output(-1, '游戏ID不能为空.');
		}
		$game = Games_Service_Game::getGame($info['gameid']);
		if (!$game) $this->output(-1, '游戏ID不存在.');
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Games_Service_Ad::getAd($id);
		if ($info && $info['id'] == 0) $this->output(-1, '无法删除');
		Util_File::del(Common::getConfig('siteConfig', 'attachPath') . $info['img']);
		$result = Games_Service_Ad::deleteAd($id);
		if (!$result) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
