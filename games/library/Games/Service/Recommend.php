...
class Games_Service_Recommend{
	 ...
	 */
	public static function getCanUseRecommends($page, $limit, $params = array()) {
		$params = self::_cookData($params);
		if(intval($page) < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::_getDao()->getCanUseRecommends(intval($start), intval($limit), $params);
		$total = self::_getDao()->getCanUseRecommendCount($params);
	...
	private static function _cookData($data) {
		...
		if(isset($data['gameid'])) $tmp['gameid'] = intval($data['gameid']);
	...
	/**
	 * 
	 * @return Games_Dao_Recommend
	 */
	private static function _getDao() {
		return Common::getDao("Games_Dao_Recommend");
	}
}
