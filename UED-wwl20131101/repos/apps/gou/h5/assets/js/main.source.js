(function(iCat){
	...
	iCat.mix(Gou, {
		init: function(){
			...
			//搜索页聚焦
			var ipt = $('#iScroll .search-bar input'),
				v = ipt.val();
			ipt.val('').focus().val(v);

			//focus
			if(!/app\.|client\./i.test(location.host)){
			...
			if(document.body.id==='prize'){
				...
				/*iCat.include('http://v3.jiathis.com/code/jia.js?uid=1!', function(){
					Event.on('#J_createLink button', '@singleTap', function(){
						var input = $('#J_createLink input'),
							val = input.val(), ajaxUrl = input.attr('data-ajaxurl');
						if(!val){
						...
						iCat.util.ajax({
							...
							success: function(d){
								if(d.success){
									$('#J_linkResult').removeClass('hidden')
										  .find('span').html(d.data.url);
									window.jiathis_config = {
										siteNum:14,
										sm:"tsina,tqq,qzone",
										url:d.data.url,
										summary:"小伙伴们，点击下载购物大厅，从此免装淘宝天猫京东，购物更轻松，还能100%拿Q币。另有话费、实物多重大奖等你来拿！"+d.data.url,
										title:"#邀请好友送话费#",
										boldNum:4,
										showClose:true,
										hideMore:true
				...
				/*iCat.include(
					['http://static.bshare.cn/b/buttonLite.js!', 'http://static.bshare.cn/b/bshareC0.js!'],
					function(){
						bShare.addEntry({
		...
		waterFall: function(){
			var wfWrap = $('.J_waterFall'), scrollPannel = $('#iScroll'),
					...
					if(data.success){
						...
						if(curData.length){
							scrollPannel.scroll(function(){
								slHeight = this.scrollHeight;
								slTop = this.scrollTop;
	...
	iCat.include(['lib/jquery/jquery.js', './plugin/gngou.new.js'], function(){	
