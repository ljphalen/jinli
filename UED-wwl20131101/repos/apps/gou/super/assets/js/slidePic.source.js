	...
	$.extend($.fn, {
		slidePic: function(options){
			...
			var me = $(this), timer,
				...
				siWidth = Math.ceil(slideItem.width()), threshold = Math.round(siWidth/4),
			...
			next.on('click', function(){
				var curIndex = me.data('curIndex'),
					...
					if(index==steps-1) next.removeClass('actived');
					if(curIndex == steps -1){
						return;
					}
			...
			slidePanel.swipe({
				swipeStatus: function(evt, phase, direction, distance, duration, fingerCount){
					if(phase=='start' || phase=='move'){
						clearTimeout(timer);
						if(opt.disableFirst == true && me.data('curIndex') == 0){
							return;
						}

						if(opt.disableLast == true && me.data('curIndex') == steps -1){
							return;
						}
						...
						if(opt.disableFirst == true && me.data('curIndex') <= 0 && direction == 'right'){
							return;
						}
			...
			function slide(index, dir, distance, duration){
				...
				if(index>=steps){
					...
					if(opt.circle && dir=='left'){
						...
						/*style.MozTransform = style.webkitTransform = 'translate3d(' + (-(idx*siWidth+siWidth/3)) + 'px,0,0)';
						style.msTransform = style.OTransform = 'translateX(' + (-(idx*siWidth+siWidth/3)) + 'px)';*/
						setTimeout(function(){
							/*style.MozTransform = style.webkitTransform = 'translate3d(' + (-idx*siWidth) + 'px,0,0)';
							style.msTransform = style.OTransform = 'translateX(' + (-idx*siWidth) + 'px)';*/
					...
					if(dir=='right'){
						/*style.MozTransform = style.webkitTransform = 'translate3d(' + siWidth/3 + 'px,0,0)';
						style.msTransform = style.OTransform = 'translateX(' + siWidth/3 + 'px)';*/
						setTimeout(function(){
							/*style.MozTransform = style.webkitTransform = 'translate3d(0px,0,0)';
							style.msTransform = style.OTransform = 'translateX(0px)';*/
			...
			function tmove(dir, distance, duration){
				...
				/*if(index==0 && distance>=siWidth/3)
					distance = siWidth/3;
				else if(index==steps-1 && distance>=siWidth/3)
					distance = siWidth/3;*/

				var dis = dir=='left'? distance : -distance;

				if(index==0 && dir=='right'){
					style.MozTransform = style.webkitTransform = 'translate3d(0px,0,0)';
					style.msTransform = style.OTransform = 'translateX(0px)';
				} else if(index==steps-1 && dir=='left') {
					style.MozTransform = style.webkitTransform = 'translate3d(' + (-index*siWidth) + 'px,0,0)';
					style.msTransform = style.OTransform = 'translateX(' + (-index*siWidth) + 'px)';
				} else {
