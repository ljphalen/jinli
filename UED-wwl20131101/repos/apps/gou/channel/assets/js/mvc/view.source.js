	...
	Gou.template =
	{
		...
		footer: //首页底部（或小辣椒）
			/\=xiaolajiao/i.test(location.href)?
				'<footer class="ft">\
					<div class="help">\
						<span>官方社区：bbs.xiaolajiao.com</span>\
						<span>微信公共账号：yusunshouji</span>\
					</div>\
					<div class="copyright">\
				...
				'<footer class="ft">\
					<div class="help">\
						...
						<a href="tel:075582583525">联系客服</a>\
	...
	Gou.view.setting = {
		'header': {//页头
			...
			nav: [
				...
				{name:'淘宝好店', link:iCat.util.fullUrl('/shop', bi)}
			],
		...
		'banner': {//轮播广告
			config: {
				tempId: 'banner',
				wrap: '#iScroll .panel',
				ajaxUrl: '/api/'+Gou.platform.prefix+'gou/ad?'+bi,
				delay: 3000,
				hooks: {
				...
				callback: function(p, cfg){
					...
					scroll = function(index){
						if(index==len){
							index = 0;
							//sPannel.css({'-webkit-transform':'translate3d(-'+item.width()*index+'px, 0, 0)', '-webkit-transition':'0ms'});
							sPannel.animate({left:0}, 0);
						} else {
							//sPannel.css({'-webkit-transform':'translate3d(-'+item.width()*index+'px, 0, 0)', '-webkit-transition':'500ms'});
		...
		'points': {//小banner
			config: {
				tempId: 'points',
				wrap: '#iScroll .panel',
				ajaxUrl: '/api/'+Gou.platform.prefix+'gou/link',
		...
		'mall': {//商场
			config: {
				tempId: 'icoItem',
				wrap: '#iScroll .panel',
				ajaxUrl: '/api/'+Gou.platform.prefix+'gou/mall'
			},
			title: '综合购物商城'
		},
		'theme': {//主题店
			config: {
				tempId: 'icoItem',
				wrap: '#iScroll .panel',
				ajaxUrl: '/api/'+Gou.platform.prefix+'gou/theme'
			},
			title: '主题店'
		},
		'tuan': {//团购&折扣
			config: {
				tempId: 'icoItem',
				wrap: '#iScroll .panel',
				ajaxUrl: '/api/'+Gou.platform.prefix+'gou/tuan'
			},
			title: '团购&折扣'
		},
		'helper': {//助手
			config: {
				tempId: 'icoItem',
				wrap: '#iScroll .panel',
				ajaxUrl: '/api/'+Gou.platform.prefix+'gou/helper'
			},
			title: '生活便民助手'
		},
		'footer': {//页尾
		...
		'codSearch': {
			config: {
				...
				multiChild: true,
				hooks: {'&': '.cod-list'},
				ajaxUrl: '/api/'+Gou.platform.prefix+'cod/search',
				callback: function(){
		...
		'list': {
			config: {
				...
				wrap: '#iScroll',
				multiChild: true,
				ajaxUrl: '/api/'+Gou.platform.prefix+'cod/index',
		...
		loadingData: function(parent, config, initDataInfo, flag){
			...
			switch (flag) {
				...
				case 2: ajaxUrl = iCat.util.storage('typeIndex') || '/api/fitting/goods'; break;
				case 3: ajaxUrl = config.ajaxUrl; break;
			}
		...
		'shopList': {
			config: {
				tempId: 'shopList',
				wrap: '#iScroll .panel',
				ajaxUrl: '/api/'+Gou.platform.prefix+'shop/index',
				hooks: {'&<0': ['data-hasnext~true', 'data-curpage~1', 'data-ajaxurl~/api/'+Gou.platform.prefix+'shop/index']},
				callback: function(parent, config, initDataInfo){
					Gou.view.setting.loadingData(parent, config, initDataInfo, 1);
				}
			}
