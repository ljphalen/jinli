	...
	GameApk.template =
	{
		...
		'<section >\
			...
			<div class="activity-list ">\
				<a data-type="-1" data-infpage="<%=data["data-infpage"]%>"><img src="<%=blankPic1%>" data-src="<%=data.img%>" alt="">\
	...
	GameApk.view.setting = {
		'slidePic':{//首页 图片轮播
				config:{
					tmplId:'slidePic',
					wrap:'.module',
					ajaxUrl:GameApk.ApiVersion=='v2'?'/api/Kingstone_indexi/turn':'/api/Kingstone_index/turn',
				}
		  	},
		'recItem':{
				config:{
					...
					ajaxUrl:GameApk.ApiVersion=='v2'?'/api/Kingstone_indexi/channel':'/api/Kingstone_index/channel',
					wrap: '.module'
				}
		},
		'activity':{
				config:{
					tmplId:'activity',
					imgSelector:'img[src$="blank1.gif"]',
					ajaxUrl:'/api/Kingstone_indexi/announce',
					wrap:'.module'
				}
		},
		'recommend':{
			config:{
				...
				wrap: '.module',
				ajaxUrl:GameApk.ApiVersion=='v2'?'/api/Kingstone_indexi/recommend':'/api/Kingstone_index/recommend',
				clear:false,
		...
		'category':{
			config:{
				wrap:'.module',
				tmplId:'category',
				ajaxUrl:'/api/Kingstone_category/index?sp='+GameApk.spParam,
				clear:false,
				callback:function(warp,cfg,data){
					// alert(GameApk.spParam);
					// alert(data.data.list[0]['game_title']);
					GameApk.view.setting.ajaxSucCallback(warp,cfg,data);
				},
				events:[{
					selector:window,type:'scroll',preventDefault:true,stopPropagation:true,
		...
		'subject':{
			config:{
				wrap:'.module',
				tmplId:'subject',
				ajaxUrl:GameApk.ApiVersion=='v2'?'/api/Kingstone_subject/index?sp='+GameApk.spParam:'/api/Kingstone_subject/index',
				clear:false,
				callback:function(warp,cfg,data){
					GameApk.view.setting.ajaxSucCallback(warp,cfg,data);
				},
