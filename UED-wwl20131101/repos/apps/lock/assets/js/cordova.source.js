...
module.exports = {
    objects: {
        ...
        navigator: {
            children: {
				gnLockScreen: {
					path: 'cordova/plugin/gnLockScreen'
				},
                notification: {
...
//add by mengqj
// file: lib/common/plugin/gnLockScreen.js
define("cordova/plugin/gnLockScreen", function(require, exports, module) {
var exec = require('cordova/exec');
...
module.exports = {
     ...
     * Show a toast 
     *
     * @param {String} message              Message to show the toast
     */
    gnLog: function(args) {
            console.log("toast message = " + args);
            exec(null, null, "GnLockScreenPlugin", "gnLog", [args]);
        },
    ...
    toast: function(message) {
            ...
            exec(null, null, "GnLockScreenPlugin", "gnToast", [message]);
        },
     ...
     *                                      data.progressRate (number) return the rate 
     * @param {Function} errorCallback		The function to call when there is an error that can not start download. (OPTIONAL)
     *                                      msg.errorInfo
     * @param {Options} options      		The options include download json and the lockscreen id. (OPTIONAL)
     */
    downloadStart: function(successCallback, errorCallback, options) {
        // successCallback required
    	if (typeof successCallback != "function") {
            console.log("downloadStart Error: successCallback is not a function");
            return;
    	}

    	// errorCallback optional
    	if (errorCallback && (typeof errorCallback != "function")) {
        	console.log("downloadStart Error: errorCallback is not a function");
        	return;
    	}
    	
        console.log("downloadStart options.json(1) = " + options.json);
        navigator.gnLockScreen.gnLog("downloadStart options.json(1) = " + options.json);

        var lockScreenJSON = null;
        if (options && typeof options.json == "string") {
        	lockScreenJSON = options.json;
        }

        var lockScreenSize = null;
        if (options && typeof options.lsSize == "number") {
            lockScreenSize = options.lsSize;
        } else if (options && typeof options.lsSize == "string") {
            lockScreenSize =  parseInt(options.lsSize,10);
        }

        var lockScreenLink = null;
        if (options && typeof options.lsLink == "string") {
            lockScreenLink = options.lsLink;
        }

        console.log("downloadStart lockScreenJSON = " + lockScreenJSON + "; lockScreenSize = " + lockScreenSize
                     + "; lockScreenLink = " + lockScreenLink);
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "downloadStart", [lockScreenJSON, lockScreenSize, lockScreenLink]);
    },

    /**
     * suspend download the lockscreen
    ...
    downloadSuspend: function(successCallback, errorCallback, options) {
        // successCallback required
        if (typeof successCallback != "function") {
            console.log("downloadSuspend Error: successCallback is not a function");
            return;
        }

        // errorCallback optional
        if (errorCallback && (typeof errorCallback != "function")) {
            console.log("downloadSuspend Error: errorCallback is not a function");
            return;
        }

        console.log("downloadSuspend option.id = " + options.id);
        var lockscreenId = null;
        if (options && typeof options.id == "string") {
            lockscreenId = options.id;
        } else if (options && typeof options.id == "number") {
            lockscreenId = options.id + "";
        }
        console.log("downloadSuspend lockscreenId = " + lockscreenId);
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "downloadSuspend", [lockscreenId]);
    },

    /**
     * Cancel download the lock screen
    ...
    downloadCancel: function(successCallback, errorCallback, options) {
        // successCallback required
        if (typeof successCallback != "function") {
            console.log("downloadCancel Error: successCallback is not a function");
            return;
        }

        // errorCallback optional
        if (errorCallback && (typeof errorCallback != "function")) {
            console.log("downloadCancel Error: errorCallback is not a function");
            return;
        }

        console.log("downloadCancel option.id = " + options.id);
        var lockscreenId = null;
        if (options && typeof options.id == "string") {
            lockscreenId = options.id;
        } else if (options && typeof options.id == "number") {
        ...
        console.log("downloadCancel lockscreenId = " + lockscreenId);
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "downloadCancel", [lockscreenId]);
    },

    /**
     * Continue download the lock screen
    ...
    downloadContinue: function(successCallback, errorCallback, options) {
        // successCallback required
        if (typeof successCallback != "function") {
            console.log("downloadContinue Error: successCallback is not a function");
            return;
        }

        // errorCallback optional
        if (errorCallback && (typeof errorCallback != "function")) {
            console.log("downloadContinue Error: errorCallback is not a function");
            return;
        }

        console.log("downloadContinue option.id = " + options.id);
        var lockscreenId = null;
        if (options && typeof options.id == "string") {
            lockscreenId = options.id;
        } else if (options && typeof options.id == "number") {
        ...
        console.log("downloadContinue lockscreenId = " + lockscreenId + "; lockScreenSize = " + lockScreenSize
                     + "; lockScreenLink = " + lockScreenLink);
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "downloadContinue", [lockscreenId, lockScreenSize, lockScreenLink]);
    },

    /**
     * Watch download the lock screen
     * @param  {Function} successCallback The function to call when the download Continue is success
     *                                    data.progressRate (number) return the rate 
     * @param  {Function} errorCallback   The function to call when there is an error that can not Continue download. (OPTIONAL)
     *                                    msg.errorInfo
    ...
    downloadWatch: function(successCallback, errorCallback, options) {
        // successCallback required
        if (typeof successCallback != "function") {
            console.log("downloadWatch Error: successCallback is not a function");
            return;
        }

        // errorCallback optional
        if (errorCallback && (typeof errorCallback != "function")) {
            console.log("downloadWatch Error: errorCallback is not a function");
            return;
        }

        console.log("downloadWatch option.id = " + options.id);
        var lockscreenId = null;
        if (options && typeof options.id == "string") {
            lockscreenId = options.id;
        } else if (options && typeof options.id == "number") {
        ...
        console.log("downloadWatch lockscreenId = " + lockscreenId + "; lockScreenSize = " + lockScreenSize);
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "downloadWatch", [lockscreenId, lockScreenSize]);
    },

    /**
     * set Lock Screen 
     * @param {Function} successCallback The function to call when the set LockScreen is success
     * @param {Function} errorCallback   The function to call when there is an error that set LockScreen . (OPTIONAL)
     *                                   if set LockScreen fail there will be return 
     *                                   msg.errorInfo (number) that told you the reason
     *                                   
     *                                   LOCKSCREEN_FAIL_DAMAGE = 1024; (file is damaged)
     *                                   LOCKSCREEN_FAIL_SDCARD_NOT_EXIST = 2048; (sd card not exit)
     *                                   LOCKSCREEN_FAIL_WRITE_FILE_FAIL = 3072; （write file error）
     *                                   LOCKSCREEN_FAIL_UNKNOWN = 4096; （unknown error）
     *                                   
     * @param {Options} options          The options include the lockscreen id. (OPTIONAL)
     */
    setLockScreen: function(successCallback, errorCallback, options) {
        // successCallback required
        if (typeof successCallback != "function") {
            console.log("setLockScreen Error: successCallback is not a function");
            return;
        }

        // errorCallback optional
        if (errorCallback && (typeof errorCallback != "function")) {
            console.log("setLockScreen Error: errorCallback is not a function");
            return;
        }

        console.log("setLockScreen option.id = " + options.id);
        var lockscreenId = null;
        if (options && typeof options.id == "string") {
            lockscreenId = options.id;
        } else if (options && typeof options.id == "number") {
        ...
        console.log("setLockScreen lockscreenId = " + lockscreenId);
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "setLockScreen", [lockscreenId]);
    },

    /**
     * [getLockScreenStatus description]
     * @param {Function} successCallback    The function to call when the set lockscreen success
     *                                      there will be return 
     *                                      data.status (number) as:
     *
     *                                      LOCKSCREEN_UNSTART = 0; （not start download）
     *                                      LOCKSCREEN_DOWNLOADING = 1; （downloading）
     *                                      LOCKSCREEN_SUSPEND = 2; （suspend maybe the net is break or initiative suspend）
     *                                      LOCKSCREEN_FAIL = 3; （when file is damaged will return the value then we'll del the file）
     *                                      LOCKSCREEN_DONE = 4; (done)
     *                                      
     * @param  {[type]} errorCallback       The function to call when there is an error that set LockScreen . (OPTIONAL)
     *                                      if set LockScreen fail there will be return 
     *                                      msg.errorInfo(number) that told you the reason
     *                                   
     *                                      LOCKSCREEN_FAIL_DAMAGE = 1024; (file is damaged)
    ...
    getLockScreenStatus: function(successCallback, errorCallback, options) {
        // successCallback required
        if (typeof successCallback != "function") {
            console.log("getLockScreenStatus Error: successCallback is not a function");
            return;
        }

        // errorCallback optional
        if (errorCallback && (typeof errorCallback != "function")) {
            console.log("getLockScreenStatus Error: errorCallback is not a function");
            return;
        }

        console.log("getLockScreenStatus option.id = " + options.id);
        var lockscreenId = null;
        if (options && typeof options.id == "string") {
            lockscreenId = options.id;
        } else if (options && typeof options.id == "number") {
        ...
        console.log("getLockScreenStatus lockscreenId = " + lockscreenId);
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "getLockScreenStatus", [lockscreenId]);
    },
    ...
    getDeviceInfo: function(successCallback, errorCallback) {
        ...
        // Get info
        exec(successCallback, errorCallback, "GnLockScreenPlugin", "getDeviceInfo", []);
    }
};
});
