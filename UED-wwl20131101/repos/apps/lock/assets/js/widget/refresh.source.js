...
(function($, undefined) {
    ...
    $.ui.define('refresh', {
        ...
        _createBtn: function (dir) {
            this._data['$' + dir + 'Elem'].html('<span class="ui-refresh-icon" style="display:none;"></span><span class="ui-refresh-label" style="display:none;"></span>');
            return this;
        },
        ...
        _changeStyle: function (dir, state) {
            ...
            switch (state) {
                case 'loaded':
                    refreshInfo['$label'].hide().html(refreshInfo['text']);
                    refreshInfo['$icon'].hide().removeClass('ui-loading');
                    data._actDir = '';
                    break;
                case 'loading':
                    refreshInfo['$label'].show().html('加载中...');
                    refreshInfo['$icon'].show().addClass('ui-loading');
                    data._actDir = dir;
                    break;
                case 'disable':
                    refreshInfo['$icon'].hide();
                    refreshInfo['$label'].hide();
