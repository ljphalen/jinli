(function(iCat){
	...
	iCat.mix(GNnews,{
		...
		slider: function(){
			...
			var sliderEl = $("#J_full_slider"), touchMode = false;
			var url = sliderEl.data('ajaxurl');
			var tpl_li = '', tpl_span = '';
			$.getJSON(url,function(res){
				var data = res.data, template = '';
				for (var i = 0; i < data.length; i++) {
					tpl_li = tpl_li + '<li><a href="'+data[i].link+'"><img src="'+data[i].img+'" ></a><a href="'+data[i].link+'"><span>'+data[i].title+'</span></a></li>';
					if (i == 0) {
						tpl_span = tpl_span + '<span class="on"></span> ';
					} else {
						tpl_span = tpl_span + '<span></span> ';
					}
				}

				template = '<div class="ui-slider-wrap"><div class="ui-slider-content"><ul class="ui-slider-pic ui-slider-text">\
						'+tpl_li+'\
						</ul></div><div class="ui-slider-handle ui-slider-handle-circle">'+tpl_span+'</div></div>';

				$('#J_full_slider').append(template);
