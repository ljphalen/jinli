...
class Db_Adapter_Pdo {
		...
		foreach ($array as $key => $val) {
			if (is_array($val)) {
				...
				if (in_array($val[0], array('>', '<', '>=', '<=', '!='))) {
					$str[] = self::fieldMeta($key) . $val[0] . self::quote($val[1]);
				} else if($val[0] == 'IN') {
					$str[] = self::fieldMeta($key) . $val[0] . self::quoteArray($val[1]);
				} else if ($val[0] == 'LIKE') {
					$str[] = sprintf("%s LIKE %s", self::fieldMeta($key), self::quote("%" . self::filterLike($val[1]) . "%"));
				}
			} else {
				$str[] = self::fieldMeta($key) . '=' . self::quote($val);
			}
	 ...
	 * @return string
	 */
	public function _filterLike($keyWord) {
		$search = array('[','%', '_', '/');
		$replace = array('[[]', '[%]', '[_]', '[/]');
