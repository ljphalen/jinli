 ...
 *
 */
class Gionee_Dao_Jhnews extends Common_Dao_Base {
	protected $_name = '3g_jhnews';
	...
	public function getList($start = 0, $limit = 20, $params = array()) {
		...
		$sql   = sprintf('SELECT * FROM %s WHERE 1 AND %s ORDER BY ontime DESC, sort DESC, id DESC LIMIT %d,%d', $this->getTableName(), $where, intval($start), intval($limit));
	...
	public function deleteByType($type) {
		$sql = sprintf('DELETE FROM %s WHERE type_id = %s AND is_ad = 0', $this->getTableName(), Db_Adapter_Pdo::quote($type));
