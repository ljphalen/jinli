...
class ProducesController extends Admin_BaseController {
	...
	public function addPostAction() {
		...
		$postData = $this->cookPostData ( $postData );
	...
	private function cookPostData($postData) {		
		$postData['start_time'] = strtotime($postData['start_time']);
		$postData['end_time']   = strtotime($postData['end_time']);
		if($postData['cat_id'] == 8){
		if (trim($postData['out_ad_id']) == '') {
			$this->output('-1', '百度的广告ID必须填写！');
		}
			unset($postData['link']);
			unset($postData['image']);
			unset($postData['is_special']);
		}else{
			unset($postData['out_ad_id']);
		}
