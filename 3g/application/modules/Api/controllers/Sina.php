 ...
 *
 */
class SinaController extends Api_BaseController {
        ...
        foreach ($position as $key => $value) {
            ...
            foreach ($list as $k => $v) {
                $data[$k] = array(
                    'title' => $v['title'],
                    'color' => $v['color'],
                    'link'  => Common::clickUrl($v['id'], 'SINA', $v['link'], $t_bi),
                );
            }
            $pos[$value] = $data;
        }
        ...
        foreach ($arr as $typeId => $name) {
            ...
            foreach ($news1 as $key => $value) {
                $tmpList[$key] = array(
                    'title' => $value['title'],
                    'color' => $value['color'],
                    'link'  => Common::clickUrl($value['id'], 'SINA', $value['url'], $t_bi),
                );
            }
            if ($data[$i]) {
                $tmpList[] = $data[$i];
            ...
            $link     = Common::clickUrl($typeId, 'SINA', $linkArr[$typeId], $t_bi);
