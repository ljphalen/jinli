...
class GoodsController extends User_BaseController {
    ...
    public function producesAction() {
        ...
        if (!intval($postData['cat_id'])) Common::redirect('/user/index/index');
        ...
        if (!empty($userInfo)) { //用户登陆后才能看到相关信息
            ...
            $ids           = User_Service_Earn::getDayEarnedGoodsIds($p);    //查询用户已領取的商品ID
            foreach ($list as $k => $v) {
            ...
            $num             = User_Service_ExperienceInfo::getLevelRewardsData($userInfo['experience_level'], 1);
