...
class Db_Adapter_Pdo {
    ...
    public static function _where($field, $op, $value) {
        ...
        switch ($op) {
            ...
            case "NOT IN":
                ...
                break;
            case "LIKE":
                $str .= sprintf("%s LIKE %s", self::fieldMeta($field), self::quote("%" . $value . "%"));
                break;
            case "&" :
                $str .= sprintf("%s & %s > 0", self::fieldMeta($field), self::quote($value));
                break;
            case "=" :
    ...
    public static function sqlGroup($group) {
        if (!is_array($group) || !count($group)) return '';
        $str = ' GROUP BY ';
        foreach ($group as $k => $v) {
            $group[$k] = self::fieldMeta($v);
        }
        return $str . implode(', ', $group);
    }
