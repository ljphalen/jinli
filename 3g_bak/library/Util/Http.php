...
class Util_Http {
    ...
    private static function _parse_url($url) {
        ...
        if (!$parseUrl || !is_array($parseUrl)) return false;
        isset ($parseUrl ['port']) || $parseUrl ['port'] = ($parseUrl ['scheme'] === 'https') ? 443 : 80;
    ...
    public static function preparePostBody(array $formvars, array $formfiles = array()) {
        if (count($formvars) == 0 && count($formfiles) == 0) return array(
            'data'    => '',
            'headers' => array()
        );
    ...
    public static function get($url, $headers = array(), $timeout = 10, $type = 'GET') {
        if (!$parseUrl = self::_parse_url($url)) return false;
    ...
    /**
     * @param string $type id,sys_ver,app_ver,model,ip
     *
     * @return array|string
     */
    static public function ua($type = '') {
        $ruleArr = array(
            'model'   => '/GiONEE-(.*)\//iU',
            'uuid'    => '/Id\/([\w]*?)/iU',
            'sys_ver' => '/RV\/([\S]*?)/iU',
        ...
        if (!isset(self::$userAgent['model'])) {
            $ua = Util_Http::getServer('HTTP_USER_AGENT');

            self::$userAgent['ua'] = $ua;
            foreach ($ruleArr as $k => $r) {
                preg_match($r, $ua, $val);
                if (!empty($val[1])) {
                    self::$userAgent[$k] = $val[1];
                    if ($k == 'uuid') {
                        $crypt                        = new Util_Encrypt();
                        self::$userAgent['uuid_real'] = $crypt->aesDecrypt($val[1]);
                    }
                }
            }
            self::$userAgent['ip'] = Util_Http::getClientIp();
        }


        if (!empty($type)) {
            $ret = isset(self::$userAgent[$type]) ? self::$userAgent[$type] : '';
        } else {
            $ret = self::$userAgent;
        }
        return $ret;
