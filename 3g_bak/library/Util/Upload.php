...
class Util_Upload {
    ...
    private function save($tmpName, $filename, $path) {
        ...
        if (function_exists("move_uploaded_file") && @move_uploaded_file($tmpName, '/' . $filename)) {
            @chmod($filename, 0777);
            //create webp
            /* if (($webp = $filename . ".webp") && !file_exists($webp)){
                image2webp($filename, $webp);
            } */
            return true;
    ...
    private function getImgSize($srcFile, $srcExt = null) {
        ...
        $extArr = array('jpg', 'jpeg', 'jpe', 'jfif');
        if (function_exists('read_exif_data') && in_array($srcExt, $extArr)) {
        ...
        if (!$srcdata['type'] || ($srcdata['type'] == 1 && in_array($srcExt, $extArr))) { //noizy fix
            return false;
        }
        return $srcdata;
    }
    ...
    private function escapeStr($string) {
        ...
        $string = preg_replace(array('/[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F]/', '/&(?!(#[0-9]+|[a-z]+);)/is'), array(
            '',
            '&amp;'
        ), $string);
