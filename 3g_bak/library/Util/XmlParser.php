...
class Util_XmlParser {
    ...
    public function getChilds($node) {
        ...
        foreach ($node->childNodes as $_node) {
            ...
            if (empty($tempChilds)) $tempChilds = ''; else
            ...
            if (!isset($childs[$nodeName])) $childs[$nodeName] = $tempChilds; else {
                $element           = $childs[$nodeName];
                $childs[$nodeName] = (is_array($element) && !is_numeric(implode('', array_keys($element)))) ? array_merge(array($element), array($tempChilds)) : array_merge((array)$element, array($tempChilds));
            }
        }
        return $childs;
    }
