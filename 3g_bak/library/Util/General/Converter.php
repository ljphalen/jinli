        ...
        for ($i = 0; $i < strlen($srcText); $i += 2) {
            ...
            if ($h > 127 && isset($this->tableIndex[$this->encodeLang][$h])) {
                ...
                if (!isset($this->tableEncode[$this->encodeLang][$h][$l])) {
                    ...
                    $this->tableEncode[$this->encodeLang][$h][$l] = $this->unicodeToUTF8(hexdec(bin2hex(fread($this->tableHandle, 2))));
                }
        ...
        if ($this->iconvEnabled && isset($SourceLang)) {
            for ($i = 0; $i < strlen($srcText); $i += 2) {
                if (ord($srcText[$i]) > 127) {
                    $tarText .= "&#x" . dechex($this->Utf8_Unicode(iconv($SourceLang, "UTF-8", $srcText[$i] . $srcText[$i + 1]))) . ";";
                } else {
                    $tarText .= $srcText[$i--];
                }
            }
