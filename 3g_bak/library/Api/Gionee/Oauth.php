...
class Api_Gionee_Oauth {
     ...
     * @param
     *                           $auth_token
     * @param $auth_token_secret return
     *                           array or false
     */
    ...
    public static function verify($params) {
        $oauth_config = Common::getConfig('apiConfig', 'oauth');
        $url          = $oauth_config ['gionee_user_url'] . '/account/verify.do';
    ...
    protected static function getQueryUrl($uri, $params) {
        $oauth_config = Common::getConfig('apiConfig', 'oauth');
        $appid        = $oauth_config ['gionee_user_appid'];
        $params       = array_merge($params, array('client_id' => $appid, 'state' => self::_getUuid()));
        $queryString  = http_build_query($params);
        $url          = $oauth_config ['gionee_user_url'] . $uri;
    ...
    protected static function auisResult($query, $data, $method = 'POST') {
        ...
        if ($result->state !== 200) {
            Common::log($result, 'oauth.log');
            return false;
        }
        $data = json_decode($result->data, true);
        if ($data ['r']) {
            Common::log($data, 'oauth.log');
            return false;
        }
        return $data;
    }
    ...
    public static function headers($query, $method) {
        $oauth_config = Common::getConfig('apiConfig', 'oauth');
        $appid        = $oauth_config ['gionee_user_appid'];
        $appkey       = $oauth_config ['gionee_user_appkey'];
        $time         = Common::getTime();
        $uuid         = self::_getUuid();
