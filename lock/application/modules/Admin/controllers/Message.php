...
class MessageController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $message) = Lock_Service_Message::getList($page, $perpage, $param);
	...
	public function detailAction() {
		$id = $this->getInput('id');
		$info = Lock_Service_Message::getMessage(intval($id));
		if(!$info || $info['uid'] != $this->userInfo['uid'] || !$id) $this->redirect($this->actions['listUrl']);
		Lock_Service_Message::updateMessage(array('status'=>1), $id);
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Lock_Service_Message::getMessage($id);
		if (($info && $info['id'] == 0) || $info['uid'] != $this->userInfo['uid']) $this->output(-1, '无法删除');
		$result = Lock_Service_Message::deleteMessage($id);
