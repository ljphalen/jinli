...
class ClicktypeController extends Admin_BaseController{
	...
	public function init() {
		parent::init();
		$this->parents = Lock_Service_ClickType::getParentsList();
	}
	...
	public function indexAction() {
		...
		$clcategorys =  Lock_Service_Clicktype::getListByParentIds($pids);
		$list = $this->_cookCategorys($this->parents, $clcategorys);
		
		$crs = Lock_Service_Click::searchClickList(array('sdate'=>$sDate, 'edate'=>$eDate));
	...
	public function add_postAction(){
		...
		$ret = Lock_Service_Clicktype::addClicktype($info);
	...
	public function editAction(){
		$id = $this->getInput('id');
		$info = Lock_Service_Clicktype::getClicktype(intval($id)); 
		$this->assign('parents', $this->parents);
	...
	public function edit_postAction(){
		...
		if (!$info['name']) $this->output(-1, '名称不能为空.');
		$ret = Lock_Service_Clicktype::updateClicktype($info, $info['id']);
		if (!$ret) $this->output(-1, '操作失败.');
		$this->output(0, '操作成功.');
	}
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Lock_Service_Clicktype::getClicktype($id);
		if ($info && $info['id'] == 0) $this->output(-1, '信息不存在无法删除');
		$ret = Lock_Service_Clicktype::deleteClicktype($id);
		if (!$ret) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
