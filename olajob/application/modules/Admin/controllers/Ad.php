...
class AdController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $ads) = Ola_Service_Ad::getList($page, $this->perpage, $search);
	...
	public function add_postAction() {
		...
		$result = Ola_Service_Ad::add($info);
	...
	public function editAction() {
		$id = $this->getInput('id');
		$info = Ola_Service_Ad::get(intval($id));
		$this->assign('ad_types', $this->ad_types);
	...
	public function edit_postAction() {
		...
		$info = $this->_cookData($info);
		$ret = Ola_Service_Ad::update($info, intval($info['id']));
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Ola_Service_Ad::get($id);
		if ($info && $info['id'] == 0) $this->output(-1, '无法删除');
//		Util_File::del(Common::getConfig('siteConfig', 'attachPath') . $info['img']);
		$result = Ola_Service_Ad::deleteAd($id);
		if (!$result) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
