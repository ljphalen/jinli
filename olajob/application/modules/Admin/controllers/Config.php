...
class ConfigController extends Admin_BaseController {
	...
	public function indexAction() {
		$configs = Ola_Service_Config::getAllConfig();
	...
	public function editPostAction() {
		$config = $this->getInput(array(
				'Ola_currency_rate_hk',
			));

		foreach(array_filter($config) as $key=>$value) {
			Ola_Service_Config::setValue($key, html_entity_decode($value));
		}
		Ola_Service_Config::setValue('Config_Version', Common::getTime());
		$this->output(0, '操作成功.');
	}
