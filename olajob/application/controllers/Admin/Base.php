...
class Admin_BaseController extends Common_BaseController {
	...
	public function init() {
		...
		$this->assign("attachPath", $this->adminroot . '/attachs');
		...
		if ($this->appCacheName && in_array($action, array('add_post', 'edit_post', 'delete'))) {
			if (is_array($this->appCacheName)) {
				foreach($this->appCacheName as $value) {
					Ola_Service_Config::setValue($value, Common::getTime());
				}
			} else {
				Ola_Service_Config::setValue($this->appCacheName, Common::getTime());
			}
		}
	}
	...
	public function checkRight() {
		...
		if(!$this->userInfo && !$this->inLoginPage()){
			...
			if ($mc != 'Admin_Index' && !in_array($mc, $userlevels) && !in_array($mca, $userlevels)) {
				exit('没有权限');
			}
		};
	}
	...
	public function getUserMenu() {
		...
		array_push($userlevels, "_Admin_Initiator", "_Admin_Index", '_Admin_Login', 'Admin_Ueditor');
	...
	public function cookieParams() {
		...
		Util_Cookie::set($name, $tmp, true, Common::getTime() + (24 * 3600));
	}
	...
	public function checkCookieParams() {
		...
		$action = $this->getRequest()->getActionName();
		$name = sprintf('%s_%s_%s', $module, $controller, $action);
        $params = Util_Cookie::get($name,true);
