...
class XS_FieldMeta
{
    ...
    public $type = 0;
    public $vno = 0;
    private $tokenizer = XS_Tokenizer::DFL;
    private $flag = 0;
    ...
    public function isBoolIndex()
    {
        ...
        return (!$this->hasIndex() || $this->tokenizer !== XS_Tokenizer::DFL);
    }
    ...
    {
        return ($this->tokenizer !== XS_Tokenizer::DFL);
    }
    public function getCustomTokenizer()
    {
        if (isset(self::$_tokenizers[$this->tokenizer])) {
            ...
            if (($pos1 = strpos($this->tokenizer, '(')) !== false
                ...
                $name = 'XS_Tokenizer' . ucfirst(trim(substr($this->tokenizer, 0, $pos1)));
                $arg = substr($this->tokenizer, $pos1 + 1, $pos2 - $pos1 - 1);
            } else {
                $name = 'XS_Tokenizer' . ucfirst($this->tokenizer);
                $arg = null;
            }
            if (!class_exists($name)) {
                if (!class_exists($name)) {
                    throw new XS_Exception('Undefined custom tokenizer `' . $this->tokenizer . '\' for field `' . $this->name . '\'');
                }
            }
            $obj = $arg === null ? new $name : new $name($arg);
            if (!$obj instanceof XS_Tokenizer) {
                throw new XS_Exception($name . ' for field `' . $this->name . '\' dose not implement the interface: XS_Tokenizer');
            }
    ...
    public function toConfig()
    {
        ...
        if ($this->type !== self::TYPE_ID && $this->tokenizer !== XS_Tokenizer::DFL) {
        ...
        if (isset($config['type'])) {
            ...
            if (defined($predef)) {
                ...
                if ($this->type == self::TYPE_ID) {
                    ...
                    $this->vno = XS_FieldScheme::MIXED_VNO;
