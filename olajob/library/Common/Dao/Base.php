...
class Common_Dao_Base {
	...
	public function get($value) {
		$sql = sprintf('SELECT * FROM %s WHERE %s = %s LIMIT 1', $this->getTableName(), $this->_primary, $value);
	 ...
	 * @return bool|int
	 */
	public function mutiInsertByKeys($data) {
		if (!is_array($data)) return false;
		$sql = sprintf('INSERT INTO %s ( %s ) VALUES %s',$this->getTableName(),implode(', ', array_keys($data[0])), Db_Adapter_Pdo::quoteMultiArray($data));
		return Db_Adapter_Pdo::execute($sql);
	}
	...
	public function getLastInsertId() {
		return Db_Adapter_Pdo::lastInsertId();
	}
