...
class AdController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $ads) = Dhm_Service_Ad::getList($page, $this->perpage, $search);
	...
	public function add_postAction() {
		...
		$result = Dhm_Service_Ad::addAd($info);
	...
	public function editAction() {
		$id = $this->getInput('id');
		$info = Dhm_Service_Ad::getAd(intval($id));
		$this->assign('ad_types', $this->ad_types);
	...
	public function edit_postAction() {
		...
		$info = $this->_cookData($info);
		$ret = Dhm_Service_Ad::updateAd($info, intval($info['id']));
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Dhm_Service_Ad::getAd($id);
		if ($info && $info['id'] == 0) $this->output(-1, '无法删除');
//		Util_File::del(Common::getConfig('siteConfig', 'attachPath') . $info['img']);
		$result = Dhm_Service_Ad::deleteAd($id);
		if (!$result) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
