...
class Util_Imagick
{
    ...
    /**
     * 创建缩略图
     * @param array $images 源文件
     * @param string $dir 缩略图存放位置
     * @param integer $dstW 缩略图宽度
     * @param integer $dstH 缩略图宽度
     * @return bool
     */
    public static function makeThumb($images, $dir, $dstW, $dstH){

        $dir = sprintf('%s/%s/', $dir, date('Ym'));
        $attachPath = Common::getConfig('siteConfig', 'attachPath');
        $savePath = sprintf('%s/%s', $attachPath, $dir); //缩略图保存路径
        ...
        foreach ($images as $image) {
            ...
            $i = pathinfo($image);
            $ext = $i['extension'] != 'jpg' ? 'jpg' : $i['extension'];
            $thumbName = sprintf("%s_%dx%d.%s", $i['filename'], $dstW, $dstH, $ext);
            $saveName = sprintf('/tmp/%s', $i['basename']);
            $thumb = $savePath . $thumbName; //缩略图
            $url = sprintf('/%s%s', $dir, $thumbName);

            file_put_contents($saveName, file_get_contents($image)); //保存到服务器
            list($ext, $dstFile) = Util_Imagick::createThumb($image, $thumb, $dstW, $dstH, $i['extension']); //创建缩略图
            if (!$dstFile) {
                file_put_contents($thumb, file_get_contents($saveName));
            }
            $thumbs[] = $url;
        }
        return $thumbs;
     ...
     * @return Imagick
     */
    public static function resize(Imagick $image, $width, $height,$crop=false){
     ...
     * 创建画布合并图片
     * @param Imagick $image
     * @param integer $dstW 宽度
     * @param integer $dstH 高度
     * @return Imagick
     */
    public static function createImage(Imagick $image,$dstW,$dstH) {


        $white = new Imagick();
        $white->newImage($dstW, $dstH, "white");
     ...
     * @param string $dstFile 缩略图
     * @param integer $dstW 缩略图宽度
     * @param integer $dstH 缩略图高度
     * @return mixed
     */
    public static function createThumb($srcFile, $dstFile, $dstW, $dstH){

        //a new imagick object
        $image = new Imagick($srcFile);

        $image = Util_Imagick::resize($image, $dstW, $dstH, $crop = 1);
        $dstW = $image->getImageWidth();
        $dstH = $image->getImageHeight();
        $type = $image->getimageformat();

        if ($type != "JPEG")$image = static::createImage($image, $dstW, $dstH);
        ...
        $image->stripImage();
        Util_Upload::mkRecur(dirname($dstFile));
    ...
    */
    public function resizex($width = 100, $height = 100, $fit = 'center', $fill_color = array(255,255,255,0) ){
