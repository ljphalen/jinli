...
class Util_Image {
	 ...
	 * @param int $p 取样份数
	 * @param float $c 调整饱和度百分比
	 */
	public static function getImagesColor($img, $p = 10, $c = 0) {
		$im = new Imagick ($img);

		$im->quantizeImage($p, Imagick::COLORSPACE_RGB, 0, false, false);
// 		$im->uniqueImageColors ();
		...
		$ret   = array('r' => $r - 25, 'g' => $g, 'b' => $b);
		$ret_2 = array($ret['r'] => 'r', $ret['g'] => 'g', $ret['b'] => 'b');


		$maxk = $ret_2[max($ret)];
		$mink = $ret_2[min($ret)];

		$ret[$maxk] += ceil($ret[$maxk] * $c);
		$ret[$mink] -= ceil($ret[$mink] * $c);
		return self::rgb2Hex($ret['r'], $ret['g'], $ret['b']);
	}

	public static function scale($imgSrc, $scaleImgFile, $toW, $toH) {

		$ret   = '';
		$image = new Util_ResizeImage();
		$image->load($imgSrc);
		...
		$ratio = '';
		if ($srcH > $toH && $srcW > $toW) { //缩小W,H
			if (($srcH - $toH) > ($srcW - $toW)) { //按W缩小
				$ratio = $ratioW;
			} else { //按H缩小
				$ratio = $ratioH;
			}
		} else if ($srcH < $toH && $srcW < $toW) { //放大W,H
			if (($srcH - $toH) > ($srcW - $toW)) { //按W放大
				$ratio = $ratioW;
			} else { //按H放大
				$ratio = $ratioH;
			}
		} else if ($srcH > $toH && $srcW < $toW) { //放大W
			$ratio = $ratioW;
		} else if ($srcH < $toH && $srcW > $toW) { //放大H
			$ratio = $ratioH;
		}

		if ($ratio) {
			$image->scale($ratio);
			$image->save($scaleImgFile);
			$ret = $scaleImgFile;
		}
		return $ret;
	}
