...
class Widget_Service_User {
	...
	static public function initLog($params) {
		...
		$uKey = 'WIDGET_USER:' . $imei;
		$info = Common::getCache()->get($uKey);
		if (empty($info['id'])) {
			$info = Widget_Service_User::getBy(array('imei' => $imei));
		}
		...
		if (empty($info['id'])) {
			...
			if (!empty($params['column_ids'])) {
				$info['column_ids'] = $params['column_ids'];
			}

			if (!empty($params['url_ids'])) {
				$info['url_ids'] = $params['url_ids'];
			}

			$id         = Widget_Service_User::add($info);
			$info['id'] = $id;
			...
			if (!empty($params['column_ids']) && $info['column_ids'] != $params['column_ids']) {
				$info['column_ids'] = $params['column_ids'];
				$change++;
			}

			if (!empty($params['url_ids']) && $info['url_ids'] != $params['url_ids']) {
				$info['url_ids'] = $params['url_ids'];
				$change++;
			}
			...
			if ($change > 0) {
				...
				Widget_Service_User::set($info, $info['id']);
			}

			Common::getCache()->set($uKey, $info, Common::KT_USER);

			$hKey = 'SYNC_VISIT_WIDGET';
	...
	private static function _cookData($data) {
		$fields = array('id', 'imei', 'model', 'net', 'gps', 'app_ver',
			'created_at', 'last_visit_at', 'ip', 'column_ids', 'url_ids');
	...
	private static function _getDao() {
		return Common::getDao("Widget_Dao_User");
	}
}