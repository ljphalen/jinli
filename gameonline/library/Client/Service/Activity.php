...
class Client_Service_Activity{
	...
	public static function getOnlineActivityInfo() {
		...
		/*
		$params['online_start_time'] = array('<=', Common::getTime());
		$params['online_end_time'] = array('>=', Common::getTime());
	...
	public static function getLastActivity() {
		return self::_getDao()->getLastActivity();
	}
	 ...
	 * @param unknown_type $id
	 */
	public static function getLog($activity_id) {
		if (!intval($activity_id)) return false;
		return self::_getLogDao()->getsBy(array('activity_id'=>intval($id)),array('create_time'=>'DESC'));
	}
