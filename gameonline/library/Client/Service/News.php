...
class Client_Service_News{
	...
	public static function updateApiNewsStatus($data,$statu) {
		...
		return self::_getDao()->updateApiNewsStatus($data, intval($statu));
	}
	...
	public static function updateGameClientNewsByOutId($ntype,$id,$out_id) {
		...
		return 	self::_getIdxGameClientNewsDao()->updateGameClientNewsByOutId($ntype,$id,$out_id);
	}
	 ...
	 * @return multitype:unknown multitype:
	 */
	public function getSortNews($page = 1, $limit = 10, $params = array()) {
		$params = self::_cookData($params);
		if ($page < 1) $page = 1;
		$start = ($page - 1) * $limit;
		...
		$ret = self::_getDao()->getSortNews($start, $limit, $params, array('sort'=>'DESC'));
		$total = self::_getDao()->countSortNews($params);
		return array($total, $ret);
	}
