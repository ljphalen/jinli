...
class Client_Service_Game extends Common_Service_Base{
	 ...
	 */
	public function getCanUseGames($page, $limit, $params = array()) {
		$data = self::_cookData($params);
		$data['id'] = $params['id'];
		if(intval($page) < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::_getDao()->getCanUseGames(intval($start), intval($limit), $data);
		$total = self::_getDao()->getCanUseGamesCount($data);
		return array($total, $ret);
	}
	...
	public function getCanUseAllGames() {
		$ret = self::_getDao()->getCanUseAllGames();
		$total = self::_getDao()->getCanUseAllGamesCount();
		return array($total, $ret);
	}
	 ...
	 * @return multitype:unknown
	 */
	public function getNewTopGames($page, $limit, $params = array()) {
		$data = self::_cookData($params);
		$data['top'] = $params['top'];
		if(intval($page) < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::_getDao()->getNewTopGames(intval($start), intval($limit), $data);
		unset($data['top']);
		$total = self::_getDao()->getNewTopGamesCount($data,$params['top']);
		return array($total, $ret);
	}
	...
	public static function getGameByStatu($status) {
		return self::_getDao()->getGameByStatu(intval($status));
	}
	...
	public static function getGameByResourceId($id) {
		if (!intval($id)) return false;
		return self::_getDao()->getGameByResourceId(intval($id));
	}
	 ...
	 * @return Ambigous <boolean, number>
	 */
	public static function batchAddBySubject($data, $sid) {
		$tmp = array();
		$v = array();
		foreach($data as $key=>$value) {
			$v = explode('|', $value);
		...
		return self::_getSubjectDao()->mutiInsert($tmp);
	}
	 ...
	 * @return boolean
	 */
	public static function deleteGameclientBySubjectGames($data) {
		foreach($data as $key=>$value) {
			$v = explode('|', $value);
			self::_getSubjectDao()->deleteBy(array('id'=>$v[0]));
		}
		return true;
	}
	...
	public static function getCategoryGames(array $params = array()) {
		$data = self::_cookInxParams($params);
	...
	public static function getSubjectGames($page = 1, $limit = 10, $params = array()) {
		...
		if ($page < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::_getSubjectDao()->getList($start, $limit, $params, array('sort'=>'DESC','resource_game_id'=>'DESC'));
	...
	public static function getSubjectBySubjectId(array $params = array()) {
		$data = self::_cookInxParams($params);
		$result = self::_getSubjectDao()->getsBy($data, array('sort'=>'DESC','resource_game_id'=>'DESC'));
		$count = self::_getSubjectDao()->count($data);
		return array($count, $result);
	}
	 ...
	 * @return Ambigous <boolean, number>
	 */
	public static function deleteGameclientBySubject($id) {
		return 	self::_getSubjectDao()->deleteBy(array('subject_id'=>$id));
	}
	...
	public static function updateSubjectsBySubjectIds($subject_id,$status) {
		...
		return 	self::_getSubjectDao()->updateSubjectsBySubjectIds($subject_id,$status);
	}
	 ...
	 * @return boolean
	 */
	public static function sortGameclientBySubjectGames($data, $sorts) {
		foreach($data as $key=>$value) {
			$v = explode('|', $value);
			self::_getSubjectDao()->update(array('sort'=>$sorts[$v[0]]), $v[0]);
		}
		return true;
	}
	 ...
	 * @return Ambigous <boolean, number>
	 */
	public static function getGameclientBySubject($id) {
		return 	self::_getSubjectDao()->getsBy(array('subject_id'=>$id,'status'=>1));
	}
	 ...
	 * @return Ambigous <boolean, number>
	 */
	public static function getCategorysByGameIds($params,$statu) {
		return 	self::_getCategoryDao()->getCanUseCategorysByGameIds(array('resource_game_id'=>$ids));
	}
	...
	public static function getGuessByGame(array $params = array()) {
		...
		$result = self::_getGuessDao()->getsBy($data, array('sort'=>'DESC','game_id'=>'DESC'));
		$count = self::_getGuessDao()->count($data);
		return array($count, $result);
	}
	...
	public static function batchAddByGuess($data) {
		...
		return self::_getGuessDao()->mutiInsert($tmp);
	}
	...
	public static function updateSubjectsByGameIds($game_id,$status) {
		if (!$game_id) return false;
		return self::_getSubjectDao()->updateSubjectsByGameIds($game_id,$status);
	}
