...
class Client_Service_Category extends Common_Service_Base{
	 ...
	 */
	public static function getCanUseCategorysByIds($page, $limit, $params = array()) {
		$data = self::_cookData($params);
		$data['id'] = $params['id'];
		if(intval($page) < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::_getDao()->getCanUseCategorysByIds(intval($start), intval($limit), $data);
		$total = self::_getDao()->getCanUseCategoryByIdsCount($params);
		return array($total, $ret);
	}
