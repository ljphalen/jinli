...
class Client_Service_Comment extends Common_Service_Base{
	...
	public static function updateCommentStatus($data,$statu,$comment_name) {
		...
		foreach($data as $key=>$value){
			...
			//查找审核记录
			$info_log = Client_Service_CommentLog::getBy(array('comment_id'=>$value));
			if($info_log){
				//有，就直接更新
				$ret = Client_Service_CommentLog::updateLog(array('status'=>$statu,'check_time'=>Common::getTime()), array('comment_id'=>$value));
			} else {
			...
			if($info['utype'] == 1) {
				$params['uname'] = $info['uname'];
				$search['uname'] = $info['uname'];
			} else {
				$params['imcrc'] = $info['imcrc'];
				$search['imcrc'] = $info['imcrc'];
			}
			$params['game_id'] = $info['game_id'];
		    if($statu == 3) {
				 //如果该账号有status为3的评论,先删除评论表中该条记录
		    	 $params['status'] = 3;
		    	 $params['id'] = array('!=',$value);
				 $ret = self::deleteByComment($params);
			} else if($statu == 2){
				 //不通过或者审核中的评论,即status为1或者status=2的评论删除
				 $search['status'] = array('<',3);
				 $search['id'] = array('!=',$value);
				 $ret = self::deleteByComment($search);
		    }
			...
			$tmp = array(
					'id'=>'',
					'comment_id' => $value,
					'comment_log_id' => ($info_log ? $info_log['id'] : $log_id),
					'check_name'=> $comment_name,
					'check_time'=> Common::getTime(),
					'status'=> $statu,
					'operate'=> 0
	...
	public static function addComment($data, $score, $game_id, $uname, $imei, $imeicrc, $isLogin, $scorelog) {
		if (!is_array($scorelog)) return false;
	      ...
	      try {
	      	...
	      	if($data){
	      		$params['status'] = array("<",3);
	      		//查找不通过或者审核中的评论,即status为1或者status=2的评论
	      		$cmm_log = self::getsByComment($params);
	      		
	      		//如果有status为1或者status=2的评论,先删除
	      		if($cmm_log) {
	      			$ret = self::deleteByComment($params);
	      			if (!$ret) throw new Exception('Delete Comment fail.', -201);
	      		}
	      		 
	      		//添加一条评论
	      		$ret = self::add($data);
	      		if (!$ret) throw new Exception('Add Comment fail.', -202);
	      	}
