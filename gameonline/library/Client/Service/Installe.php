...
class Client_Service_Installe{
	...
	public static function updateInstalleDate($id) {
		return self::_getDao()->updateInstalleDate(intval($id));
	}
	...
	public static function batchAddByInstalle($data,$id) {
		...
		self::_getDao()->updateInstalleDate(intval($id));
	 ...
	 * @return boolean
	 */
	public static function getInstalleByGtype($installe_id) {
		if (!$installe_id) return false;
		return self::_getDao()->getInstalleByGtype(intval($installe_id));
	}
	...
	public static function updateInstalleStatus($ids,$status) {
		return self::_getDao()->updateInstalleStatus($ids, $status);
	}
	...
	public static function getIdxList($page = 1, $limit = 10, $params = array(), $orderBy = array('id'=>'DESC')) {
		$params = self::_cookIdxData($params);
		if ($page < 1) $page = 1; 
		$start = ($page - 1) * $limit;
