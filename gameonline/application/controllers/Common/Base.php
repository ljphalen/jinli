...
abstract class Common_BaseController extends Yaf_Controller_Abstract {
    ...
    public function init() {
        ...
        Util_Cookie::set("apk_version", $tj[1], true, Common::getTime() + 30);
    ...
    public function getFilterGame($client = ''){
    	...
    	//艾米游戏过滤的游戏ID
    	if($client == 'amigame') {
    		if (is_array($cfliter = $cache->get("-gf-". $client))) return $cfliter;
    	...
    	//非游戏大厅跟艾米游戏要过滤的游戏ID
    	if (is_array($cfliter = $cache->get("-gf-default". $client))) return $cfliter;
    	$default = Game_Service_Config::getValue('game_filter_default');
    	$filter = $default ? explode('|', $default) : '';
    	if($filter){
