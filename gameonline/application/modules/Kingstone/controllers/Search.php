...
class SearchController extends Kingstone_BaseController {
	
	public $actions =array(
			'indexUrl' => '/kingstone/search/index',
			'detailtUrl' => '/kingstone/search/detail',
			'errorUrl' => '/kingstone/search/error',
			'engineUrl' => '/kingstone/search/engine',
			'enginetUrl' => '/kingstone/search/enginet',
			'tjUrl' => '/kingstone/search/index/tj',
	);
	public $perpage = 10;
	...
	public function indexAction() {
		...
		if($local_games && $action){
			...
			if($baidu_games && $action){
				...
				$games = array();
				$this->redirect($webroot. '/kingstone/search/error?keyword='.$keyword);
				exit;
			}
		}
	...
	public function moreAction() {
	    ...
	    if($local_games){
	    	...
	    	if($baidu_games){
				...
				$games = '';
				$this->redirect($webroot.'/kingstone/error/index');
				exit;
			}
	    }
   ...
   /**
	...
	private function _detailv1($gameid){
		...
		foreach ($games as $key => $value) {
			...
			$href = $webroot . '/kingstone/search/detail/?id=' . $value['id'] . '&pc=1&intersrc=gltj' . $gameid.'_GID'.$value['id'] . '&t_bi=' .$t_bi ;
