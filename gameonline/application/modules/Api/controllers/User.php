...
class UserController extends Api_BaseController {
	...
	public function regpassAction(){
		...
		if($response['u'] && $response['tn']){
			...
			$result = array('uuid' => $response['u'], 'uname'=> $response['tn'], 'nickname' => $nickName, 'optime' => $reg['time']);
			//网页版tgc处理登陆
	...
	private function _loginProcess($data){
		...
		//加入最后登录时间 用户名 昵称
		return array('uuid'=> $data['u'], 'uname' => $data['tn'], 'nickname'=> $userInfo['nickname'], 'optime' => $time);
	}
