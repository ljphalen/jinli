...
class NewsController extends Admin_BaseController{
	...
	public function indexAction() {
		...
		if ($type) {
			...
			list($total, $news) = Browser_Service_News::getList($page, $perpage, $param);
	...
	public function add_postAction(){
		...
		$ret = Browser_Service_News::addNews($info);
	...
	public function edit_postAction(){
		...
		if (!$info['ontime']) $this->output(-1, '发布时间不能为空.');
		if (!$info['start_time']) $this->output(-1, '开始时间不能为空.');
		$ret = Browser_Service_News::updateNews($info, $info['id']);
		if (!$ret) $this->output(-1, '操作失败.');
		$this->output(0, '操作成功.');
	}
	...
	public function edit_status_postAction() {
		...
		//设置原选中的状态为0
		Browser_Service_News::updateStatusByIds($ids['oids'], 0);
				
		$ret = Browser_Service_News::updateStatusByIds($ids['ids'], 1);
		if (!$ret) $this->output(-1, '操作失败.');
		$this->output(0, '操作成功.');
	}
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Browser_Service_News::getNews($id);
		if ($info && $info['id'] == 0) $this->output(-1, '信息不存在无法删除');
		$ret = Browser_Service_News::deleteNews($id);
		if (!$ret) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
