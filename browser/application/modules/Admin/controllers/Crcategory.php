...
class CrcategoryController extends Admin_BaseController{
	...
	public function init() {
		parent::init();
		$this->parents = Browser_Service_Crcategory::getParentsList();
	}
	...
	public function indexAction() {
		...
		$clcategorys =  Browser_Service_Crcategory::getListByParentIds($pids);
		$list = $this->_cookCategorys($this->parents, $clcategorys);
		
		$crs = Browser_Service_Cr::searchCrList(array('sdate'=>$sDate, 'edate'=>$eDate));
		...
		//总pv量
		$total_pv = Browser_Service_Stat::searchPvList(array('sdate'=>$sDate, 'edate'=>$eDate, 'tj_type'=>'0'));
		//总点击量
		$total_crs = Browser_Service_Cr::getTotalByTime(array('sdate'=>$sDate, 'edate'=>$eDate));
	...
	public function listAction() {
		...
		/*
		//总pv量
		$total_pv = Browser_Service_Stat::searchPvList(array('sdate'=>$sDate, 'edate'=>$eDate));
		//总点击量
		$total_crs = Browser_Service_Cr::getTotalByTime(array('sdate'=>$sDate, 'edate'=>$eDate));
	...
	public function add_postAction(){
		...
		$ret = Browser_Service_Crcategory::addCrcategory($info);
	...
	public function editAction(){
		$id = $this->getInput('id');
		$info = Browser_Service_Crcategory::getCrcategory(intval($id)); 
		$this->assign('parents', $this->parents);
	...
	public function edit_postAction(){
		...
		if (!$info['name']) $this->output(-1, '名称不能为空.');
		if (!$info['url']) $this->output(-1, 'URL不能为空.');
		$ret = Browser_Service_Crcategory::updateCrcategory($info, $info['id']);
		if (!$ret) $this->output(-1, '操作失败.');
		$this->output(0, '操作成功.');
	}
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Browser_Service_Crcategory::getCrcategory($id);
		if ($info && $info['id'] == 0) $this->output(-1, '信息不存在无法删除');
		$ret = Browser_Service_Crcategory::deleteCrcategory($id);
		if (!$ret) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
