...
class ArticleAction extends SystemAction
{
	...
	public function addSave()
	{
		...
		if(!empty($_FILES["file_path"]["tmp_name"])) {
			...
			if (is_array ( $uploadList )) {
				...
				$_POST['file_name'] = $uploadList[0]['name'];
	...
	public function edit()
	{
		...
		if ($info['mold'] == ArticleModel::MOLD_ARTICLE) {
			$this->display('add');
		} elseif ($info['mold'] == ArticleModel::MOLD_DOWNLOAND) {
			$this->display('sdkadd');
		} elseif ($info['mold'] == ArticleModel::MOLD_HIDE_ARTICLE) {
		    $this->display('hideadd');
		} elseif ($info['mold'] == ArticleModel::MOLD_OTHER_DOWNLOAND) {
		    $this->display('fileadd');
		}
	}
