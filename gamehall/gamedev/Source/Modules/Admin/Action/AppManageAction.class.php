...
class AppManageAction extends SystemAction
{
	...
	protected $_export_config = array(
		...
		'category_p' => array('分类名称','ApksModel::getCategoryByApk','function', '{{field_val}}', 'id'),
	...
	function _filter(&$map){
		...
		if(!empty($_search["category_p"])) {
			$ids = D("Dev://Apks")->where(array("category_p"=>$_search["category_p"]))->getField("app_id", true);
			if(!empty($ids))
				$map["id"] = array("in", array_merge((array)$map["id"], array_unique($ids)));
		}
