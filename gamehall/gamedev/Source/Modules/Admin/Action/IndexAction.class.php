...
class IndexAction extends SystemAction
{
	...
	public function stat()
	{
		...
		for ($i=0; $i < 31; $i++)
		{
			...
			$game_list = D('Dev://Apks')->where($add_map)->select();
			$game_add[$i] = count($game_list);
			$game_update[$i] = 0;
			if (is_array($game_list))
			{
				foreach ($game_list as $key => $val)
				{
					 $is_update = D('Dev://Apks')->where(array("app_id"=>$val['app_id'], "status"=>array("gt", 1), "id"=>array("lt", $val['id'])))->count();
					 if ($is_update)
					 {
					 	$game_update[$i]++;
					 }
			...
			/*$game_add[$i] = D('Dev://Apps')->where($add_map)->count();
			//var_dump(Date('Y-m-d H:i:s',$day_time),$d_apk->getlastsql());
			
			//游戏版本更新数
			$up_map = array('created_at' => $time_cond);
			$game_update[$i] = $d_apk->where($up_map)->count();
			$game_update[$i] = $game_update[$i] > $game_add[$i] ? $game_update[$i] - $game_add[$i] : $game_update[$i];
			*/
			//游戏审核通过
			$up_map = array('checked_at' => $time_cond);
			...
			//审核数
			//2014-06-12 10:26:56, 由 徐玉兰 激活。
			//在报表中，已审核的数量＝审核通过+已上线+已下线
			$check_count[$i] = $d_accounts->join(' account_infos ON accounts.id = account_infos.account_id')->where('account_infos.status > 1 and left(`audit_at`,10)="'.$now_date.'"')->count ( '*' );
