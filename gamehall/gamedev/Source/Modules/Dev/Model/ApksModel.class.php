...
/**
 * 应用model类
 * 状态(-4:封号, -3:认领下线, -2:已下线, -1:审核不通过, 0:未提交审核, 1:审核中/待审核, 2:审核通过, 3:已上线, 4:自动上线)
 *
...
class ApksModel extends Model{
	...
	const APK_CLAIM_OFFLINE = -3; // 认领下线
	const APK_FOBID_OFFLINE = -4; // 封号下线
	...
	protected $status = array (
		...
		self::APK_CLAIM_OFFLINE => '认领下线',
		self::APK_FOBID_OFFLINE => '封号下线',
	);
	...
	CONST SAFE_STATUS_QIHU = 4;		//腾讯
	CONST SAFE_STATUS_ALL = 7;		//检测是否都通过，每增加值时，其值对应增加
	...
	public static function siteStaus($site)
	{
		$arr = array(
			...
			SafeModel::SITE_QIHU => self::SAFE_STATUS_QIHU,
		);
		return $arr[$site];
	} 
	...
	{
		return D("Category")->where(array("id"=>$category_id))->getField('name');
	}
	
	public static function getCategoryByApk($apk_id)
	{
		$category_id = D("Apks")->where(array("id"=>$apk_id))->getField('category_p');
		
		return ApksModel::getCategory($category_id);
	}
