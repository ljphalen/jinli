...
class ApkClient extends ApksModel {
	...
	public function apk($uid, $userName, $filename, $appId, $otype = 0, $apkId = 0) {
		...
		if ($app_count) {				//已经存在此包
			...
			if ($ownner_apps_count==0 && $ownner_apks_count==0) {
				...
				//新上传apk文件地址
				$newUploadApkPath = $this->apkPath . $filename;
				APP_DEBUG && Log::write($newUploadApkPath, "new_upload_apk_path");
				
				//新上传apk文件签名
				$newUploadApkSign = $this->getApkSign($newUploadApkPath);
				APP_DEBUG && Log::write($newUploadApkSign, "new_upload_apk_sign");
				if ($newUploadApkSign == '') {
				    return $this->setErr ( 14 );
				}
				
				//原有的apk文件地址
				$existsApkPath = Helper("Apk")->get_path("apk") . $apkInfo['file_path'];
				APP_DEBUG && Log::write($existsApkPath, "exists_apk_path");
				
				//原有的apk文件签名
				$existsApkSign = $this->getApkSign($existsApkPath);
				APP_DEBUG && Log::write($existsApkSign, "exists_apk_sign");
				if ($existsApkSign == '') {
				    return $this->setErr ( 14 );
				}
				
				
				// 判断 新上传apk文件签名 和 原有的apk文件签名 是否一致
				if ($newUploadApkSign != $existsApkSign) {
				    return $this->setErr ( 15 );
				}
	...
	public function readApkConfig($filename, $apkPath = NULl) {
		...
		$command = $this->apkConfig ['APKTOOL'] . 'aapt d badging "'. str_replace('"', '\"', $this->apkPath . $filename) .'"';
	...
	/**
	 * 获取apk包MD5签名信息
	 * @param string $apk_path apk文件地址
	 * @return string
	 */
	public function getApkSign($apk_path) {
	    if(!is_file($apk_path)) {
	        return false;
	    }
	    ...
	    $JAVA_BIN_PATH = C('apk.JAVA_BIN_PATH');
	    if ($JAVA_BIN_PATH != '') {
	       $shell = 'export PATH=$PATH:' . $JAVA_BIN_PATH . ';' . $shell;
	    }
