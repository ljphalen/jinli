...
class AppsAction extends BaseAction {
	...
	function index() {
		...
		if (empty ( $userAppIds ['ids'] )) { // 无应用
		    ...
		    $where = array (
					'author_id' => $this->uid,
					"app_id" => array ( "in", $userAppIds ['ids'] ) 
			);

		    $newIds = $Apks->where ( $where )->group ( "app_id" )->getField ( "app_id, max(id) as maxid", true );
			$apkMap = array ( 'author_id' => $this->uid );
			if (! empty ( $newIds ))
				$apkMap ['id'] = array ( 'in', array_values($newIds) );
			
			// 获取各种状态对应的app总数
			$apknum = array();
			$apknum['status'] = $Apks->where ( $apkMap )->group ( "status" )->getField("status, count(id) as count", true);
			$apknum['all'] = array_sum($apknum['status']);
			
			//已上线应用包含已上线和自动上线
			$apknum['status'][3] = $apknum['status'][3] + $apknum['status'][4];
		}
		...
		if ($status !== $this->appAll)
			$map ["status"] = $status;
		//已经上线包括上线和自动上线
		if ($status == self::APK_ONLINE)
			$map ["status"] = array("egt", self::APK_ONLINE);
	...
	function apkUploadH5() {
	    ...
	    if (!file_exists($uploadFile)) {
	        Log::write ( $uploadFileRedisKey, Log::DEBUG );
	        Log::write ( $uploadFile, Log::DEBUG );
	        echo json_encode(array('status'=>0, 'data' => '上传文件发生异常'));
	        exit;
	    }
	    ...
	    if(empty($this->uid)){
	        echo json_encode(array('status'=>0, 'data' => '您已超时退出，请登陆后重新上传'));
	        exit;
	    }
	    ...
	    // 设置apk文件路径为绝对路径
	    $apkClient->setPath ( dirname($uploadFile) . '/' );
	    // 处理apk信息
	    $apkInfo = $apkClient->apk ( $this->uid, $this->user ['nickname'], basename($uploadFile), $appId, $type, $apkId );
	    
	    // 读取包信息失败
	    if (is_int ( $apkInfo )) {
	        $errData = $apkClient->getErr ( $apkInfo );
	        echo json_encode(array('status'=>0, 'data' => $errData['error']));
	        Log::write ( 'UPLOAD_READ_FAIL:'.$errData['error'], Log::EMERG );
	        exit;
	    }
	    ...
	    //成功返回
	    echo json_encode(array('status'=>1, 'data' => $apkInfo));
	    exit;
		...
		if (! empty ( $preApkinfo )) {
			...
			if (! empty ( $preApkinfo ['category_two'] ) && empty ( $apkinfo ['category_two'] ) 
				...
				$arr = array("brief","keyword","description","category_one","category_two",
								...
								"category_p","category_p_son","category_s","category_s_son");
				foreach ($arr as $v){
					if(empty($apkinfo[$v]))
	...
	*/
	...
	function appsave() {
		...
		$_POST ['category_p'] = intval($_POST ['category_p']);
		$_POST ['category_p_son'] = intval($_POST ['category_p_son']);
		$_POST ['category_s'] = intval($_POST ['category_s']);
		$_POST ['category_s_son'] = intval($_POST ['category_s_son']);
		...
		if (isset ( $post ['saveSub'] ) && $post ['saveSub'] == "1") {
			if ($post['category_one'] == "100"){
				if(empty($_POST ['category_p']) || empty($_POST ['category_p_son']))
					$this->error ( "分类不能为空！" );
			}else{
			...
			//empty ( $post ['changelog'] ) && $this->error ( "应用更新日志不能为空！" );
			$_POST ['status'] = 1;
			
			//期望上线时间不能大于高版本的期望上线时间
			if($post["onlinetime_type"] > 1 && strtotime($post["online_time"]) < time() + 600)
            ...
            $apkName = $Apks->where ( array ( "id" => $apk_id ) )->getField ( "app_name" );
            $success_msg = "您的应用《".$apkName."》已经提交审核！我们将会在1-3个工作日内进行审核！";
		} else {
			$_POST ['status'] = 0;
		}
		unset ( $_POST ['saveSub'] );
		...
		if (! $Apks->create ()) {
			...
			if ($res !== false) {
                ...
                else{ // 联运应用，插入contract表
                    M('contract')->where(array('package'=>$package))->setField('app_id',$app_id);
                }
			//	$this->success ( $success_msg, U ( "apps/manage", array ( "id" => $apk_id ) ) );
			//  以下为新增 礼包入口
                $add_gift = U('gift/add',array('id'=>$app_id));
                $cancel = U ( "apps/manage", array ( "id" => $apk_id ) );
                $this->assign('add_gift',$add_gift);
                $this->assign('cancel',$cancel);
                $this->assign('message',$success_msg);
                $this->display('gift_success');
			} else {
				Log::write ( $Apks->_sql (), Log::EMERG );
				Log::write ( $Apks->getDbError (), Log::EMERG );
				$this->error ( '保存失败！' );
	...
	function category_fix() {
		
		set_time_limit(60);
		
		//只获取有提交状态的应用
		$category_fix_apps = D("Apks")->where(array("author_id"=>$this->uid, "status"=>array("neq", 0)))->getField("distinct(app_id) as id, app_id", true);
		$apps = D("Apps")->where(array("author_id"=>$this->uid, "id"=>array("in", $category_fix_apps)))->select();
		foreach ($apps as $k=>$v)
		{
			$apkinfo = D("Apks")->where(array("app_id"=>$v["id"]))->field('category_p,category_p_son,category_s,category_s_son')->find();
			$apps[$k] = array_merge($v, $apkinfo);
		}
		...
		//添加提交数据记录
		$log = sprintf("category_fix_log_uid_%d_data_%s", $this->uid, var_export($_POST, true));
		Log::write($log, Log::EMERG);
		
		$category = array_map("intval", $_POST);
		foreach ($apps as $app)
		{
			$data = array();
			if($category["category_p".$app["id"]] > 0)
				$data["category_p"] = $category["category_p".$app["id"]];
			if($data["category_p"] && isset($category["son_category_p".$app["id"]]))
				$data["category_p_son"] = $category["son_category_p".$app["id"]];
			if($category["category_s".$app["id"]] > 0)
				$data["category_s"] = $category["category_s".$app["id"]];
			if($data["category_s"] && isset($category["son_category_s".$app["id"]]))
				$data["category_s_son"] = $category["son_category_s".$app["id"]];
			
			$model = D("Apks");
			$result = $model->where(array("app_id"=>$app["id"]))->data($data)->save();
			if(false === $result)
			{
				D("Syserror")->error("分类修复异常",
				json_encode($data).$model->getDbError(),
				$model->_sql(), 0, "2");
			}
		}
		
		$this->success("更新成功", U("Apps/Index"));
	}
