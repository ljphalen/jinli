...
class TestAction extends Action
{
	...
	function test_syncstatus()
	{
		$apps = D("Apps")->getField("id,status",true);
		
		$count = array();
		foreach ($apps as $appId=>$status)
		{
			$method = $status > 0 ? 'online' : 'offline';
			$res = helper("Sync")->done($appId, $method);

			$count[$appId][$method] = $res;
		}
	...
	function test_glob()
	{
		$p = $this->_get("path", "trim", "Data/Runtime");
		$f = $this->_get("file", "trim", "");
		$g = $this->_get("grep", "trim", "");
