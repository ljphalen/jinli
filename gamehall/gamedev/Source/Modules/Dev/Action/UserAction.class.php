...
class UserAction extends BaseAction {
	...
	public function changePwd() {
		...
		if ($accInfo) {
			...
			if ($accInfo ['crypted_password'] === $oldpassword) {
				...
				if ($res) {
					$this->assign ( "jumpUrl", U("User/info") );
	...
	function contact_edit(){
		...
		if (empty($id) || !$getOne = D("Contact")->_getOne($id, $this->uid))
			return $this->error("用户不存在");

		$this->assign("info", $getOne);
	...
	function save_contact(){
		if ($this->_post()) {
			$model = D("Contact");
			$post = $this->_post();
			$post['account_id'] = $this->uid;
			if (!empty($post['id'])) {
				$id = $post['id'];
				$getOne = D("Contact")->_getOne($id, $this->uid);
				empty($getOne) && $this->error("无法查到此联系人信息");
				$ret = $model->_update($id, $this->uid, $post);
				if (false !== $ret) {
	...
	function contact_del(){
		...
		$ret = D("Contact")->_del($id, $this->uid);
		if ($ret) {
			$this->redirect("user/contact");
		}
		$this->error("删除失败");
