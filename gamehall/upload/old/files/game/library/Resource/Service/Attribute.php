...
class Resource_Service_Attribute extends Common_Service_Base {
	...
	const ATTRIBUTE_LIST = 2;
	const CHANGE_LIST = 3;
	const RESOLUTION_LIST = 4;
	const VERTION_LIST = 5;
	const OPERTATOR_LIST = 6;
	const GAME_ATTRIBUTE = 7;
	const LABEL_TYPE = 8;
	const GAME_DEVICE = 9;
	
	public static  $mAttributeType =  array (
			self::ATTRIBUTE_LIST => '属性列表',
			self::CHANGE_LIST => '资费列表',
			self::RESOLUTION_LIST => '分辨率列表',
			self::VERTION_LIST => '版本列表',
			self::OPERTATOR_LIST => '运营商列表',
			self::GAME_ATTRIBUTE => '游戏属性',
			self::LABEL_TYPE => '标签类型',
			self::GAME_DEVICE => '游戏支持设备');
	const CACHE_EXPIRE = 86400;
	
    /**
     * Resource_Service_Attribute::getListByImeiSort
     * 获取个性化排序的主分类列表
     *
     * @param int $page
     * @param int $limit
     */
    public static function getListByImeiSort($imei) {
        
        //获取所有主分类
        $params = array('id'=>array('NOT IN', array(100, 101)), 'at_type' => 1, 'status' => 1);
        list($total, $result) = self::getList(1, 100, $params, array('sort'=>'DESC'));
        ...
        //获取imei分类点击数
        $categoryClickStatisticsDao = Common_Dao_SplitFactory::getDao("Client_Dao_CategoryClickStatistics");
        $categoryClickStatisticsItem = $categoryClickStatisticsDao->getByImei($imei);
        //加缓存 1小时过期
        $cacheKey = 'imei_category_statistics_' . $imei;
        $cacheData = Cache_Factory::getCache()->get($cacheKey);
        if ($cacheData != false) {
        	return $cacheData;
        }
        ...
        //imei分类排序数据
        $categoryData = array();
        if (isset($categoryClickStatisticsItem['id']) && trim($categoryClickStatisticsItem['category_json']) != '') {
            $categoryData = json_decode($categoryClickStatisticsItem['category_json'], true);
        }
        ...
        if (count($categoryData) <= 0) {
            $mainCategoryList = $result;
        } else {
            //id map
            $isImeiListMap = array();
            $notImeiListMap = array();
            foreach ($result as $value) {
                if (array_key_exists($value['id'], $categoryData)) {
                    $isImeiListMap[$value['id']] = $value;
                } else {
                    $notImeiListMap[$value['id']] = $value;
                }
            }
            //var_dump($result);
            ...
            //从大到小排序
            krsort($categoryData);
            foreach ($categoryData as $categoryId => $pv) {
                if (!isset($isImeiListMap[$categoryId])) {
                	continue;
                }
                $mainCategoryList[] = $isImeiListMap[$categoryId];
            }
            //var_dump($mainCategoryList);
            foreach ($notImeiListMap as $notImeiCategory) {
                $mainCategoryList[] = $notImeiCategory;
            }
        }
        
        //缓存1小时
        Cache_Factory::getCache()->set($cacheKey, $mainCategoryList, 3600);
    
        return $mainCategoryList;
    }
    
    /**
     * Resource_Service_Attribute::getListByImeiSortPage
     * 获取个性化排序的主分类列表 分页
     *
     * @param string $imei
     * @param int $page
     * @param int $limit
     */
    public static function getListByImeiSortPage($imei, $page = 1, $limit = 8) {
        $start = ($page - 1) * $limit;
        
        //获取所有排序分类
        $mainCategoryList = self::getListByImeiSort($imei);
        
        //数组分页
        $total = count($mainCategoryList);
        if ($start < ($total - $limit)) {
            return array($total, array_slice($mainCategoryList, $start, $limit));
        } elseif ($start < $total) {
            return array($total, array_slice($mainCategoryList, $start));
        }
	...
	public static function max($field = "", $params = array()){
		return self::_getDao()->max($field , $params);
	}
	...
	public static function  getJsonData($categorys, $checkVer, $game_caini, $sp, $tjUrl, $detailUrl, $page, $hasnext ,$intersrc, $source, $filter = array()) {
		...
		//猜你喜欢
		if($game_caini && ($model == "client") && strnatcmp($version, '1.5.1') < 0){
			...
			if(($model == "client") && $checkVer >= 2) $tmp['data-type'] = 0;
			$cn_img = $game_img;
		}
		...
		foreach($categorys as $key=>$value) {
			...
			if($checkVer >= 2 && ($model == "client")){
				$temp[$key]['data-type'] = 0;
			}
			$imgs[] = $value['img'];
		}
		if($game_caini && $page < 2 && ($model == "client") && strnatcmp($version, '1.5.1') < 0 ) array_unshift($temp, $tmp);
		if($game_caini && ($model == "client") && strnatcmp($version, '1.5.1') < 0 )  array_unshift($imgs, $cn_img);
    ...
    public static function getGameData($category_id,$sp,$filter = array()) {
        ...
        if($category_id == '100'){              //全部游戏
            ...
            $orderBy = array('downloads' => 'DESC', 'id' => 'DESC');
            list($total, $games) = Resource_Service_Games::getList(1, 3, $params, $orderBy);
        } else {
            if($filter){
	...
	public static function getDataVersion($type = self::GAME_ATTRIBUTE) {
		$redisCache = self::getCacheObject ();
		$versionKey = self::getversionKeyName($type);
	...
	public static function getversionKeyName($type = self::GAME_ATTRIBUTE){
		$versinoKey = Util_CacheKey::ATTRIBUTE_VERSION.'_'.$type;
		return $versinoKey;
	}
	
	public static function updateVersionToCache($type = self::GAME_ATTRIBUTE){
		$redisCache = self::getCacheObject ();
		$versionKey = self::getversionKeyName($type);
		$version = Common::getTime();
		$redisCache->set($versionKey, $version, self::CACHE_EXPIRE);
	...
	public static function getDataKeyName($type = self::GAME_ATTRIBUTE, $version){
		$dataKeyName = Util_CacheKey::ATTRIVUTE_DATA . '::' . $type.'_'.$version;
		return $dataKeyName;
	}
