...
class Client_EvaluationController extends Admin_BaseController {
	...
	public function edit_postAction() {
        ...
        $oldInfo = Client_Service_News::getNews($info['id']);
		$ret = Client_Service_News::updateNews($info, intval($info['id']));
		if (!$ret) $this->output(-1, '操作失败');
        $this->refreshCache($oldInfo['game_id']);
        $this->refreshCache($info['game_id']);
		$this->output(0, '操作成功.'); 		
	}
    ...
    private function refreshCache($gameId){
        Resource_Service_GameExtraCache::refreshGameEvaluation($gameId);
