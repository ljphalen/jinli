...
class Ad_RecommendnewController extends Admin_BaseController {
	...
	public function otherEditAction() {
	    ...
	    $info = $this->handleOtherInfoWithSort($info);
            $ids = common::resetKey($info, 'list_id');
            if($info[1]['list_type'] == 'new') {
                $newList = Client_Service_News::getNewsByIds(array('id' => array('IN',array_keys($ids)), 'status' => Game_Service_H5RecommendNewHd::STATUS_OPEN));
                foreach($newList as $key => $newInfo) {
                    $newTitle[$newInfo['id']] = $newInfo['title'];
                }
            } else {
                $newList = Client_Service_Hd::getHdByIds(array('id' => array('IN',array_keys($ids)), 'status' => Game_Service_H5RecommendNewHd::STATUS_OPEN));
                foreach($newList as $key => $newInfo) {
                    $newTitle[$newInfo['id']] = $newInfo['title'];
                }
            }
	...
	public function otherPostAction() {
	    $data = $this->getInput(array('recommend_id', 'type', 'day_id', 'list_id', 'list_title', 'title', 'list_resume'));
            Common_Service_Base::beginTransaction();
            if(!$data['title']) {
                $this->output(-1, '标题不可为空');
            }
	    if($data['recommend_id']) {
                $info['title'] = $data['title'];
                Game_Service_H5RecommendNew::updateRecommendNewBy($info, array('id' => $data['recommend_id']));
	...
	public function ajaxHandleAction() {
	    ...
	    if($data['type'] == 'new') {
                $checkInfo = Util_CheckInfoStatus::check(Util_CheckInfoStatus::TYPE_NEW, $data['infoId'], '', true);
	    } else {
                $checkInfo = Util_CheckInfoStatus::check(Util_CheckInfoStatus::TYPE_HD, $data['infoId'], '', true);
	    }
	    if($checkInfo) {
	        $this->output(0, 'ID正确', array('title' => html_entity_decode($checkInfo['title'])));
	    } else {
	...
	private function handleOtherPostWithRecommend($info, $recommend_id, $type) {
	    foreach($info['list_id'] as $key => $value) {
	        $list[] = array('list_type' => $info['type'], 'list_id' => $value, 'list_sort' => $key, 'list_title' => $info['list_title'][$key], 'recommend_id' => $recommend_id, 'list_resume' => $info['list_resume'], 'tmp_id' => Game_Service_H5HomeAutoHandle::getID());
	    }
	    if($type == 'add') {
	...
	public function gamesSortAction() {
	    $id = intval($this->getInput('id'));
	    $list = Game_Service_H5RecommendGames::getGames($id, 0, 9999);
	...
	private function cookImgData($requestData) {
		...
		if(!isset($requestData['link'])) $this->output(-1, '链接参数不能为空.');
                $msg = Game_Service_Util_Link::checkLinkValue($requestData['link_type'], $requestData['link']);
		if($msg !== '') {
		    $this->output(-1, $msg);
		}
