...
class Mall_GoodsController extends Admin_BaseController {
	...
	public function editGoodPostAction() {
		$info = $this->getPost(array('id', 'type', 'total_num', 'preson_limit_num', 'consume_point', 'title', 'descrip', 'img', 'icon', 'status', 'start_time', 'end_time', 'effect_time'));
		$info = $this->_cookData($info);
		$discountInfo = $this->getPost(array('isdiscount','zhekou','zhekou_start_time','zhekou_end_time'));
		$discountInfo = $this->checkDiscountData($discountInfo);
		$info['discountArr'] = $this->getDiscountInfo($discountInfo);
...
SCRIPT;
	...
	private function _cookData($info) {
		...
		if(!preg_match("/^[0-9]*[1-9][0-9]*$/",$info['total_num'])) $this->output(-1, '商品总数量只能为正整数.');
		if(!$info['preson_limit_num']) $this->output(-1, '每人兑换商品数量不能为空.');
		if($info['type'] == 1 && ($info['preson_limit_num'] >=2 || $info['preson_limit_num'] < 1 || $info['preson_limit_num'] > 1)) {
		    $this->output(-1, '每人兑换商品数量只能为1个.');
		}
		if(!$info['consume_point']) $this->output(-1, '兑换商品消耗积分不能为空.');		if(!$info['start_time']) $this->output(-1, '开始时间不能为空.');
