...
class Client_AcouponController extends Admin_BaseController {
	...
	public function sendTicketPostAction(){
		...
		$denomination = $this->getInput('denomination');
		...
		if($userObject == 2 && $denomination <= 0){
			$this->output(-1, 'A券的面额要大于等于0，请输入大于等于0的数');
		}
		if(!$startTime){
		...
		//组装赠送数据 导入赠送
		if($userObject == 1){
			$rs = $this->multiSendTicket($fileName, $startTime, $endTime, $userType, $reason);
		}elseif($userObject == 2){
			$rs = $this->individualSendTicket($denomination, $startTime, $endTime, $uname, $userType, $reason);
		}		 
	}
	 ...
	 */
	private function individualSendTicket($denomination, $startTime, $endTime, $uname, $userType, $reason){
		$aid = date('YmdHis').uniqid();
		if($userType == 1){
        ...
        $sendArr[] = array(
				'aid'=>$aid,
				'denomination'=>strval($denomination),
	 ...
	 * @param unknown_type $endTime
	 */
	private function multiSendTicket($fileName, $startTime, $endTime, $userType, $reason){
