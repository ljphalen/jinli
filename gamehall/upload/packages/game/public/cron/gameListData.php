...
$perPage = 100;
$begin = Resource_Service_GameListData::buildBegin();
if($begin){
    $work = true;
    $page = 1;
    do {
        ...
        $result = Resource_Service_GameListData::saveListData($listData);
        if (!$result) {
            $work = false;
    ...
    if($work){
        $finish = Resource_Service_GameListData::buildFinish();
        if(!$finish){
