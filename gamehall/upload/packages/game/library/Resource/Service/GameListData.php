...
class Resource_Service_GameListData {
     ...
     * @return bool
     */
    public static function buildBegin(){
        $obj = self::getListContent();
        return $obj->buildContentBegin();
    }
     ...
     * @return bool
     */
    public static function buildFinish(){
        $obj = self::getListContent();
        return $obj->buildContentFinish();
    }
     ...
     * @return bool
     */
    public static function saveListData($listData){
        $obj = self::getListContent();
        return $obj->storeListContent(self::GAME_ID_KEY, $listData);
    }
    ...
    private static function getListContent(){
        ...
        if (!self::$instances[$key]) {
            self::$instances[$key] = new Cache_ListContent($contentKey);
        }
    ...
    public static function getIndexInstance($indexKey) {
        ...
        $contentKey = Util_CacheKey::GAME_LIST_CONTENT_KEY;
        if (!self::$instances[$key]) {
            self::$instances[$key] = new Cache_ListIndex($contentKey, $indexKey);
        }
        return self::$instances[$key];
    }
