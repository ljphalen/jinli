...
class Game_Service_H5HomeAutoHandle {
    ...
    public static function handleTmpDataToSave() {
        ...
        try {
            foreach ($tmpTableName as $key => $table) {
                $listDataTmp = self::getTableName($table)->getList('0', self::DATA_TMP_NUM, array('tmp_id' => self::getID()));
    ...
    private static function saveToFirstStepData($tableName, $param, $toDay = '') {
        $listDataTmp = self::getTableName($tableName)->getList('0', self::DATA_TMP_NUM, $param);
    ...
    private static function saveToNextStepData($tableList, $idList, $isCopy = '') {
        ...
        foreach ($tableList as $key => $table) {
            $listDataTmp = self::getTableName($table)->getList('0', self::DATA_TMP_NUM, array('recommend_id' => array('IN', $oldIdList)));
    ...
    private static function getTableName($name) {
        return self::getDao()->changeTableName($name);
    }
