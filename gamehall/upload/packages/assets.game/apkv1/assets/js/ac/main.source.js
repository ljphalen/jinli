(function() {
	...
	var activity = {
				...
				success:function(data){
							...
							if(info['discount']){
								html += '<li>\
									<a data-type="0" data-viewtype="WebView" data-source="prizeexchange" data-infpage="'+info["title"]+','+info["redirectLink"]+'">\
										<div class="goods-list window-discount-title rem-border-1px">\
										<i class="bg-discount-title"><div>限时<span>'+info["discount"]+'</span>折</div></i>\
										<img src="http://assets.gionee.com/apps/game/apkv1/pic/blank.gif" alt="" data-src="'+info["img"]+'">\
											<span class="consume-points">消耗积分：<em><strike>'+info["consumePoints"]+'</strike></em></span>\
											<span class="enough-points">只需积分：<em>'+info["enoughPoints"]+'</em></span>\
					...
					setTimeout(function(){
						if(hasNext == 0 || hasNext == 'false'){
							...
							if(isGoods){
								var version=activity.getUrlParam('sp').split('_')[1];
								if(activity.isVersionBiggerorEqual(version,'1.5.9')){
									$('.J_loadMore .bottom').attr('style','background-image:url()');
								}
							}
						} else{
							moreBtn.addClass('invisible');
		...
		getServerId:function(){
			var version=activity.getUrlParam('sp').split('_')[1];
			var serverId='';
			if(activity.isVersionBiggerorEqual(version,'1.5.7')){//1.5.7增加的取serverId的接口
				serverId=activity.getValueAction(action.getServerId,{url:location.href}).serverId;
			}
			return serverId;
...
function isVersion(targetVersion){
