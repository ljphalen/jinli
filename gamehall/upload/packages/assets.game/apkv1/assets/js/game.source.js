...
;(function(iCat, $) {
    iCat.app('Gapk', function() {
        ...
        //客户端页面跳转的处理
        function skipPage(data, type,viewType,id,gameId,source,subViewType) {
            if (!data) return;
            var infpage = data.split(','),
                version = GameApk.ApiVersion;
            if(!(window.gamehall||navigator.gamehall)){
                location.href = infpage[infpage.length-1];
                return;
            }
            switch(version){
                case 'v1':
                ...
                case 'v2':
                    ...
                    break;
                case 'v3':
                    openV3Page(type,viewType,id,gameId,infpage,source,subViewType);
                    break;
            }
        }
        ...
        function openV3Page(type,viewType,id,gameId,infpage,source,subViewType){
            var cfg={}; cfg.args={ };
            var url='',title='';
            ...
            if(viewType.toLowerCase()=='webview'||subViewType.toLowerCase()=='webview'){
                url=infpage[infpage.length-1];
                title=infpage[0];
            }
            cfg.args.newArgs={
                viewType:viewType,
            ...
            //for 1.5.8 添加subViewType
            if(subViewType!=''){
                cfg.args.newArgs.param.subViewType=subViewType;
            }
            if(type==0){
                cfg.type='list';
        ...
        var o = {
            ...
            openPage: function(argument) {
                ...
                $('body').delegate('a[data-infpage]', 'click', function() {
                    var data = $(this).attr('data-infpage') || '',
                        ...
                        subViewType=$(this).attr("data-subViewType")||'',
                        id=$(this).attr('data-id'),gameId=$(this).attr('data-gameId');
                    if (GameApk.ApiVersion == "v1") {
                        skipPage(data);
                    } else {
                        skipPage(data,dataType,viewType,id,gameId,source,subViewType);
                    }
                });
            ...
            lazyLoad: function(pNode, t) {
                var imgs = $(pNode).find('img[src$="blank.gif"],img[src$="blank1.gif"],img[src$="blankAvator.gif"]'),
                    _fn = function(o) {
