...
var LoadIcon = React.createClass({
	render:function(){
		return(
			<div className="layer-load" name="ajaxLoad">
				<div className="bg-layer">
				</div>
				<div className="bg-icon">
					<i className="load-icon"></i>
					<p className="msg">加载中,请稍后....</p>
				</div>
			</div>
		);
	}
...
var Row =  React.createClass({
	...
	render:function(){
		...
		return(
			<a href={item.href}>
				<li className="row">
					<Now index={this.props.index+1} />
					<h3 className="game-name omit">{item.name}</h3>
					<span className="go-forward"></span>
				</li>
			</a>
		);
...
var ProbablyLike =  React.createClass({
	render:function(){
			var name = "你可能还喜欢"
				,ProbablyShow = "block"
			;
		...
		return (
			<div className="probably-like" style={{"display":ProbablyShow}}>
