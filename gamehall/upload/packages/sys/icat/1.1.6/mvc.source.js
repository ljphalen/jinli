...
(function(iCat, root, doc){
	...
	UTIL({
		...
		ajax: function(options) {
			...
			options.hasContent = !/^(?:GET|HEAD)$/.test(options.type);
			if(iCat.isObject(options.data)){
				var argus = [];
				iCat.foreach(options.data, function(v, k){
					argus.push(k + '=' + v);
				});
				options.data = argus.join('&');
			}
			if(!options.hasContent){
				options.url = options.url + (options.url.indexOf('?')<0? '?':'&') + options.data;
			}
			
			var xml_http;
			var	contentType = options.hasContent?
				'application/x-www-form-urlencoded;charset=UTF-8' : 'text/html;charset=UTF-8';
			try{
			...
			xml_http.setRequestHeader('Content-Type', contentType);
			...
			xml_http.send(options.hasContent && options.data || null);
			...
			makeHtml: function(ss, pNode, clear){
				...
				if(iCat.isArray(ss)){
					iCat.foreach(ss, function(s){
						_makeHtml(s, UTIL.queryOne(pNode) || doc.body, clear);
					});
				}
				else if(iCat.isString(ss)){
					_makeHtml(ss, UTIL.queryOne(pNode) || doc.body, clear);
				}
			},
	...
	View.prototype = {
		...
		_render: function(mdata){
			...
			var that = this, html = '',
				...
				elBaseBed = UTIL.queryOne(GData.baseBed) || doc.body,
			...
			if(pWrap){
				...
				var o = doc.createElement('wrap');
					o.innerHTML = html;
				var cnodes = o.childNodes, el;
				/*for(var i=0, len=cnodes.length; i<len; i++){
					el = cnodes[i];
					if(el && el.nodeType===1) fragment.appendChild(cnodes[i]);
				}*/
				while(cnodes.length){
					fragment.appendChild(cnodes[0]);
				}
	...
	function ajaxFetch(cfg, now, fdata){
		...
		if(now || !gkey){
			...
			UTIL.ajax(iCat.mix({
				...
				success: function(data){
					...
					if(cfg.repeatKey){
						...
						if(ajaxConfig.success) ajaxConfig.success(jsonData);
						fnOpt(jsonData);
					}
					else if(isDiff || cfg.mustdo){
			...
			}, ajaxConfig, 'success, error', true, false));
		} else {
	...
	function _storage(){
		...
		if(len===1)
			...
			key = argus[0];
			data = argus[1];
			data = iCat.isString(data)? data : JSON.stringify(data);
			ls.removeItem(key);
			try {
				ls.setItem(key, data);
			} catch(e) {};
