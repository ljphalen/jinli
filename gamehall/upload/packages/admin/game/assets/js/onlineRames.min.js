...
if(i>=n){l='[name="list"]:eq(0)';}else{l='[name="list"]:eq('+(i+1)+")";}a(i,l,m);});$("#saveForm").on("click",'[name="edit"],[name="add"]',function(){var i=$(this).attr("link");
confirmInfo="";location.href=i;});$("#save").click(function(){var n=new f("#file li"),i=new f('#recommend [name="list"]'),m=$("#saveForm"),l;if(!m.length){return;
}l={dataType:"json",data:{banner:JSON.stringify(n.getValArr()),recommend:JSON.stringify(i.getValArr())},success:function(o){ajaxLoader.hide();if(o.success){confirmInfo="";
ajaxRedirect(o,j.toUrl);return;}$("#alert").dialog("open");$('#alert [name="msg"]').html(o.msg);}};ajaxLoader.show();m.ajaxSubmit(l);});$("#saveForm").on("click",'[name="del"]',function(){var l=$(this).attr("ajaxUrl"),i=$(this).attr("link");
if(confirm("此操作不可恢复，确定要删除吗？")){$.get(l,function(m){if(m.success){confirmInfo="";location.href=i;}else{$("#alert").dialog("open");$('#alert [name="msg"]').html(m.msg);
}});}});$("#alert").dialog({autoOpen:false,width:400,buttons:{Ok:function(){$(this).dialog("close");}}});var d=template("rollHtml",{supplies:j.data.roll}),h=template("listHtmlSet",{imgUrl:j.imgUrl,supplies:j.data.recommend}),k=template("fileHtml",{imgUrl:j.imgUrl,supplies:j.data.roll}),g=template("listHtml",{imgUrl:j.imgUrl,supplies:j.data.recommend});
...
e<c;e++){if(j.data.recommend[e].type=="openList"){$("#openList").hide();}if(j.data.recommend[e].type=="package"){$("#package").hide();}}$('#package [name="open"]').click(function(){var i=$(this).attr("link");
$.get($(this).attr("ajaxUrl"),{},function(l){if(l.success){confirmInfo="";location.href=i;return;}$("#alert").dialog("open");$('#alert [name="msg"]').html(l.msg);
});});});