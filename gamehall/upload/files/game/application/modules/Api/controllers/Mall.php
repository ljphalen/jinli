...
class MallController extends Api_BaseController {
	...
	private function queryExchangeList($uuid, $reqPage) {
	    ...
	    do{
	       ...
	       foreach ( $srcData as $value ) {
	       	   $info = $this->getGoodInfo($value['mall_id']);
	           $dataItem['id'] = $value['id'];
	           $dataItem['prizeName'] = $this->getPrizeName($info);
	...
	private function getPrizeName($good) {
	   if($good['type'] == Mall_Service_Goods::DISCOUNT_COUPON || $good['type'] == Mall_Service_Goods::GIFT){
	       $exchangeLog = Mall_Service_GiftExchangeLog::getBy(array('good_id'=>$good['id']));
	       $title = $good['title'].'激活码:('.$exchangeLog['activation_code'].')';
	   } else {
	       $title = $good['title'];
	   }
	   return $title;
	...
	private function getGoodInfo($mallId) {
