...
class Thirdinterface_GameController extends Api_BaseController {
	...
	private function getGameListDataForAoRuan($info) {
		...
		$searchParams['online_time'][0] = array('>=', $info['starttime']);
		$searchParams['online_time'][1] = array('<=', $info['endtime']);
		//$searchParams['status'] = Resource_Service_Games::STATE_ONLINE;
	...
	private function makeGameInfoForAoRuan($gameInfo, $freeId, $adId) {
		...
		$downlink = Resource_Service_GameData::filterDownloadUrl($gameVersionInfo['link']);
		...
		$gameListData = array('packageName'=> $gameInfo['package'],
					           ...
					           'apkSize'=>$gameVersionInfo['file_size'],
					           'createTime'=>intval($gameInfo['create_time']),
					           'updateTime'=>intval($gameInfo['online_time']),
	...
	private function getGameListDataForWanKa($info) {
		$searchParams['online_time'][0] = array('>=', date('Y-m-d H:i:s',$info['startTime']));
		$searchParams['online_time'][1] = array('<=', date('Y-m-d H:i:s',$info['endTime']));
		$searchParams['status'] = Resource_Service_Games::STATE_ONLINE;
		$orderBy = array('online_time'=>'ASC');
