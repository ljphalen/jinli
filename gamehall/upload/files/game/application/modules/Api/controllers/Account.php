...
class AccountController extends Api_BaseController {
	...
	private function saveGetTimeBehaviour() {
	    ...
	    $behaviour->saveBehaviours(Client_Service_ClientBehaviour::ACTION_GET_TIME);
	}
	...
	public function reggvcAction(){
		$request = $this->getInput(array('tn', 'vid', 'vtx', 'sp'));
		$tn = $request['tn'];
		$vid = $request['vid'];
		$vtx = $request['vtx'];
		$sp = $request['sp'];
		if(!$tn || !$vid || !$vtx || !$sp) $this->_response('-1');
		$package = 'gn.com.android.gamehall';
		$items = self::_parseSp($sp);
		$imei = $items['imei'];
		$pkgVersion = $this->getPkgVersion($sp, $package);
		$ip = Common::getClientIP();
		$data = Api_Gionee_Account::registerByGvc($tn, $vid, $vtx, 'vtext', $imei, $pkgVersion, $ip);
		$this->_response('', array('api' => $data));
	}
	 ...
	 * @param string imei    		加密后的imei号
	 * @param string client_pkg    	客户端包名。
	 * @param string mode    		登录端 【1-客户端 2-web】
	 * @param string sp      		客户端sp参数
	 * @return json          		接口返回值
	 */
	public function pregAction(){
		$request = $this->getInput(array('carrier', 'imei', 'client_pkg', 'mode', 'sp'));
		$carrier = $request['carrier'];
		$secretDeviceId = $request['imei'];
		$mode = $request['mode'];
		$package = $request['client_pkg'];
		$sp = $request['sp'];
		if(!$carrier || !$secretDeviceId || !$package || !$mode || !$sp) $this->_response('-1');
		$pregVersion = $this->getPkgVersion($sp, $package);
		$result = Api_Gionee_Account::pregByClient($carrier, $secretDeviceId, $pregVersion);
		$data = $this->packageResonse($result);
		$this->_response('', array('api' => $data));
	}
	 ...
	 * @return array
	 */
	private function packageResonse($data){
		if($data['data']){
			return json_decode($data['data'], true);
		}
		$authString = $data['headers']['WWW-Authenticate'];
		$authString = str_replace(array('SMS ','"',','), array('','','&'), $authString);
		parse_str($authString, $authArray);
		return $authArray;
	}
	 ...
	 * 
	 */
	public function pregresultAction(){
		$request = $this->getInput(array('session', 'mode', 'sp'));
		$session = $request['session'];
		$mode = $request['mode'];
		$sp = $request['sp'];
		if(!$session || !$mode || !$sp) $this->_response('-1');
		$time = Common::getTime();
		$response = Api_Gionee_Account::pregResult($session);
		//清除帐号中心返回tn参数中的+86
		if($response['tn']){
			if(stristr($response['tn'], '+86') !== FALSE){
				$response['tn'] = substr($response['tn'], 3);
			}
		}
		$result = array('api' => $response);
		if($response['u'] && $response['tn']){
			//v1.5.3 增加pk
			$p= Api_Gionee_Account::encode_password($response['pt']);
			...
			//登录初始化
			$this->loginInitial($response['u'], $sp['game_ver'], $time, $sp['imei'], $response['tn']);
	...
	public function getsmsAction(){
		$request = $this->getInput(array('tn','s','sp'));
		$tn = $request['tn'];
		$s = ($request['s']) ? $request['s'] : '';
		$sp = $request['sp'];
		if(!$tn|| !$sp) $this->_response('-1');
		$package = 'gn.com.android.gamehall';
		$items = self::_parseSp($sp);
		$imei = $items['imei'];
		$pkgVersion = $this->getPkgVersion($sp, $package);
		$ip = Common::getClientIP();
		$data = Api_Gionee_Account::getSmsForRegister($tn, $s, $imei, $pkgVersion, $ip);
		$this->_response('', array('api' => $data));
	}
	...
	public function regsmsAction(){
		$request = $this->getInput(array('tn','sc','s','sp'));
		$tn = $request['tn'];
		$sc = $request['sc'];
		$s = ($request['s']) ? $request['s'] : '';
		$sp = $request['sp'];
		if(!$tn || !$sc || !$sp) $this->_response('-1');
		$package = 'gn.com.android.gamehall';
		$items = self::_parseSp($sp);
		$imei = $items['imei'];
		$pkgVersion = $this->getPkgVersion($sp, $package);
		$ip = Common::getClientIP();
		$data = Api_Gionee_Account::registerBySmsCode($tn, $sc, $s, $imei, $pkgVersion, $ip);
		$this->_response('', array('api' => $data));
	}
	...
	public function loginAction(){
	    ...
	    if (Common::isValidImei($imei)) {
			$ret = $this->isLegalLogin($imei);
			if(!$ret) {
				$this->_response('-1');
			}
		}
	 ...
	 * 检查登陆次数
	*/
	private function isLegalLogin($imei) {
		$unameNum = 0;
		$search = $unames = array();
		$currTime = strtotime('-1 day');
		$recently = floor($currTime/3600) * 3600;
	...
	public function checkloginAction(){
		$request = $this->getInput(array('uuid', 'mode', 'lctime', 'sp', 'token', 'h', 'getExtra'));
		$uuid = $request['uuid'];
		$mode = $request['mode'];
		...
		if(isset($request['getExtra'])) {
			$userInfo = Account_Service_User::getUserInfo(array('uuid'=>$uuid));
		    $address = $this->clientAddress($userInfo);
		    $data['data'] = $address;
		}
		$this->_response('', $data);
	}
	...
	public function modifypassAction(){
		$request = $this->getInput(array('uuid', 'uname','oldPassword', 'newPassword', 'vid', 'vtx', 'vty', 'mode', 'sp', 'token', 'h'));
		$uname = $request['uname'];
		$uuid = $request['uuid'];
		...
		$h = $request['h'];
		//参数不符合
		if(!$uuid || !$uname || !$oldPassword || !$newPassword || !$mode || !$h || !$token) $this->_response('-1');
		//token过期或异常
		if(!($this->_checkToken($h, $token))) $this->_response('-2');
		...
		$vtx = ($request['vtx']) ? $request['vtx'] : '';
		$vty = ($request['vty']) ? $request['vty'] : '';
		$oldPassword = Api_Gionee_Account::encode_password($oldPassword);
		$newPassword = Api_Gionee_Account::encode_password($newPassword);
		$response = Api_Gionee_Account::passmodify($uname, $oldPassword, $newPassword, $vid, $vtx, $vty);
		$result = array('api' => $response);
		if($response['u']){
			$data = array(
				'passkey' => Api_Gionee_Account::createPasskey($response['u'], $newPassword), 
			);
			Account_Service_User::updateUser($data, array('uuid' => $uuid));
			$userInfo = Account_Service_User::getUserInfo(array('uuid'=>$uuid));
	 ...
	 * @param string token token 值
	 * @param string h  请求接口之前 h 值
	 * @return 状态码【0-需要登陆 1-操作正常 2-包含敏感词 -3-操作失败 -4-生日格式非法】
	 */
	public function modifyuserAction(){
		$request = $this->getInput(array('uuid', 'uname', 'nickname', 'sex', 'birthday', 'favorType', 'mode', 'sp', 'token', 'h'));
		$uname = $request['uname'];
		$uuid = $request['uuid'];
		...
		$h = $request['h'];
		//参数不符合
		if(!$uname || !$mode || !$h) $this->_response('-1');
		//token过期或异常
// 		if(!($this->_checkToken($h, $token))) $this->_response('-2');
		//生日非法
		if($birthday && (strtotime($birthday) > time())) $this->_response('-4');
	...
	public function saveAvatarAction(){
		...
		$h = $request['h'];
		//参数不符合
		if(!$uuid || !$mode || !$h) $this->_response('-1');
		//账号是否在线-v1.5.4使用uuid
		$online = Account_Service_User::checkOnline($uuid, $imei, 'uuid');
		if(!$online) $this->_response('0');
		//上传到本地临时目录
	...
	private function _addUser($data, $act){
		...
		if (!$user) {
			...
			Account_Service_User::updateUser($reg, array('uuid' => $data['u']));
		}
	...
	private function _loginProcess($data){
		...
		if (!$user) {
			...
			if($data['tn'] != $user['uname']){
				...
				//更新用户信息表
				Account_Service_User::updateUserInfo(array('uname'=>$data['tn']), array('uuid' => $data['u']));
		...
		$response = array(
				...
				'avatar'=> $userInfo['avatar'] ? $attachPath . $userInfo['avatar'] : "",
				'lltime' => $time,
				'birthday' => ($userInfo['birthday']) ? $userInfo['birthday'] : "0000-00-00",
		);
		$address = $this->clientAddress($userInfo);
		$response = array_merge($response, $address);
		return $response;
	}
	
	/**
	 * 预注册客户端中版本号信息
	 * 版本号信息, 格式：机型/ROM版本号/App名称/App版本号, 若某项信息没有，则以空字符串代替，分隔符需要保留。
	 * @param string $sp
	 * @param string $pkg
	 * @return string 
	 */
	private function getPkgVersion($sp, $pkg){
		$items = self::_parseSp($sp);
		$version = sprintf('%s/%s/%s/%s', $items['device'], $items['rom_ver'], $pkg, $items['game_ver']);
		return $version;
	}
	...
	public function loginSendAction(){
		...
		$info = $this->getInput(array('puuid', 'uname', 'sp', 'clientId', 'serverId', 'imei'));
		...
		$this->inspectorIsValidRequest ( $info, $clientVersion, $data );
	
	    if($info['puuid']){
			$cache = Cache_Factory::getCache();
			$cacheHash = Util_CacheKey::getUserInfoKey($info['puuid']);
...
// 		$activity ->sendTictket();
	 ...
	 * @param clientVersion
	 */
	 private function inspectorIsValidRequest($info, $clientVersion, $data) {
	...
	private function clientAddress($userInfo=array()) {
	    $address = array(
    	    'address' => !is_null($userInfo['address']) ? strip_tags(html_entity_decode($userInfo['address'])) : "",
    	    'receiverName' => !is_null($userInfo['receiver']) ? strip_tags(html_entity_decode($userInfo['receiver'])) : "",
    	    'tel' => !is_null($userInfo['receiverphone']) ? strip_tags(html_entity_decode($userInfo['receiverphone'])) : "",
    	    'qq' => !is_null($userInfo['qq']) ? strip_tags(html_entity_decode($userInfo['qq'])) : "",
    	    'vipLevel' => $userInfo['vip'] ? $userInfo['vip'] : User_Config_Vip::VIP_1,
        );
	    return $address;
	}
