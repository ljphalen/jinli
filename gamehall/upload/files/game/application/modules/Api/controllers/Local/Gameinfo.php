...
class Local_GameinfoController extends Api_BaseController {
    ...
    public function getDetailsAction() {
        ...
        $this->fillBasicInfomation($gameInfo, $from);
    ...
    public function getAdditionalInfoAction() {
        ...
        $this->fillGifts($gameId, $clientVersion);
    ...
    private function fillGifts($gameId, $clientVersion) {
        $top3List = array();
    ...
    private function fillBasicInfomation($gameInfo, $from) {
        ...
        $this->mDetailsData['downloadCount'] = (self::GAME_SOURCE_BAIDU == $from)?'':$gameInfo['downloadCount'];
...
str;
    ...
    private function fillOpenInfo($gameId) {
        $openList = array();
        $open = Game_Api_WebRecommendOpen::getNewOpen($gameId);
        if($open) {
            $openData = array();
            $dateTime = date('n月j日 H:i', $open['open_time']);
            list($date, $time) = split(' ', $dateTime, 2);
            $openData['date'] = $date;
            $openData['time'] = $time;
            if($open['server_code']) {
                $openData['areaId'] = $open['server_code'] . '区';
            }
            $openData['areaName'] = $open['server_name'];
            $openType = Game_Service_GameOpen::$open_type[$open['open_type']];
            if(! $openType) {
                $openType = "";
            }
        ...
        $this->mDetailsData['serviceInfoItems'] = $openList;
    }
    
    private function fillBasicInfomationForWanka($gameInfo, $from) {
        $mDetailsData = array();
        $mDetailsData['gameId'] = $gameInfo['id'];
        $mDetailsData['iconUrl'] = $gameInfo['img'];
        $mDetailsData['name'] = html_entity_decode($gameInfo['name'], ENT_QUOTES);
        $mDetailsData['category'] = html_entity_decode($gameInfo['category_title'], ENT_QUOTES);
        $mDetailsData['fileSize'] = $gameInfo['size'] . 'M';
        $mDetailsData['language'] = html_entity_decode($gameInfo['language'], ENT_QUOTES);
        $mDetailsData['score'] = $gameInfo['client_star'];
        return $mDetailsData;
    }
    
    public function getDetailsForWankaAction() {
        list($gameId, $from) = $this->parseGameIdInput();
    
        $gameInfo = $this->getGameResourceById($gameId, $from);
        $mDetailsData = $this->fillBasicInfomationForWanka($gameInfo, $from);
        $fullPicture = $gameInfo['gimgs'];
        $mDetailsData['bannerList'] = $fullPicture;
    
        $this->localOutput(0, 'game infomation', $mDetailsData);
    }    
}
