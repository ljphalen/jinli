...
class Local_IndexController extends Api_BaseController {
	...
	public function columnAction() {	
	 ...
	 private function makeChannelListItems($channelListData, $apiConfig, $attachroot) {
	 	...
	 	foreach ($channelListData as $channelList ){
	 		...
	 		if(strnatcmp($clientVersion, '1.6.1') >= 0 && $channelList['relevance'] == 'bbslist'){
	 			$firstChanelItems[] = array( 'title'=>$channelList['name'],
	 					...
	 					'items'=>$this->getDeafaultBbsItems()
	 			) ;
            // 二级频道大于1个
	 		}elseif(count($channelList['items']) > 1){		 
	...
	private function  getDeafaultBbsItems(){
		$items =array( 
		           0=> array('title'=>'资讯',
				       		 'source'=>'wknewscenter',
				             'viewType'=>'WankaNewsCenterView',
				       ),
					1=> array('title'=>'社区',
							  'source'=>'forum',
							  'viewType'=>'ForumView',
