...
public $perpage = 10;
	...
	public function myticketAction() {
		...
		if($online){
			...
			if( $rs->data){
                ...
                //游戏券
                if($rs_list['gameVoucher'] > 0){
                    $result['gameTicket'] = number_format($rs_list['gameVoucher'], 2, '.', '');
                }
	...
	/**
	 * 
	 * @param  $basePrizeConfig
	 * @param  $online
	 * @param  $continueLoinDay
	 * @return string
	 */
	...
	private function ticketList($page, $uuid){
		...
		foreach($dataList as $m=>$n){
            ...
            $out[$m]['ticketType'] = $n['ticket_type'];
            ...
            $out[$m]['gameTips'] = $this->getGameTips($n);
    ...
    private function getGameTips($item){
        if($item['ticket_type']==1) return "";
        $gameInfo = Resource_Service_GameData::getBasicInfo($item['game_id']);
        return ($gameInfo) ? $gameInfo['name'] : '';
    }
}
