...
class Festival_TouchgameController extends Client_BaseController{
    ...
    public function shareAction(){
        ...
        $account = $this->getOnlineAccount($request);
        if(!$account) $this->output(0, '',array('share'=>false));
        $uuid = $account['uuid'];
        ...
        if($userChance && (!$userChance['share'])){
            ...
            $this->output(0, '', array('share'=>true, 'remainTimes' => $userChance['remainTimes']));
        }
        $this->output(0, '', array('share'=>false));
    }
    ...
    private function saveUserChance($uuid, $data){
        $expire = strtotime('tomorrow') - time() - 1;
        $cacheKey = $this->getCacheKey();
        $cache = Cache_Factory::getCache();
