...
class Freedl_HdController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		if ($s['start_time']) {
			$startTime = Util_TimeConvert::floor(strtotime($s['start_time']), Util_TimeConvert::RADIX_DAY);
			$params['start_time'] = array('>=', $startTime);
		}
		if ($s['end_time']) {
			$endTime = Util_TimeConvert::ceil(strtotime($s['end_time']), Util_TimeConvert::RADIX_DAY) - 1;
	...
	private function _cookData($info) {
		...
		$info['start_time'] = Util_TimeConvert::floor($info['start_time'], Util_TimeConvert::RADIX_DAY);
		$info['end_time'] = Util_TimeConvert::ceil($info['end_time'], Util_TimeConvert::RADIX_DAY) - 1;
