...
class Client_GiftController extends Admin_BaseController {
	...
	public function gamelistAction() {
		...
		foreach($result as $key=>$value){
			...
			$remainGiftNum[$gameId] = Client_Service_Gift::getGiftNumByGameId($gameId, '1.6.1');
		} 
	...
	public function addHotPostAction() {
		...
		if ($gift_info['vip_level'] > 1) $this->output(-1, '热门礼包只能添加vip等于1的礼包,该礼包vip等级大于1');
	...
	public function editAction() {
		$id = $this->getInput('id');
		$giftVipLevel = $this->getInput('giftLevel');
		$info = Client_Service_Gift::getGift(intval($id));
		$giftLevel = User_Config_Vip::getGiftLevel($info['vip_level']);
		$vipTitle = User_Config_Vip::$giftName[$giftLevel];
		...
		$this->assign('remainGifts', $remainGifts);
		$this->assign('info', $info);
		$this->assign('giftLevel', $giftLevel);
		$this->assign('giftVipLevel', $giftVipLevel);
		$this->assign('vipTitle', $vipTitle);
	}
	...
	public function editlogAction() {
		...
		$giftLevel = User_Config_Vip::getGiftLevel($gift_info['vip_level']);
		$vipTitle = User_Config_Vip::$giftName[$giftLevel];
	...
	public function edit_postAction() {
		...
		Client_Service_Gift::updataGiftNumCacheByGiftId($info['id']);
		
		
		$this->updataGameCache($info['game_id']);
		if (!$ret) {
			$this->output(-1, '操作失败');
		}
	    ...
	    //如果该礼包vip等级大于1,则删除热门礼包中该礼包
	    if($info['vip_level'] > User_Config_Vip::GIFT_1){
	        Client_Service_GiftHot::deleteBy(array('gift_id'=>$info['id']));
	    }
	...
	private function updataGameCache($gameId){
	    Resource_Service_GameExtraCache::refreshGameGift($gameId);
	...
	public function privilegeGiftListAction() {
	    ...
	    foreach($giftLevelList as $giftLevel=>$giftLevelTitle){
	        $total = $this->getOneOfPrivilegeGiftListByUserVipLevel($giftLevel);
	        $privilegeGiftList[$giftLevel] = array(
	                'total' => $total,
	                'giftLevelTitle' => User_Config_Vip::$giftName[$giftLevel],
	        );
	    }
	...
	public function oneOfPrivilegeGiftListAction() {
		...
		foreach($privilegegiftsList as $key=>$value){
		    ...
		    $gameInfo = Resource_Service_GameData::getBasicInfo($gameId);
		    $gameName[$value['id']] = $gameInfo['name'];
		}
	...
	private function getOneOfPrivilegeGiftListByUserVipLevel($giftLevel) {
	    return $this->getPrivilegeGiftData($giftLevel);
	}
	
	private function getPrivilegeGiftData($giftLevel) {
	    $search = array();
	    if($giftLevel == User_Config_Vip::GIFT_1){
	        $search['game_status']  = Resource_Service_Games::STATE_ONLINE;
	        $total = Client_Service_GiftHot::count($search);
	    } else {
