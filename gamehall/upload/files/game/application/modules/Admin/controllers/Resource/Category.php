...
class Resource_CategoryController extends Admin_BaseController {
    ...
    public function batchUpdateGameAction(){
        ...
        //更新缓存
        $this->refreshCategoryIndex($info['ids']);
        $this->output('0', '操作成功.');
    }
    ...
    private function refreshCategoryIndex($ids){
        foreach($ids as $id){
            $categoryData = Resource_Service_GameCategory::getBy(array('id'=>$id));
            Async_Task::execute('Async_Task_Adapter_CategoryList', 'updateIndex', $categoryData['game_id']);
        }
    }
}
