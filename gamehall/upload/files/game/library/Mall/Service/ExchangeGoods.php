...
class Mall_Service_ExchangeGoods extends Common_Service_Base{
	...
	public static function exchangeGoods($data) {
		...
		try {
    		...
    		if($trans) {
    		    ...
    		    if($good['type'] == Mall_Service_Goods::GIFT || $good['type'] == Mall_Service_Goods::DISCOUNT_COUPON){
	...
	private static  function saveExchangeGoodLog($data, $good) {
	    $exchangeLog = array();
	    $exchangeTime = ($good['type'] == Mall_Service_Goods::ACOUPON ||  $good['type'] == Mall_Service_Goods::GIFT || $good['type'] == Mall_Service_Goods::DISCOUNT_COUPON ? strtotime($data['exchange_time']) : '');
	    $exchangeStatus = ($good['type'] == Mall_Service_Goods::ACOUPON || $good['type'] == Mall_Service_Goods::GIFT || $good['type'] == Mall_Service_Goods::DISCOUNT_COUPON ? 1 : 0);
	    $exchangeLog = array(
	            'id'=>'',
	            'mall_id'=>$data['goodId'],
	            'uuid'=>$data['uuid'],
	...
	private static  function updateExchangeGiftLog($data, $good) {
	    ...
	    if(!$saveExchangeLogRet || ($good['type'] == Mall_Service_Goods::GIFT && !$addGiftLogRet)){
	        return false;
	    }
