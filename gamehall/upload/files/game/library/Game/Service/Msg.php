...
class Game_Service_Msg {
	...
	static $msgType    = array(			
		...
		*/		
	    ...
	    109  =>  array('key'=>'giftGive'   ,'txt'=>'礼包赠送消息'),
	    110  =>  array('key'=>'giftGive'   ,'txt'=>'道具赠送消息'),
	...
	public static function addMsg($data) {
		if (!is_array($data)) return false;
		$sendInputs = html_entity_decode($data['sendInput'], ENT_QUOTES);
		$data = self::_cookData($data);
		...
		Common::getQueue()->push('game_client_operating_msg',$msgId);
	...
	public static function genMsgOutput($v, $isPush = false) {
		switch($v['type']) {
			...
			case 202 : //专题
                ...
                $params = Game_Api_Util_SubjectUtil::getClientApiSubjectParamsById($v['contentId']);
                if($params['subViewType']) {
                    $param['subViewType'] = $params['subViewType'];
		...
		$startTime = $v['start_time'] > $v['create_time'] ? $v['start_time'] : $v['create_time'];
		$t = array(
				'id'        => $v['id'],
	 ...
	 * @return array
	 */
	public static function genSysMsgOutput($v, $puuid, $uname, $version= false) {
		...
		$startTime = $msg['start_time'] > $msg['update_time'] ? $msg['start_time'] : $msg['update_time'];
		$t = array(
				'id'        => $msg['id'],
				'type'      => $type,
				'viewType'  => $viewType,
	 ...
	 * 保存赠送消息到消息队列中
	 */
	public static function saveMsg($uuid, $type, $desc, $title=""){
	    if(!$uuid || empty($desc)) return false;
	    $time = Common::getTime();
	    $message = array(
	            'type' =>  $type,
	            'top_type' =>  self::SYS_MSG,
	            'totype' =>  self::TARGET_SPECIFIED_ACCOUNT,
	            'title' =>  $title,
	            'msg' =>  $desc,
	            'status' =>  self::NO_READ,
	            'start_time' =>  $time,
	    ...
	    $debugMsg = array('msg' => "赠送消息 ", 'event'=> $message);
	 ...
	 */
	public static function getsMaps($params,$orderBy = array('create_time'=>'DESC','id'=>'DESC')) {
		if (!is_array($params)) return false;
