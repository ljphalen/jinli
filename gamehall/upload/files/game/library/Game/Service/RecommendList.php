...
class Game_Service_RecommendList {
	...
	public static function updateGameStatus($gameId, $status) {
	    ...
	    if($status == Resource_Service_Games::STATE_OFFLINE) {
	        $bannerFlg = Game_Api_RecommendBanner::gameIsShowing($gameId);
	        $textFlg = Game_Api_RecommendText::gameIsShowing($gameId);
	        $searchParams['link_type'] = Game_Service_Util_Link::LINK_CONTENT;
	        $searchParams['link'] = $gameId;
	        $data = array('status' => Game_Service_RecommendNew::RECOMMEND_CLOSE_STATUS);
