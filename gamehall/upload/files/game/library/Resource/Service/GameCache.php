...
class Resource_Service_GameCache extends Common_Service_Base{
    ...
    //本地缓存有效时间
    const LOCAL_CACHE_EXPIRE = 120;
    
    const PACKAGE_INFO_CACHE_EXPIRE = 90000;
    ...
    public static function saveGameDataToCache($gameId){
    	...
    	//保存历史版本
    	$gameVersions = self::getGameVersionsFromDb($gameId);
    	if($gameVersions){
    		self::saveGameVersionsToCache($gameId, $gameVersions);
    	}
  	
    	//保存游戏的差分包给升级接口
    	self::saveGameDiffPackageToCache($gameInfo);
    	
    	//保存包名与游戏名的关系
    	self::saveGamePackageRelationGameIdToCache($gameInfo['packagecrc'], $gameInfo['id']);
    	return  true;
    }

    public  static function saveGamePackageRelationGameIdToCache($packagecrc, $gameId) {
    	$packages[$packagecrc]  =  $gameId;
    	$hashKey = self::getGamePacekageRelationGameIdfoKey();
    	$ret = self::_getCache()->hMset($hashKey, $packages, self::PACKAGE_INFO_CACHE_EXPIRE);
    	return $ret;
    }
    

    public static function saveGameDiffPackageToCache($gameInfo) {
    	//游戏-版本-差分包
    	$game = array(
    			"id"           => $gameInfo['id'],
    			"version"      => $gameInfo['version'],
    			'version_code' => $gameInfo['version_code'],
    			"versions"     => self::getVersionRelationIds($gameInfo['id']),
    			'diff'         => self::getDiffInfo($gameInfo['id'], $gameInfo['version_id']),		
    	);
    	$key = self::getGameDiffInfoKey($gameInfo['id']);
    	$ret = self::_getCache()->set($key, $game, self::PACKAGE_INFO_CACHE_EXPIRE);
    	return $ret;
    }
    
 
    public static function getVersionRelationIds($gameID){
    ...
    function getDiffInfo($gameID, $versionId){
    ...
    public static function deleteGameCache($gameId){
    	...
    	$cache->delete($infoCkey);
    	
    	$infoCkey = ":" . $gameId . ":versions";
    	$cache->delete($infoCkey);
    	
    	//删除差分信息
    	$key =self::getGameDiffInfoKey ( $gameId );
    	$cache->delete($key);
    	
    	//删除差分包关系
    	self::deleteDiffInfoFromCache ( $gameId);

    	
    }

	private static function deleteDiffInfoFromCache($gameId) {
		//删除差分包
    	$params['id'] = $gameId;
    	$gameInfo = Resource_Service_Games::getBy($params);
    	$packagecrc = $gameInfo['packagecrc'];
    	$hashKey = self::getGamePacekageRelationGameIdfoKey ();
    	$ret = self::_getCache()->hDel($hashKey, $packagecrc);
    	
	}
	
	public static function getGamePacekageRelationGameIdfoKey() {
		$key = Util_CacheKey::GAME_PACEKAGE_RELATION_GAMEID;
		return $key;
	}
	
	public  static function getGameDiffInfoKey($gameId) {
		$key = Util_CacheKey::GAME_PACKAGE_DIFF_INFO.$gameId;
		return $key;
	}
    ...
    private static function getInfoCacheKey($gameId) {
        return Util_CacheKey::GAME_INFO . $gameId;
    }
    ...
    public static function getGameAllVersionFromCache($gameId){
    	...
    	$data = self::_getCache()->hGetAll($ckey);
    	if(empty($data)){
    		$data = self::getGameVersionsFromDb($gameId);
    		if($data){
	...
	private static function _getGameImgData($gameId){
		//游戏详情图片
		$screenImg = Resource_Service_Img::getScreenImg($gameId);
		$result= array(
				'gimgs' => json_encode($screenImg[Resource_Service_Img::SCREEN_480_800]),
				'simgs' => json_encode($screenImg[Resource_Service_Img::SCREEN_240_400]),
                'hdimgs' => json_encode($screenImg[Resource_Service_Img::SCREEN_720_1280]),
		);
		return $result;
	}
	 ...
	 * @return array
	 */
	private static function getGameDiffFromDb($gameId, $versionId){
		$diffs = array();
		$diffs = Resource_Service_Games::getsByIdxDiff(array('game_id'=>$gameId, 'version_id'=> $versionId));
		return $diffs;
	}
	 ...
	 * @return array
	 */
	private static function getGameVersionsFromDb($gameId){
		$versions = array();
