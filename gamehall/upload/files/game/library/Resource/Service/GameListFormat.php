...
class Resource_Service_GameListFormat {
    ...
    private static function buildItem148($item){
        ...
        if ($flag) {
            ...
            if ($item['commonGift']) array_push($attach, '礼');
    ...
    private static function buildItem151($item) {
        ...
        $item['attach'] = self::getAttachFlag($item);
        ...
        $item['downloadCount'] = self::formartNumber($downloadCount);
        self::clearItem($item);
        return $item ;
    }
    
    
    private static function getAttachFlag($item){
        $apkVer = Yaf_Registry::get("apkVersion");
        //版本低于1.5.5不支持直接返回
     ...
     */
    public static function formartNumber($downloads){
    	 if(!$downloads){
    	 	return '';
    	 }
    	 $ouputStr = '下载';
    	 if($downloads < 10000){
    	 	return $downloads.$ouputStr;
    	 }
    	 if(10000 <= $downloads && $downloads < 100000){
    	 	return round( $downloads/10000, 1).'万'.$ouputStr;
    	 }
    	 
    	 if(100000 <= $downloads && $downloads < 100000000){
    	 	return round( $downloads/10000,1).'万'.$ouputStr;
    	 }
    	 
    	 if(100000000 <= $downloads ){
    	 	return round( $downloads/100000000,1).'亿'.$ouputStr;
    	 }
    ...
    private static function getRewardFlag($item){
        ...
        $isSendTicket = ($isSendTicket && $ticketCount) ? true : false;
