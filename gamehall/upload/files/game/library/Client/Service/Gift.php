...
class Client_Service_Gift extends Common_Service_Base{
    ...
    const GIFT_LIST_EXPIRE = 600;
    const PAGE_LIMIT = 10;
    const LOCK_GIFT_EXPIRE = 1;
    
    const GIFT_REMAIN_ZERO = 1;
    const GIFT_PERCENT = 2;
    const GIFT_YUNYING = 3;
	...
	public static function updateGiftGameId($status,$game_id) {
		...
		$list = self::getDao()->getsBy(array('game_id'=>$game_id));
		foreach ($list as $gift) {
		    self::updateGiftStatusInAd($gift['id'], $gift['status']);
		}
	}
	...
	public static function updateGiftStatus($giftIds, $status) {
	    ...
	    foreach($giftIds as $key=>$id) {
			...
			self::updateGiftStatusInAd($id, $status);
		}
		return true;
	}
	...
	public static function updataGiftNumCacheByGiftId($giftId){
		...
		$cache = Cache_Factory::getCache();
		$cacheKeys = self::getClientCacheKeyArr();
		$time = Common::getTime();
		$cacheRelevanceGiftInfoKey      = 'giftNum';
		foreach($cacheKeys as $cacheHash){
		    $cacheHashKey = $gameId.'gameRelevanceGiftInfo'.$cacheHash;
		    
		    $params = $gifts = array();
		    $params['game_id'] = $gameId ;
		    $params['status'] = 1 ;
		    $params['game_status']=1;
		    $params['effect_start_time'] = array('<=', $time);
		    $params['effect_end_time'] = array('>=', $time);
		    if($cacheHash == Util_CacheKey::GIFT_LESS_THAN_VERSION){
		    ...
		    $result = $cache->hSet($cacheHashKey, $cacheRelevanceGiftInfoKey, $giftNum, Client_Service_Gift::GIFT_EXPIRE);
		}
		return $result;
	 ...
	 * 通过游戏ID 关联的礼包数
	 */
	public static function getGiftNumByGameId($gameId, $clientVersion='1.6.1'){
		if(!$gameId){
			return false;
		}
	 	...
	 	$cache = Cache_Factory::getCache();
	 	$params = array();
	    if(strnatcmp($clientVersion, '1.6.1') < 0){
		    $cacheKey = Util_CacheKey::GIFT_LESS_THAN_VERSION;
		    $vipLevelList = User_Config_Vip::getVipListByGiftLevle(User_Config_Vip::GIFT_1);
		    $params['vip_level'] = array('IN',$vipLevelList);
		} else {
		    $cacheKey = Util_CacheKey::GIFT_MORE_THAN_VERSION;
		}
		
		$cacheHash = $gameId.'gameRelevanceGiftInfo'.$cacheKey ;
		$cacheRelevanceGiftInfoKey      = 'giftNum';
	...
	public static function getBackGroundGiftGiftNumByGameId($gameId, $getGiftSource){
	    if(!$gameId){
	        return false;
	    }
	
	    $params = array();
	    if($getGiftSource == Util_CacheKey::GIFT_LESS_THAN_VERSION){
	        $vipLevelList = User_Config_Vip::getVipListByGiftLevle(User_Config_Vip::GIFT_1);
	        $params['vip_level'] = array('IN',$vipLevelList);
	    }
	 ...
	 * 入口检测法刷新游戏礼包
	 */
	public static function getGameCommonGift($gameId){
	    if (!intval($gameId)) return false;
	    $result = self::getBackGroundGiftGiftNumByGameId($gameId,Util_CacheKey::GIFT_LESS_THAN_VERSION);
	    $gift = $result ? 1 : 0;
	    return $gift;
	}
	 ...
	 * 入口检测法刷新游戏礼包
	 */
	public static function getGameVipGift($gameId){
	    if (!intval($gameId)) return false;
	    $result = self::getBackGroundGiftGiftNumByGameId($gameId,Util_CacheKey::GIFT_MORE_THAN_VERSION);
	    $gift = $result ? 1 : 0;
	    return $gift;
	}
	 ...
	 * 礼包信息存入缓存
	 */
	public static function updataGiftBaseInfoByGameId($gameId){
		if(!$gameId){
			return false;
		}
		$params['game_id'] = $gameId ;
		...
		$gifts = self::getDao()->getsBy($params);
		foreach ($gifts as $val){
			self::updateGiftDataToCache($val['id']);
		}
	...
	public  static function lock($key) {
		$cache = Cache_Factory::getCache();
		$result = $cache->lock($key, self::LOCK_GIFT_EXPIRE);
		return $result;
	}
	 ...
	 */
	 public static function reduceRemainActivitionCodeCache($giftId, $currentRemains) {
	 	 ...
	 	 $lockKey = 'giftActivityCodeRemainNum'.$giftId;
	...
	public static function reduceTotalActivitionCodeCache($giftId, $currentTotal) {
		...
		$lockKey = 'giftActivityCodeTotal'.$giftId;
		if(!self::lock($lockKey)){
			return false;
		}
		$cache = Cache_Factory::getCache();
	...
	public static function getClientCacheKey($clientVersion) {
	    $clientCacheKey = Util_CacheKey::GIFT_LESS_THAN_VERSION;
	    if(Common::compareClientVersion($clientVersion, '1.6.1')){
	        $clientCacheKey = Util_CacheKey::GIFT_MORE_THAN_VERSION;
	    }
	    return $clientCacheKey;
	}
	
	public static function getClientCacheKeyArr() {
	    return array(Util_CacheKey::GIFT_LESS_THAN_VERSION, Util_CacheKey::GIFT_MORE_THAN_VERSION);
	}
	
	/**
	 * 更新礼包列表版本到redis缓存
	 */
	public static function updateGiftListVersionToCache($gameId = 0){
		$redisCache = Cache_Factory::getCache();
		$versionKey = Client_Service_Gift::getGiftListVersionKeyName();
		$versionData = Common::getTime();
		$redisCache->set($versionKey, $versionData, self::GIFT_LIST_EXPIRE);
	}
	
	public static function getPrivilegeGiftListKeyName($page, $giftLevel, $versionData) {
	    $dataKeyName = Util_CacheKey::GIFT . '::' . Util_CacheKey::PRIVILEGE_GIFT_LIST.'_'.$page.'_'.$giftLevel.'_'.$versionData;
	    return $dataKeyName;
	}
	
	
	public static function getGiftListKeyName($page, $gameId = 0, $versionData, $clientCacheKey) {
		$dataKeyName = Util_CacheKey::GIFT . '::' . Util_CacheKey::GIFT_LIST.'_'.$page.'_'.$gameId.'_'.$clientCacheKey.'_'.$versionData;
		return $dataKeyName;
	}
	
	public static function getGiftListVersionKeyName(){
		$versinoKey = Util_CacheKey::GIFT_LIST_VERSION;
		return $versinoKey;
	}
	
	public static function getGiftListKeyNameForNew($page, $versionData, $clientCacheKey) {
		$dataKeyName = Util_CacheKey::GIFT . '::' . Util_CacheKey::GAME_GIFT_LIST.'_'.$page.'_'.$clientCacheKey.'_'.$versionData;
		return $dataKeyName;
	}
	
	
	public static function getGiftListVersion() {
		$redisCache = Cache_Factory::getCache();
		$versionKey = Client_Service_Gift::getGiftListVersionKeyName();
		$dataVersion = $redisCache->get($versionKey);
		if($dataVersion === false){
			$dataVersion = Common::getTime();
			$redisCache->set($versionKey, $dataVersion, Client_Service_Gift::GIFT_LIST_EXPIRE);
		}
		return $dataVersion;
	}
	
	public static function updatePrivilegeGiftListVersionCache(){
	    $redisCache = Cache_Factory::getCache();
	    $versionKey = Client_Service_Gift::getPrivilegeGiftListKeyName();
	    $versionData = Common::getTime();
	    $redisCache->set($versionKey, $versionData, self::GIFT_LIST_EXPIRE);
	}
	
	public static function getPrivilegeGiftListVersion() {
	    $redisCache = Cache_Factory::getCache();
	    $versionKey = Client_Service_Gift::getPrivilegeGiftListKeyName();
	    $dataVersion = $redisCache->get($versionKey);
	    if($dataVersion === false){
	...
	private static function cookData($data) {
		...
		if(isset($data['vip_level'])) $tmp['vip_level'] = $data['vip_level'];
		if(isset($data['dev_email'])) $tmp['dev_email'] = $data['dev_email'];
	...
	/**更新广告中礼包状态*/
	public static function updateGiftStatusInAd($giftId, $status) {
	    try {
	        Game_Service_SingleRecommend::updateGiftStatus($giftId, $status);
	    } catch (Exception $e) {
	    }
	}
