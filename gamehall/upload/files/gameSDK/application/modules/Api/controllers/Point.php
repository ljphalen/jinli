...
class PointController extends Api_BaseController {
    ...
    private function getGainMsg($gainType, $gainSubType, $days) {
        ...
        if($gainMsg == $this->gainTypes[9]){
        	$cfgInfo = Activity_Service_Cfg::getByID($gainSubType);
        	$gainMsg = $cfgInfo[Activity_Service_Cfg::NAME] ;
        }
        if($gainMsg == $this->gainTypes[10]){
            $infoData = Festival_Service_TouchGame::getByInfo(array('id'=>$gainSubType));
            $gainMsg = $infoData['name'] ;
        }
    	return $gainMsg;
    }
    ...
    public function prizeStartAction(){
        ...
        //非法设备
        $isInvalidDev = Point_Service_Prize::inspectorIsValidRequest($request);
        if(!$isInvalidDev){
            $this->output(-1, '请求非法');
        }
        //登录处理
        ...
        //黑名单处理
        $isBlackUser = Point_Service_BlackList::getBy(array('uuid'=>$request['puuid']));
        if($isBlackUser){
            $this->output(-1, '非法用户');
        }
        //获取用户积分
        ...
        $context['sub_type'] = $result['sub_type'];
        $context['day'] = $result['day'];
        $context['amount'] = $result['amount'];
        $context['sub_amount'] = $result['sub_amount'];
    	...
    	switch ($data['type']){
            ...
            case 0:
                ...
                if($data['sub_type'] == 1){
                    //最低奖项积分
                    $result['prizeType'] = 3;
                    $result['prizeName'] = $data['sub_amount'];
                }
    	}
    	return $result;
    }
    ...
    private function prizeTitle($config){
    	...
    	switch ($config['type']){
    		...
    		case 3:
    			...
    			break;
            case 0:
                if($config['sub_type']==1){
                    $result = sprintf(' %d 积分', $config['sub_amount']);
                    break;
                }
