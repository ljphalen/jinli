...
class CategoryController extends Front_BaseController{
	...
	public function indexAction() {
	    ...
	    //游戏分类名称
		$categorys = Resource_Service_Attribute::getsBy(array('at_type'=>1,'parent_id'=>0, 'status'=>1), array('sort'=>'DESC'));
	    ...
	    if($category_id == 100 || $category_id == 101){
	    	...
	    	//v1.5.6 主分类的标识
	        $category_params['parent_id'] = $category_id;
	        //过滤
	        if($this->filter){
	        	$category_params['game_id'] = array('NOT IN', $this->filter);
	        }
	   
	    	$games_all = Resource_Service_GameCategory::getsBy($category_params);
	    	if($games_all){
