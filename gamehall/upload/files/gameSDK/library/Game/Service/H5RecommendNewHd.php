...
class Game_Service_H5RecommendNewHd {
		...
		foreach($data as $key => $value) {
                    if($value['list_id']) {
                        if($value['list_type'] == 'new') {
                            $checkInfo = Util_CheckInfoStatus::check(Util_CheckInfoStatus::TYPE_NEW, $value['list_id']);
			} else {
                            $checkInfo = Util_CheckInfoStatus::check(Util_CheckInfoStatus::TYPE_HD, $value['list_id']);
			}
                        if(!$checkInfo) {
                            return false;
                        }
	...
	private static function checkField($data) {
		...
		if(isset($data['list_title'])) $dbData['list_title'] = $data['list_title'];
                if(isset($data['list_resume'])) $dbData['list_resume'] = $data['list_resume'];
