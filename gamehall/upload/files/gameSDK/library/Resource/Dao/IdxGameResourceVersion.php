...
class Resource_Dao_IdxGameResourceVersion extends Common_Dao_Base{
	...
	public function getIdxVersionByNewVersion($params) {
		$where = Db_Adapter_Pdo::sqlWhere($params);
		$sql = sprintf('SELECT * FROM  (SELECT * FROM  %s WHERE %s ORDER BY update_time DESC) AS T GROUP BY T.game_id ORDER BY T.update_time DESC',$this->getTableName(), $where);
	...
	public function getCountResourceVersion($params) {
		$where = Db_Adapter_Pdo::sqlWhere($params);
		$sql = sprintf('SELECT game_id,count(game_id) AS num FROM %s WHERE %s GROUP BY game_id',$this->getTableName(), $where);
		return $this->fetcthAll($sql);
	}
