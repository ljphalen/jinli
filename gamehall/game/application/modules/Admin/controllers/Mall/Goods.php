...
SCRIPT;
	...
	private function _cookData($info, $actionType) {
		...
		if($info['istakepart'] && $info['type'] == Mall_Service_Goods::GIFT
		 || !$info['istakepart'] && $goodType == Mall_Service_Goods::GIFT && $actionType == 'add'
		 || !$info['istakepart'] && $info['type'] == Mall_Service_Goods::GIFT && $actionType == 'edit'
		 ) {
		    $this->checkGiftNumber($info, $actionType);
		}
		
		if(!$info['total_num'] && $info['type'] != Mall_Service_Goods::GIFT) $this->output(-1, '商品总数量不能为空.');
		if(!preg_match("/^[0-9]*[1-9][0-9]*$/",$info['total_num']) && $info['type'] != Mall_Service_Goods::GIFT) $this->output(-1, '商品总数量只能为正整数.');
		...
		($info['preson_limit_num'] >=2 || $info['preson_limit_num'] < 1 || $info['preson_limit_num'] > 1)) {
