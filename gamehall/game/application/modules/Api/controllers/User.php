...
class UserController extends Api_BaseController {
	const WEB_SP = "Web_1.5.5_5.0.16_Android5.0_720*1280_I01000_wifi_FD34645D0CF3A18C9FC4E2C49F11C510";
	...
	public function registerbygvcAction(){
        ...
        $items = Common::parseSp(self::WEB_SP);
        $imei = $items['imei'];
        $pkgVersion = $this->getPkgVersion(self::WEB_SP, $package);
        $ip = Common::getClientIP();
		$data = Api_Gionee_Account::registerByGvc($tn, $vid, $vtx, $vty, $imei, $pkgVersion, $ip);
		//返回结果
		$this->output(0,'',$data);
	}
	...
	private function _addUser($data, $act){
		...
		$info = array('uuid' => $data['u'], 'uname'=> $data['tn'], 'nickname' => $nickName);
		//vip
		$counts = Account_Service_User::getUserCounts();
		$info['vip'] = User_Config_Vip::VIP_1;
		$info['vip_mon_expr'] = 0;
		$info['vip_act_expr'] = 0;
		$info['vip_rank'] = $counts;
		Account_Service_User::addUserInfo($info);
	...
	public function tradeAction(){
		...
		if(strtolower($token) == strtolower(md5($ciphertext.$str))){
			...
			if($event == $this->mNotifyConsume){
				$this->cosumeSend ( $uuid, $money, $api_key, $path, $file_name, $tradeTime );
	...
	private function cosumeSend($uuid, $money, $api_key, $path, $file_name, $tradeTime) {
		//获得缓存
...
// 		$activity->sendTictket();
        ...
        if($user) {
            Account_Service_User::addUserVipMoneyExpr($uuid, $money);
            $tradeTime = strtotime($tradeTime);
            
            $expr = array();
            $expr[User_Service_VipExpr::F_ADD_EXPR] = $money;
            $expr[User_Service_VipExpr::F_CREATE_TIME] = $tradeTime;
            $expr[User_Service_VipExpr::F_EXPR] = $user['vip_mon_expr'] + $user['vip_act_expr'];
            $expr[User_Service_VipExpr::F_LOGS] = $api_key;
            $expr[User_Service_VipExpr::F_TYPE] = User_Service_VipExpr::TYPE_MONEY;
            $expr[User_Service_VipExpr::F_UUID] = $uuid;
            User_Service_VipExpr::addVipExpr($expr);
            
            $params = array('uuid' => $uuid, 'apiKey' => $api_key, 'money' => $money, 'tradeTime' => $tradeTime);
            Async_Task::execute('Async_Task_Adapter_UserGame', 'consume', $params);
        }
	}
	...
	public function editAddressAction() {
	    $uuid = $this->getInput('puuid');
	    $this->checkOnline($uuid);
	    $info = $this->getInput(array('receiverName', 'tel', 'address', 'qq'));
	    $info['receiverName'] = html_entity_decode($info['receiverName'], ENT_QUOTES);
	    $info['tel'] = html_entity_decode($info['tel'], ENT_QUOTES);
	    $info['address'] = html_entity_decode($info['address'], ENT_QUOTES);

	    if(! is_null($info['qq'])) {
	        $info['qq'] = html_entity_decode($info['qq'], ENT_QUOTES);
	        $info['qq'] = strip_tags($info['qq']);
	    }else{
	        $info['qq'] = "";
	    }
	    $info['receiverName'] = strip_tags($info['receiverName']);
	    $info['tel'] = strip_tags($info['tel']);
	    $info['address'] = strip_tags($info['address']);
	    
	    if(is_null($info['receiverName'])) {
	        $this->localOutput(-1, '收货人不能为空');
	    }
	    $nameLength = Util_String::strlen($info['receiverName']);
	    if($nameLength == 0) {
	        $this->localOutput(-1, '收货人不能为空');
	    }
	    if(is_null($info['tel'])) {
	        $this->localOutput(-1, '联系人电话不能为空');
	    }
	    $telLength = Util_String::strlen($info['tel']);
	    if($telLength == 0) {
	        $this->localOutput(-1, '联系人电话不能为空');
	    }
	    if(is_null($info['address'])) {
	        $this->localOutput(-1, '联系人地址不能为空');
	    }
	    $addressLength = Util_String::strlen($info['address']);
	    if($addressLength == 0) {
	        $this->localOutput(-1, '联系人地址不能为空');
	    }
	    if($nameLength > 20) {
	        $this->localOutput(-1, '收货人不能超过20个字');
	    }
	    if(Util_String::strlen($info['qq']) > 20) {
	        $this->localOutput(-1, 'QQ号字符过长');
	    }
	    $isMob="/^1[3-5,8]{1}[0-9]{9}$/";
	    $isTel="/^([0-9]{3,4})?(-)?[0-9]{7,8}$/";
	    if(!preg_match($isMob, $info['tel']) && !preg_match($isTel, $info['tel'])) {
	        $this->localOutput(-1, '联系人电话格式不正确');
	    }
	    if($addressLength > 100) {
	        $this->localOutput(-1, '联系人地址不能超过100个字');
	    }
	    $userData['receiver'] = $info['receiverName'];
	    $userData['receiverphone'] = $info['tel'];
	    $userData['address'] = $info['address'];
	    $userData['qq'] = $info['qq'];
	    $params['uuid'] = $uuid;
	    $ret = Account_Service_User::updateUserInfo($userData, $params);
	    if (!$ret) $this->localOutput('-1', '操作失败');
	    $this->localOutput('0', '操作成功', $info);
	}
