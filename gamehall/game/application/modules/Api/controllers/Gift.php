...
class GiftController extends Api_BaseController {
    ...
    public function grabAction() {
    	...
    	if($uname){
    		...
    		if((strnatcmp($version, '1.6.1') >= 0)){
    		    ...
    		    if($giftInfo['vip_level'] > $giftLevel){
    ...
    private function grabGiftWarningEmail($giftInfo, $giftTotal, $remainGiftNum) {
        if($remainGiftNum == 0){
            $this->giftEmaiToQueue($giftInfo, $remainGiftNum, '剩余礼包比例预警',true, false);
        } else {
            if($giftTotal >= 50){
                //预警提示,多个邮箱有逗号隔开
                $temp_arr = array();
                ...
                if($giftNum && $remainGiftNum == intval($num) ){
                    $this->giftEmaiToQueue($giftInfo, $remainGiftNum, '剩余礼包比例预警',false, false);
                }
            } else if($giftTotal >= 10 && $giftTotal < 50){
                if($remainGiftNum < 5){
                    $this->giftEmaiToQueue($giftInfo, $remainGiftNum, '剩余礼包比例预警',false, true);
                }
            }
        }
     ...
     * 把要发邮件的礼包警告信息写入信息队列中
     */
    public function giftEmaiToQueue($giftInfo, $remainGiftNum, $emailSubject, $isAllWarning=false, $isBackstage=true){
        $email = array();
        if($isAllWarning){
            $email[] = array(
                    html_entity_decode(Game_Service_Config::getValue('game_gift_eamil')),
                    $giftInfo['dev_email'],
            );
        } else {
            if($isBackstage){
                ...
                $ret = Client_Service_GiftWarnEmail::getBy(array('is_zero'=>1,'gift_id'=>$giftInfo['id']));
                if($ret) return;
                $email[] = $giftInfo['dev_email'];
            }
        }
    	...
    	if($email){
    	    ...
    	    foreach($email as $email){
    	        $temp_arr = array('gift_id'=>$giftInfo['id'],
    ...
    private function getVipTitle($giftLevel) {
        return User_Config_Vip::$giftName[$giftLevel].'('.User_Config_Vip::$vipName[$giftLevel][1].')';
    }
