...
class MallController extends Client_BaseController {
	...
	private function getIndexSeckillGoods() {
	    ...
	    return Mall_Service_Goods::getBy($parmes);
	}
	...
	private function getSeckillGoods($seckillType){
	    $currentTime = Common::getTime();
	    $parmes['category_id'] =  Mall_Service_Goods::SECKILL_CATEGORY;
	    $parmes['status'] = Mall_Service_Goods::STATUS_OPEN;
	    if($seckillType == 'past'){
	    ...
	    list(,$goods) = Mall_Service_Goods::getList(1, 2, $parmes);
	    return $goods;
	}
	...
	private function getIndexJsonGoods($data) {
		...
		foreach($data as $key=>$value) {
			...
			$goods[] = array(
			        ...
			        'discount'=>$listDiscountArr['discount'],
			        'goodType' =>$goodType,
			        'isLogin' =>$isLogin,
			        'isCanExchange' =>$buttonCode,
			        'enoughPoints'=>Mall_Service_Goods::convertPointVal($listDiscountPoint),
			);
		}
		return $goods;
	}
