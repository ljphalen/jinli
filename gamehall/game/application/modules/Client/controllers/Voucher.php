...
class VoucherController extends Client_BaseController {
    ...
    private function buildUseableListData($data, $time){
        ...
        foreach($data as $item){
            $list[] = array(
                ...
                'status' => ($item['start_time'] > $time ) ? 5 : ( $item['balance'] ? 4 : 3),
                'chargeMount' => $item['denomination'],
                'leftMount' => $item['balance'],
            );
        }
        return $list;
    }
    ...
    private function useAticket($uuid, $page) {
        ...
        $data = array('list' => $this->buildUseableListData($data, $time), 'hasnext' => $hasNext, 'curpage' => $page, 'balance'=> $balance);
        return $data;
    }
    ...
    private function usedAticket($uuid, $page) {
        ...
        $data = array('list' => $this->buildUsedListData($data, $time), 'hasnext' => $hasNext, 'curpage' => $page,);
        return $data;
    }
