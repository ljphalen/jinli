...
class Account_Service_User{
	...
	public static function checkOnline($data, $imei="", $type='uname'){
		...
		$user = self::getUser(array($type=>$data), array('reg_time'=>'DESC'));
		//用户不存在
	 ...
	 */
	public static function checkOnlineByWeb(){
		...
		//用户信息表
		$userInfo = self::getUserInfo(array('uuid'=>$user['uuid']));
	 ...
	 * @return boolean 
	 */
	public static function getUser($params, $orderBy = array()){
		if (!is_array($params)) return false;
		return self::_getUserDao()->getBy($params, $orderBy);
	}
	
	public static function getUserCounts($params = array()) {
	    return self::_getUserDao()->count($params);
	}
	
	public static function addUserVipMoneyExpr($uuid, $expr) {
	    $params = array('uuid' => $uuid);
	    $user = self::_getInfoDao()->getBy($params);
	    if(! $user) return false;
	    $data['vip_mon_expr'] = $user['vip_mon_expr'] + $expr;
	    $expr = $data['vip_mon_expr'] + $user['vip_act_expr'];
	    $vip = User_Config_Vip::getVip($expr);
	    if($vip > $data['vip']) {
	        $data['vip'] = $vip;
	    }
	...
	public static function addUserVipActivityExpr($uuid, $expr) {
	    $params = array('uuid' => $uuid);
	    $user = self::_getInfoDao()->getBy($params);
	    if(! $user) return false;
	    $data['vip_act_expr'] = $user['vip_act_expr'] + $expr;
	    $expr = $data['vip_act_expr'] + $user['vip_mon_expr'];
	    $vip = User_Config_Vip::getVip($expr);
	    if($vip > $data['vip']) {
	        $data['vip'] = $vip;
	    }
	 ...
	 * @return 
	 */
	public static function getsByUser($params) {
		if (!is_array($params)) return false;
	...
	/**获取用户的vip等级*/
	public static function getUserVipLevel($uuid) {
	    $user = self::getUserInfo(array('uuid' => $uuid));
	    $vipLevel = $user['vip'] ? $user['vip'] : User_Config_Vip::VIP_1;
	    return $vipLevel;
	}
	...
	public static function getVipRankList() {
		$ret = self::_getInfoDao()->getVipRankList();
		return $ret;
	}
	
	public static function getUserAllInfoList($page = 1, $limit = 10, $params = array()) {
		if ($page < 1) $page = 1; 
		$start = ($page - 1) * $limit;
		$ret = self::_getUserDao()->getUserInfoList($start, $limit, $params);
		$total = self::_getUserDao()->coutUserInfoList($params);
		return array($total, $ret);
	}
	
	public static function updateVipRank() {
		return self::_getInfoDao()->updateVipRank();
	}
	 ...
	 * @return array
	 */
	public static function getUserInfoListByBirthday($page = 1, $limit = 10, $today){
		if(!$today) return false;
		if ($page < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::_getInfoDao()->getListByBirthday($start, $limit, $today);
		$total = self::_getInfoDao()->getCountByBirthday($today);
		return array($total, $ret);
	}
     ...
     * @return string
     */
    public static function countUserLoginLog($params){
	...
	private static function _cookInfoData($data) {
		...
		if(isset($data['vip'])) $tmp['vip'] = $data['vip'];
		if(isset($data['vip_mon_expr'])) $tmp['vip_mon_expr'] = $data['vip_mon_expr'];
		if(isset($data['vip_act_expr'])) $tmp['vip_act_expr'] = $data['vip_act_expr'];
		if(isset($data['vip_rank'])) $tmp['vip_rank'] = $data['vip_rank'];
