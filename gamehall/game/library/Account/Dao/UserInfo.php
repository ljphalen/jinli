...
class Account_Dao_UserInfo extends Common_Dao_Base{
	...
	public function updateVipRank() {
	    $sql = "CREATE TABLE IF NOT EXISTS `tmp_vip_rank` (`uuid` varchar(50) NOT NULL, `rank` int(10) DEFAULT '0', `expr` decimal(36,2) DEFAULT 0, `vip` TINYINT( 1 ) DEFAULT '0', PRIMARY KEY (`uuid`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
	    ...
	    $sql = "insert into tmp_vip_rank(`uuid`, `rank`, `expr`) (SELECT `uuid`,(@vip_rank:= @vip_rank + 1) as rank, (`vip_mon_expr`+`vip_act_expr`) as expr FROM game_user_info, (SELECT @vip_rank:= 0) tt ORDER BY expr desc, id asc);";
	    $ret = Db_Adapter_Pdo::execute($sql);
	    if(! $ret) {
	        return 3;
	    }
