...
class Client_Service_Giftlog{
	const KEY_STATE_FREE = 0; 
	const KEY_STATE_USED = 1; 
	const KEY_MY_GIFT_ID_EXPRIE = 600;
	const GRAB_GIFT_LOG = 0;
	const SEND_GIFT_LOG = 1;
	const EXCHANGE_GIFT_LOG = 2;
	const PPERPAGE = 10;
	...
	public static function getByGiftId($gift_id) {
		...
		return self::getDao()->getBy(array('gift_id'=>$gift_id,'log_type'=>self::GRAB_GIFT_LOG));
	}
	...
	public static function getsByGiftId($gift_id) {
		if (!intval($gift_id)) return false;
		return self::getDao()->getsBy(array('gift_id'=>$gift_id,'log_type'=>self::GRAB_GIFT_LOG));
	}
	...
	public static function getByImeiGiftId($imei,$gift_id) {
		...
		return self::getDao()->getBy(array('imeicrc'=>$imei,'gift_id'=>$gift_id,'log_type'=>self::GRAB_GIFT_LOG), array('create_time'=>'DESC'));
	}
	...
	public static function getByUnameGiftId($uname,$gift_id) {
		...
		return self::getDao()->getBy(array('uname'=>$uname,'gift_id'=>$gift_id,'log_type'=>self::GRAB_GIFT_LOG), array('create_time'=>'DESC'));
	}
	...
	public static function getGiftlogByStatus($status,$gift_id) {
		return self::getDao()->count(array('status'=>$status,'gift_id'=>$gift_id,'log_type'=>self::GRAB_GIFT_LOG));
	}
	...
	public static function getGiftlogCount($gift_id) {
		return self::getDao()->count(array('gift_id'=>$gift_id,'log_type'=>self::GRAB_GIFT_LOG));
	}
	...
	private static function getByMygiftLogs($giftLogIinfo, $page, $version){
	    ...
	    if(strnatcmp($version, '1.5.6') < 0){
	        ...
	        list($total, $myGiftLogs) = self::getList($page, self::PPERPAGE, $params, $orderBy = array('create_time'=>'DESC'));
	    }
