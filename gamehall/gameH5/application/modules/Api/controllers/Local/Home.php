...
class Local_HomeController extends Api_BaseController {
    ...
    public function slideAdAction() {
    	...
    	$dataList['slideItems']  = self::getSlideAdCacheData($clientVersion);
		$this->localOutput('','',$dataList);
    }
    ...
    public function textActivityAdAction() {
    	$dataList = Client_Service_Ad::getTextAdCacheData();
    	$data['activityItem'] = $dataList;
    ...
    public function dailyRecommendAction(){
        $sp = $this->getInput('sp');
    	$dataList = self::getDailyRecommendCacheData($sp);
    ...
    public function newRecomendListAction() {
    	...
    	$data['success'] = true;
    	$data['msg'] = '';
    	$data['data'] = $this->getRecommendCacheData($page, $sp);
    	$this->clientOutput($data);
    }
    ...
    public function newRecomendFirstPageListAction() {
    	...
    	$data['data']['listData']['success'] = true;
    	$data['data']['listData']['msg'] = '';
    	$data['data']['listData']['data'] = $this->getRecommendCacheData($page, $sp);
    	$this->clientOutput($data);
    }
    
    private function getDailyRecommendCacheData($sp) {
        $spArr = Common::parseSp($sp);
        ...
        if($systemVersionResult){
            $systemVersion = Game_Service_H5RecommendNew::GREATE_SYSTEM_VERSION;
        }else{
            $systemVersion = Game_Service_H5RecommendNew::LESS_SYSTEM_VERSION;
        }
        $dataList = Game_Service_RecommendDay::getIndexRecommendCacheData($systemVersion);
        return $dataList;
    }
    
    private function getSlideAdCacheData($clientVersion) {
        ...
        $dataList = Client_Service_Ad::getSlideAdCacheData($clientVersion);
        return $dataList;
    }
    ...
    private function getRecommendCacheData($page, $sp) {
    	...
    	$clientVersion = Common::getClientVersion($spArr['game_ver']);
    	$systemVersion = Common::getSystemVersion($spArr['android_ver']);
    	$cacheListData = Game_Service_H5RecommendNew::getRecommendListCacheData($page, $device, $clientVersion, $systemVersion);
    ...
    public function recommendGameListAction() {
        ...
        $pageData = array();
        if($recommendId) {
            $pageData = Game_Service_H5RecommendNew::getClientRecommendGamesList($recommendId, $page);
        }
        $data['sign'] = 'GioneeGameHall';
        $data['msg'] = '';
        $data['success'] = true;
    ...
    public function gameExtraInfoAction() {
        $gameIdList = $this->getInput('gameIdList');
        $data['sign'] = 'GioneeGameHall';
        $data['msg'] = '';
        $data['success'] = true;
        if (! $gameIdList) {
        ...
        foreach ($gameIdList as $gameId) {
            $gameData = array();
            $extalInfo = Resource_Service_GameData::getExtraInfo($gameId);
            if(! $extalInfo) continue;
            $gameData['hasGift'] = $extalInfo['attach'] == 1;//礼包
            $gameData['freedl'] = $extalInfo['freedl'];
            $gameData['reward'] = $extalInfo['reward'];
            $gameData['score'] = $extalInfo['score'];
        ...
        $data['data'] = $gamesInfo;
        Game_Service_RecommendDay::checkGameExtrainfo($checkInfo);
        Game_Service_H5RecommendNew::checkGameExtrainfo($checkInfo);
        $this->clientOutput($data);
    }
