...
class CategoryController extends Game_BaseController{
    ...
    private function getHotSubject() {
	    ...
	    foreach ($subjectList as $subject) {
	        $item = array(
	                        ...
	                        'url' => '/subject/detail?id='.$subject['id']
	        );
	        $list[] = $item;
	    }
	    return array('viewAllHref' => '/subject/index', 'list' => $list);
	}
	    ...
	    if($result){
	        foreach ($result as $value){
	            ...
	            $list[] = array(
	                            ...
	                            "url" => "{$this->actions['detailUrl']}?id=0&pid={$value['id']}",
	...
	private function getSubCategoryList($parentId, $isDetail=false) {
	    ...
	    if ($isDetail && $subCategory) {
	    	$subItems[] = array(
	                        "id" => 0,
	                        "title" => '全部',
	                        "url" => "{$this->actions['detailUrl']}?id=0&pid={$parentId}"
	       );
	    }
	    foreach ($subCategory as $item){
	        $subItems[] = array(
	                        ...
	                        "url" => "{$this->actions['detailUrl']}?id={$item['id']}&pid={$parentId}"
	        );
	...
	private function getDetailGameList($inputVar) {
	    list($gameIds, $hasNext, $page, $total) = $this->getGameIds($inputVar);
	    $list = array();
	    ...
	    $data = array(
	                    ...
	                    'hasNext' => $hasNext,
	                    'curPage' => $page,
	                    'ajaxUrl' => $this->actions['detailAjaxUrl'].'?id='.$inputVar['id'].'&pid='.$inputVar['pid']
	    );
	    return $data;
	}
	...
	private function getGameIds($request) {
	    ...
	    return array($gameIds, $hasNext, $page, $total);
	}
}
