...
class Client_GiftController extends Admin_BaseController {
	...
	public function editlog_postAction() {
		...
		if(in_array($activation_code,$logs)){
			...
			$total = Client_Service_Giftlog::getGiftlogCount($gift_id);
			Client_Service_Gift::updateGiftTotalCache($gift_id, $total);
			
			$remain_gifts = Client_Service_Giftlog::getGiftlogByStatus(0, $gift_id);
		    Client_Service_Gift::updateGiftRemainNumCache($gift_id, $remain_gifts);

		    $GiftInfo = Client_Service_Gift::getGift($gift_id);
		    Client_Service_Gift::updataGiftBaseInfoCache($GiftInfo, $gift_id);
	...
	public function addlog_postAction() {
		$gift_id = intval($this->getInput('gift_id'));
		$game_id = $this->getInput('game_id');
		...
		//查找该礼包的所有激活码
		$logs = Client_Service_Giftlog::getByGiftId($gift_id);
		$logs = Common::resetKey($logs, 'activation_code');
		$logs = array_unique(array_keys($logs));
		
		$tmp = $temp = array();
		$ret = Client_Service_Giftlog::getBy(array('gift_id'=>$gift_id),array('send_order'=>'DESC'));
	...
	public function add_postAction() {
		...
		if (!$game_info) $this->output(-1, '该游戏id不存在或游戏已下线');
		$info['game_status'] = 1;
		$info['game_sort'] = $this->getGameSort($info['game_id']);
		$result = Client_Service_Gift::addGiftGame($info);
		if (!$result) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
	...
	public function edit_postAction() {
		...
		$ret = Client_Service_Gift::updateGiftInfo($info, intval($info['id']));
		if (!$ret) {
			$this->output(-1, '操作失败');
		}
	    ...
	    if($info['status'] == Client_Service_Gift::GIFT_STATE_CLOSEED){
	    	//更新热门礼包游戏id
	    	Client_Service_GiftHot::updateBy(array('status'=>Client_Service_Gift::GIFT_STATE_CLOSEED), array('gift_id'=>$info['id']));
