...
class Client_Service_Gift extends Common_Service_Base{
	 ...
	 *
	 * Enter description here ...
	 * @param unknown_type $newGiftInfo
	 */
	public static function updateGiftInfo($newGiftInfo, $giftId) {
		if (!is_array($newGiftInfo)) {
			return false;
		}
		...
		try {
			//更新礼包
			$queryRemainKey = array('gift_id'=>$giftId, 'status' => Client_Service_Giftlog::KEY_STATE_FREE);
			$gift = Client_Service_Giftlog::getBy($queryRemainKey);
			if(!$gift) {
				$newGiftInfo['status'] = 0;
			}

			$ret = self::updateGiftDatabase($newGiftInfo, $giftId);
			if (!$ret) {
				parent::rollBack();
			...
			//更新热门礼包游戏id
			$hotGiftInfo = array('game_id'=>$newGiftInfo['game_id'], 'status'=>$newGiftInfo['status']);
			...
			if($trans) {
				...
				if ($result) {
					self::updataGiftBaseInfoCache($newGiftInfo, $giftId);
