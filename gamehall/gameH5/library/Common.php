...
class Common {
    ...
    public static function isOnlineEnvironment() {
        if (!defined('ENV')) {
            return true;
        }
        return (ENV == 'product') ? true : false;
    }
    ...
    static public function getClientVersion($clientVersion){
    	$pos = strrpos($clientVersion, '.');
    	if($pos < 5){
    		$pos = 5;
    	}
    	return substr($clientVersion, 0, $pos);
    ...
    private static function compareVersion($version1, $version2) {
    	...
    	for ($i = 0; $i <= $minLength; $i ++) {
            ...
            $tmp = strncmp($originArr[$i], $destArr[$i]);
    	    if($tmp == 0) continue;
    	    $result = $tmp;
    	    break;
    	}
    ...
    static public function getFilteredAppList($oldAppList) {
	    $pkey = "appFilterList";
    	$cache = self::getCache();
    	$filterList = $cache->get($pkey);
    	if (!$filterList) {
	    	$filterList = self::getConfig('appFilterConfig');
	    	$cache->set($pkey, $filterList, 24*60*60*60);
    	}
...
str;
    ...
    static public function getRandChar($length) {
        $str = '';
        $strPol = "ABCDEFGHIJKLOYZX0123456789abcdefhikuqstx";
