...
$lock = $cache->get("_cron_ulogin_lock");
if($lock) exit("nothing to do.\n");
$cache->set("_cron_ulogin_lock", 1, 6*60); //6分钟过期

$time = Common::getTime();
$lastTime = $cache->get('_cron_ulogin_last_time');
...
$cache->set('_cron_ulogin_last_time', $time);

$cache->set("_cron_ulogin_lock", 0, 6*60); //6分钟过期
exit;
