...
class Client_Dao_DailyTaskLog extends Common_Dao_Base{
	...
	public function getReportList($params = array()){
		$where = Db_Adapter_Pdo::sqlWhere($params);
		$sql = sprintf('SELECT send_object, task_id,SUM(denomination) AS total_denomination,COUNT(DISTINCT UUID) AS total_num FROM %s WHERE %s   GROUP BY task_id,send_object', $this->getTableName(), $where);
		return  $this->fetcthAll($sql);
	}
