...
class Festival_ActivityController extends Client_BaseController{	
	...
	public function indexAction(){
		...
		//限制的客户端版本
		$configVersion = Festival_Service_BaseInfo::getConfigClientVersion();
		$ClientVersionKey = $festivalInfo[Festival_Service_BaseInfo::FIELD_CLIENT_VSERSIONS];
		$limitClientVersion = $configVersion[$ClientVersionKey];
	...
	public function prizeRuleInfoAction(){
		...
		$prizesInfo = Festival_Service_Prizes::getsBy($params);
	...
	private function checkClientVersion($currentClientVersion, $festivalInfo) {
		$configClientVersion = Festival_Service_BaseInfo::$mCleintVersionConfig;
		$clientVersionKey = $festivalInfo[Festival_Service_BaseInfo::FIELD_CLIENT_VSERSIONS];
	...
	private function getPrizeInfo($festivalId, $online, $uuid) {
		...
		list(, $prizeList) = Festival_Service_Prizes::getList(self::ONE_PAGE, 9 , $params);
