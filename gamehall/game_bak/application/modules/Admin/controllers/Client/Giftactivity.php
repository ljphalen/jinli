...
SCRIPT;
	...
	public function addActivityPostAction() {
		...
		if($result){
		    Resource_Service_GameData::updateRewardFlagCache($activityInfo['game_id']);
		}
		$this->output(0, '操作成功');
	}
	...
	public function editActivityPostAction() {
		...
		if(!$ret) $this->output(-1, '操作失败');
		if($ret){
		    $this->updateGiftActivityCache($activityInfo['id']);
		    Resource_Service_GameData::updateRewardFlagCache($activityInfo['game_id']);
		}
		$this->output(0, '操作成功.'); 		
	}
	...
	private function getActivationCodeParams($input,$giftId) {
	    ...
	    if ($input['start_time'] && $input['end_time']) $params['create_time'] = array(array('>=', strtotime($input['start_time'])), array('<=', strtotime($s['end_time']))) ;
	    if ($input['uname']) {
