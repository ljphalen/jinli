...
class Client_Ad_RecommendlistController extends Admin_BaseController {
	...
	public function editPostAction() {
		...
		foreach ($bannerSort as $index) {
		    ...
		    $banner['sort'] = $sort--;
		    $bannerList[$index-1] = $banner;
		}

		$sort = count($recommendSort);
		foreach ($recommendSort as $index) {
		    $rec = $recommendList[$index-1][Game_Manager_RecommendList::RECOMMEND_INFO];
		    $rec['sort'] = $sort--;
		    $recommendList[$index-1][Game_Manager_RecommendList::RECOMMEND_INFO] = $rec;
		}
