...
class GiftController extends Api_BaseController {
	...
	private function getGiftListCacheData($page, $gameId ){
		...
		$versionData = Client_Service_Gift::getGiftListVersion ();
		$localCache = Cache_Factory::getCache(Cache_Factory::ID_LOCAL_APCU);
		$dataKeyName = Client_Service_Gift::getGiftListKeyName ($page, $gameId, $versionData);
		$totalKeyName = Client_Service_Gift::getGiftListTotalKeyName ( $gameId, $versionData );
		$giftListData = $localCache->get($dataKeyName);
		$total        = $localCache->get($totalKeyName);
		if($giftListData === false || $total === false){
			list($dbTotal, $giftListData) = $this->getGiftListFromDb ($page, $gameId);
			if($dbTotal){
				$localCache->set($dataKeyName,  $giftListData, Client_Service_Gift::GIFT_LIST_EXPIRE);
				$localCache->set($totalKeyName, $dbTotal,  Client_Service_Gift::GIFT_LIST_EXPIRE);
			}	
		}
