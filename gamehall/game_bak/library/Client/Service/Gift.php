...
class Client_Service_Gift extends Common_Service_Base{
	...
	public static function updateGiftListToCache($gameId){
		...
		$redisCache->set($versionKey, $versionData, Client_Service_Gift::GIFT_LIST_EXPIRE);
	}
	
	
	
	public static function getGiftListTotalKeyName($gameId = 0, $versionData) {
		$totalKeyName = Util_CacheKey::GIFT . '::' . Util_CacheKey::GIFT_LIST.'_'.$gameId.'_total'.'_'.$versionData;
		return $totalKeyName;
	}
	...
	public static function getGiftListVersion() {
		$redisCache = Cache_Factory::getCache();
		$versionKey = Client_Service_Gift::getGiftListVersionKeyName();
		$versionData = $redisCache->get($versionKey);
