...
class Resource_Service_Pgroup{
	const DEFAULT_PGROUP = 1;
	const GAME_STATE_ONLINE = 1;
	const GAME_STATE_OFFLINE = 0;
    const CACHE_EXPRIE = 3600;	
    const MODE_GROUP_KEY = 'modeGroup';
	...
	public static function  getModeGroupCacheData(){
		$cache = Cache_Factory::getCache();
		$cacheKey = self::MODE_GROUP_KEY  ;
		$modelGroupCacheData =  $cache->get($cacheKey);
		if($modelGroupCacheData === false){
			list(, $modelGroup) = Resource_Service_Pgroup::getAllPgroup();
			$cache->set($cacheKey, $modelGroup, self::CACHE_EXPRIE);
		}else{
			$modelGroup = $modelGroupCacheData;
		}
		return $modelGroup;
	}
	
	public static function  updateModeGroupCacheData(){
		$cache = Cache_Factory::getCache();
		$cacheKey = self::MODE_GROUP_KEY ;
		list(, $modelGroup) = Resource_Service_Pgroup::getAllPgroup();
		if(is_array($modelGroup)){
			$cache->set($cacheKey, $modelGroup, self::CACHE_EXPRIE);
		}	
	}
	 ...
	 */
	public static function getGroupByDevice($device){
        $group = self::DEFAULT_PGROUP;
        if (! $device) {
            return $group;
        }
        $modeGroup = self::getModeGroupCacheData();
        if (! is_array($modeGroup)) {
            return $group;
        }
        $type = Resource_Service_Type::getResourceTypeByTitle($device);
        if (! $type) {
            return $group;
        }
	    $deviceId = $type['id'];
        foreach ($modeGroup as $key => $val) {
            $deviceArr = explode(',', $val['p_id']);
            if (is_array($deviceArr) && in_array($deviceId, $deviceArr)) {
                $group = $val['id'];
                break;
            }
        }
