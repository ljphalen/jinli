...
class Util_CacheForApi {
	...
	private static $_CONTROLL_KEY_CODE = "a.0s@d!f@#\$s^za`df%*540+|d/?";
...
//         }
	...
	private static function getControllKey($api) {
        ...
        $key = self::$_KEY_SEPARATOR . self::$_PREFIX . self::$_KEY_SEPARATOR . 
            $class . self::$_KEY_SEPARATOR . $method . self::$_KEY_SEPARATOR . 
            md5($class . self::$_KEY_SEPARATOR . $method . self::$_KEY_SEPARATOR . self::$_CONTROLL_KEY_CODE);
	    return $key;
	}
	...
	private static function existsControllValueKey($api, $key) {
        $controllKey = self::getControllKey($api);
	    $cache = Cache_Factory::getCache();
	    return $cache->hExists($controllKey, $key);
	}
	
	private static function saveControllValueKey($api, $args, $expireTime) {
