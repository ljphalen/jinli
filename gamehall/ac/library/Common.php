...
class Common {
	...
	static public function upload($name, $dir) {
		...
		$uploader = new Util_Upload(array('maxSize'=>$maxSize,'allowFileType'=>$allowType));
	 ...
	 * @param string $cku
	 * @param string $chl
	 * @param string $intersrc
	 * @param string $redirect
	 * @return string
	 */
	static public function detailurl($cku, $chl, $intersrc, $redirect) {
		if (strpos($redirect, '?') !== false) {
				$redirect.=sprintf('&cku=%s&chl=%s&intersrc=%s', $cku, $chl, $intersrc);
			} else {
				$redirect.=sprintf('?cku=%s&chl=%s&intersrc=%s', $cku, $chl, $intersrc);
			}
		return $redirect;
	 ...
	 * @param string $tj_type
	 * @return string
	 */
	static public function dlurl($url,$appid, $cku, $chl, $intersrc, $redirect) {
		return sprintf('%s?appid=%s&cku=%s&chl=%s&intersrc=%s&_url=%s',$url, $appid, $cku, $chl, $intersrc, urlencode($redirect));
	}
}