...
class Db_Adapter_Pdo {
	...
	public static function setDefaultAdapter($adapter) {
		if (!is_array($adapter)) return false;
	...
	public static function getAdapter() {
		...
		$adapterConfig = Yaf_Registry::get($adapterName);
		return  Db_Pdo::factory($adapterConfig);
	}
	...
	public static function _where($field, $op, $value) {
		...
		switch ($op) {
			...
			case "LIKE":
				...
				break;
			case "FIND":
				$str .=	sprintf(" FIND_IN_SET(%s , %s) ", self::quote($value), self::fieldMeta($field));
				break;
			case "=" :				
