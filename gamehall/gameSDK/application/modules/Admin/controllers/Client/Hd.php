...
class Client_HdController extends Admin_BaseController {
	...
	public $actions = array(
	    ...
	    'giftActivityListUrl' => '/Admin/Client_Giftactivity/index',
	);
	
	public $perpage = 20;
	public $hdType = array(
	        Client_Service_Hd::HD_TYPE_DEFAULT => '默认',
	        Client_Service_Hd::HD_TYPE_COMMENT => '评论',
	);
	/**
	 * 
	 * Enter description here ...
	...
	public function editAction() {
		...
		$this->assign('hdType', $this->hdType);
	}
	...
	public function get_nameAction() {
		...
		$game_info = Resource_Service_Games::getBy(array('id'=>$game_id, 'status'=>1));
		if(!$game_info) {
	...
	function batchUpdateAction() {
		...
		if($info['action'] =='open'){
		    ...
		    $idList = array_values($info['ids']);
		    $params = array('id' => array('in', $idList), 'status' => Client_Service_Hd::STATUS_OPEN);
		    $hdList = Client_Service_Hd::getsBy($params);
		    
			$ret = Client_Service_Hd::updateHdStatus($info['ids'], 0);
		    if (!$ret) $this->output('-1', '操作失败.');
		    foreach ($hdList as $hd) {
		        Client_Service_Hd::hdClosed($hd['id']);
		    }
	...
	public function add_postAction() {
		$info = $this->getPost(array('sort','hd_type', 'game_id', 'title', 'img', 'status', 'start_time', 'end_time', 'content', 'update_time', 'placard', 'award'));
	...
	public function edit_postAction() {
		$info = $this->getPost(array('id', 'sort','hd_type', 'game_id', 'title', 'img', 'status', 'start_time', 'end_time', 'content', 'update_time', 'placard', 'award'));
		$info = $this->_cookData($info);
		$gameId = $this->getInput('gameId');
		$gameStatus = $this->getInput('gameStatus');
		$info['update_time'] = Common::getTime();
		$hdId = intval($info['id']);
		$oldInfo = Client_Service_Hd::getHd($hdId);
		
		$ret = Client_Service_Hd::updateHd($info, $hdId);
		Client_Service_PushMsg::updateBy(array('game_id'=>$info['game_id']),  array('game_id'=>$gameId,'contentId'=>$info['id']));
		if (!$ret) $this->output(-1, '操作失败');
		if($info['status'] == Client_Service_Hd::STATUS_CLOSE && $oldInfo['status'] == Client_Service_Hd::STATUS_OPEN) {
		    Client_Service_Hd::hdClosed($hdId);
		}
		$this->output(0, '操作成功.'); 		
	}
	...
	private function _cookData($info) {
		...
		$game_info = Resource_Service_Games::getBy(array('id'=>$info['game_id'], 'status'=>1));
		if(!$game_info) {
		    $this->output(-1, '该游戏id不存在或游戏已下线');
		}
		...
		if(!$info['award']) $this->output(-1, '奖励内容不能为空.');
