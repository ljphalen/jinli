...
class Client_RankController extends Admin_BaseController {
	...
	public $actions = array(
		...
		'olrankUrl'=>'/Admin/Client_Rank/olactive',
	    'soaringUrl'=>'/Admin/Client_Rank/soaring',
	...
	public function mostAction() {
		...
		$search = array();
		if($start_time) {
			$search['DAY_ID'] >= date('Ymd', strtotime($start_time));
		}
		list(, $bi_games) = Client_Service_RankResult::getList(1, $configs['game_rank_mostnum'], $search);
	    ...
	    $categorys = Resource_Service_Attribute::getsBy(array('at_type'=>1,'status'=>1), array('sort'=>'DESC'));
		//categorys list
	...
	public function soaringAction() {
		$page = intval($this->getInput('page'));
		if ($page < 1) $page = 1;
		$limit = Game_Service_Config::getValue('game_rank_soaringnum');
		$day_id = date('Y-m-d');
		$params['day_id'] = Client_Service_RankVersion::getRankDayId(Client_Service_RankVersion::SOARING_RANK);
		list(, $bi_games) = Client_Service_SoaringRank::getList(1, $limit,$params);
		...
		$this->assign('s', $s);
		$this->assign('total', $total);
		$url = $this->actions['soaringUrl'].'/?' . http_build_query($s) . '&';
		$this->assign('pager', Common::getPages($total, $page, $this->perpage, $url));
	}
	
	public function olactiveAction() {
		$page = intval($this->getInput('page'));
		if ($page < 1) $page = 1;
		$limit = Game_Service_Config::getValue('game_rank_olranknum');
		$day_id = date('Y-m-d');
		$params['day_id'] = Client_Service_RankVersion::getRankDayId(Client_Service_RankVersion::ONLINE_ACTION_RANK);
		list(, $bi_games) = Client_Service_OlactiveRank::getList(1, $limit,$params);
		$bi_games = Common::resetKey($bi_games, 'game_id');
		$bi_games = array_unique(array_keys($bi_games));
		...
		$this->assign('s', $s);
		$this->assign('total', $total);
		$url = $this->actions['olrankUrl'].'/?' . http_build_query($s) . '&';
		$this->assign('pager', Common::getPages($total, $page, $this->perpage, $url));
	}
	...
	public function editAction() {
		...
		$config = $this->getInput(array(
				'game_client_rank','game_rank_newnum','game_rank_mostnum','game_rank_monthnum',
				'game_rank_weeknum','game_rank_cnewnum','game_rank_fastestnum','game_rank_onlineknum','game_rank_pcnum',
	    		'game_rank_olranknum', 'game_rank_soaringnum'
		));
	...
	public function addAction() {
		...
		if(count($client_games)){
			...
			if ($s['category_id']) {
				$game_ids = Resource_Service_Games::getIdxGameResourceCategoryBy(array('parent_id'=>$s['category_id'], 'status' => 1, 'game_status'=>1));
