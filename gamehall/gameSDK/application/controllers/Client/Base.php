...
class Client_BaseController extends Common_BaseController {
	...
	public function init() {
		...
		//加载动画class
		$apkVersion = Yaf_Registry::get("apkVersion");
		$clientPkg = $this->getInput('client_pkg');
		$loadingStyle = $this->getAnimationLoadingStyle($apkVersion, $clientPkg);
		$this->assign("loadingStyle", $loadingStyle);
	...
	/**
	 * 获取客户端页面加载动画
	 * @param sting $apkVersion
	 * @return string
	 */
	private function getAnimationLoadingStyle($apkVersion, $clientPkg){
		$className = "loading";
		return $className;
// 		$destVersion = '1.5.7';
		
// 		if(strtolower($clientPkg) == "com.gionee.gsp"){
// 			return $className;
// 		}
...
// 		if(Common::compareWithVersion($apkVersion, $destVersion)){
// 			$className = "loading_mouse";
// 		}
