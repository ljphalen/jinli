...
class Game_Manager_RecommendList {
    ...
    public static function initTextInfo($text) {
        ...
        $notice['id'] = $text['id'];
        $notice['adId'] = $text['id'];
        $notice['name'] = $text['title'];
        $notice['type'] = Game_Service_Util_Link::$linkType[$text['link_type']];
        $notice['linkId'] = $text['link'];
        $notice['info'] = $text['title'];
        $notice['status'] = $text['status'];
        return $notice;
    }
    ...
    public static function initRecommend($recommendList) {
        ...
        foreach ($recommendList as $key => $info) {
            ...
            if($recommend['rec_type'] == Game_Service_RecommendNew::REC_TYPE_LIST) {
                ...
                $tmp['names'] = $gameName;
                $tmp['typeName'] .= " - " . Game_Service_RecommendNew::$list_template[$recommend['list_tpl']];
    ...
    private static function gameInfo($gameId) {
        ...
        $gameData = array(
            ...
            'resume' => mb_strlen($gameInfo['resume'], 'utf-8') > 10 ? mb_substr($gameInfo['resume'], 0, 10, 'utf-8') : $gameInfo['resume'],
        );
