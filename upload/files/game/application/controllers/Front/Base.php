...
class Front_BaseController extends Common_BaseController {		
	...
	public function init() {
		...
		//PV统计
		Cache_Factory::getCache()->increment('front_pv');
		if(!Util_Cookie::get('search_unit_key', false)) {
			Util_Cookie::set('search_unit_key', md5($_SERVER['HTTP_USER_AGENT'].Common::getClientIP()), false, strtotime("+30 day"), '/', $this->getDomain());
		}
		if ($this->getToday()) {
			Cache_Factory::getCache()->increment('front_uv');
		}
	}
	...
	public function getDowloadRank(){
		...
		if ($data) {
			...
			list(, $down_games) = Client_Service_RankResult::getList(1, 60, $params);
