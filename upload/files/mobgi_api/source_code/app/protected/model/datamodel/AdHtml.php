<?php
Doo::loadModel('datamodel/base/AdHtmlBase');

class AdHtml extends AdHtmlBase{
     ...
     * 通过产品名称查找
     */
    public function getAdHtmlByProductName($product_name,$updated,$platform){
     ...
     * 获取未审核的html列表
     */
    public function getAdhtmlCheck($ischeck=array(1),$where){
        $whereSql=$this->_conditions($where);
        $sql="select id,product_name,`ischeck`,owner,creattime,platform from mobgi_ads.ad_html where ischeck in(".  implode(",",$ischeck).") and ad_product_id in(".  implode(",",Doo::session()->__get("role_productid")).") and $whereSql";
    ...
    public function upd($post){
        $this->html_name=$post["html_name"];
        $this->html_url=$post["html_url"];
