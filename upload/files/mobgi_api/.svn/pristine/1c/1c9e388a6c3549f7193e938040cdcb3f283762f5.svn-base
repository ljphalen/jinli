<?php
Doo::loadModel('datamodel/base/AdVideoBase');

class AdVideo extends AdVideoBase{
    public function __construct($properties = null) {
        parent::__construct($properties);
        Doo::db()->reconnect("ads");
    }
    public function getAdvideo($id){
        return $this->getOne(array("select"=>"*","where"=>"id='".$id."'","asArray"=>TRUE));
    }
    /*
     * 通过产品名称查找
     */
    public function getAdVideoByProductName($product_name,$updated,$platform){
     ...
     * 获取未审核的video列表
     */
    public function getAdvideoCheck($ischeck=array(1),$where){
        $whereSql=$this->_conditions($where);
        $sql="select id,product_name,`ischeck`,owner,creattime,platform from mobgi_ads.ad_video where ischeck in(".  implode(",",$ischeck).") and ad_product_id in(".  implode(",",Doo::session()->__get("role_productid")).") and $whereSql";
