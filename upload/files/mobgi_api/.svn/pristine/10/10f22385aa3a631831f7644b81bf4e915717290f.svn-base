...
class DooModel{
     ...
     * @return string
     */
    private static  function _where($field, $op, $value) {
        $str = "";
        switch ($op) {
            case ">":
            case "<":
            ...
            case "!=":
            case "<>":
                $str .=  self::_fieldMeta($field) . $op . self::_quote($value);
                break;
            case "IN":
                $str .= self::_fieldMeta($field) . $op . self::_quoteArray($value);
                break;
            case "NOT IN":
                $str .= self::_fieldMeta($field) . $op . self::_quoteArray($value);
                break;
            case "LIKE":
                $str .= sprintf("%s LIKE %s", self::_fieldMeta($field), self::_quote("%" . self::_filterLike($value) . "%"));
                break;
            case "=" :
                $str .= self::_fieldMeta($field) . '=' . self::_quote($value);
                break;
        }
        return $str;
    }
     ...
     * @return string
     */
    private static  function _filterLike($keyWord) {
        $search = array('[','%', '_', '/');
        $replace = array('\[', '\%', '\_', '\/');
     ...
     * @return string
     */
    private static  function _quote($string, $parameter_type = null) {
        return Doo::db()->quote($string, $parameter_type);
    }
     ...
     * @return string
     */
    private static  function _quoteArray($variable) {
        if (empty($variable) || !is_array($variable)) return '';
        $_returns = array();
        foreach ($variable as $value) {
            $_returns[] = self::_quote($value);
        }
     ...
     * @return string
     */
    private static  function _fieldMeta($data) {
