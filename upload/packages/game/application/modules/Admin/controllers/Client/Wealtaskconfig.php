...
class Client_WealtaskconfigController extends Admin_BaseController {
	...
	public function indexAction() {
	    ...
	    foreach($result as $key=>$value){
	    	$num = Client_Service_TicketTrade::getNum(array('sub_send_type'=>$value['id'],'send_type'=>1,'status'=>1));
	    	$ticket_total = Client_Service_TicketTrade::getCount(array('sub_send_type'=>$value['id'],'send_type'=>1,'status'=>1));
	    	$tmp[$value['id']]['num'] = $num;
	    	$tmp[$value['id']]['ticket_total'] = $ticket_total;
	    }
	...
	public function dailyTaskAction() {
		...
		foreach($result as $key=>$value){
			$finish_num = Client_Service_DailyTaskLog::getNum(array('task_id'=>$value['id'],'status'=>1));
			$ticket_total = Client_Service_DailyTaskLog::getTotal(array('task_id'=>$value['id'],'status'=>1));
			$tmp[$value['id']]['num'] = $finish_num;
			$tmp[$value['id']]['ticket_total'] = $ticket_total;
		}
		$this->assign('result', $result);
		$this->assign('s', $s);
