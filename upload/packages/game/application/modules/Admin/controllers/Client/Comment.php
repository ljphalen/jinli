...
class Client_CommentController extends Admin_BaseController {
	...
	public function checkAction() {
		...
		foreach($cm_ids as $k=>$v){
			$info = Client_Service_Comment::getComment($v);
			$top_ret = Client_Service_Comment::getByComment(array('game_id'=>$info['game_id'],'status'=>3,'is_del'=>0,'is_top'=>1));
			$cmm_ids[$info['game_id']][] = $v;
			if(count($cmm_ids[$info['game_id']]) >=2) {
				$flag = 1;
				$str = '同一个游戏只能有1条置顶评论.';
				break;
			}
	...
	*/
	...
	public function pexportAction() {
		...
		//excel-head
		$filename = "词库管理_".date('YmdHis', Common::getTime());
		Util_Csv::putHead($filename);
		$title = array(array('词组','数量','创建时间', '状态', '维护人'));
		Util_Csv::putData($title);
		//循环分页查询输出
		
		while(1){
			list(, $rs) = Client_Service_Phrase::getList($page, $this->perpage, $params);
			if (!$rs) break;
			$tmp = array();
	...
	public function exportAction() {
		...
		//excel-head
		$filename = "评论审核_".date('YmdHis', Common::getTime());
		Util_Csv::putHead($filename);
		$title = array(array('游戏ID','游戏名称','评论时间', '审核时间', '评论状态', '敏感词','评论内容', '用户名称','用户昵称','机型', '版本', 'SDK版本'));
		Util_Csv::putData($title);
		//循环分页查询输出
		
		while(1){
			list(, $rs) = Client_Service_Comment::getList($page, $this->perpage, $params);
			if (!$rs) break;
	...
	*/
	public function mexportAction() {
		...
		//excel-head
		$filename = "用户评论_".date('YmdHis', Common::getTime());
		Util_Csv::putHead($filename);
		$title = array(array('游戏ID','游戏名称','评论时间', '审核时间', '是否置顶','评论内容', '用户名称','用户昵称','机型', '版本', 'SDK版本'));
		Util_Csv::putData($title);
		//循环分页查询输出
	...
	public function lexportAction() {
		...
		//excel-head
		$filename = "审核记录_".date('YmdHis', Common::getTime());
		Util_Csv::putHead($filename);
		$title = array(array('评论序号','游戏ID','游戏名称','评论内容', '用户名称','用户昵称','机型', '版本', 'SDK版本','审核记录'));
		Util_Csv::putData($title);
		//循环分页查询输出
			
		while(1){
			
			list(, $rs) = Client_Service_CommentLog::getList($page, $this->perpage, $params);
			if (!$rs) break;
	...
	public function sexportAction() {
		...
		//excel-head
		$filename = "敏感词库_".date('YmdHis', Common::getTime());
		Util_Csv::putHead($filename);
		$title = array(array('ID', '敏感词', '词组', '添加时间', '状态', '维护人', '过滤次数'));
		Util_Csv::putData($title);
		//循环分页查询输出
		
		while(1){
			list(, $rs) = Client_Service_Sensitive::getList($page, $this->perpage, $params);
			if (!$rs) break;
			$tmp = array();
			foreach ($rs as $key=>$value) {
				$create_time = $value['create_time'] ? date('Y-m-d H:i:s', $value['create_time']) : '';
	...
	public function bexportAction() {
		...
		//excel-head
		$filename = "黑名单_".date('YmdHis', Common::getTime());
		Util_Csv::putHead($filename);
		$title = array(array('类型', '号码/imei', '添加时间', '状态', '维护人'));
		Util_Csv::putData($title);
		//循环分页查询输出
		
		while(1){
			list(, $rs) = Client_Service_Blacklist::getsearchList($page, $this->perpage, $params);
			if (!$rs) break;
			$tmp = array();
			foreach ($rs as $key=>$value) {
				$create_time = $value['create_time'] ? date('Y-m-d H:i:s', $value['create_time']) : '';
				$tmp[] = array(
						$this->utype[$value['utype']],
						($value['utype'] == 1 ? $value['name'] : $value['imei']),
						$create_time,
						($value['status'] ? '开启' : '关闭'),
						$value['uname']
				);
