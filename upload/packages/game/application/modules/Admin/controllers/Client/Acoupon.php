...
class Client_AcouponController extends Admin_BaseController {
	...
	private function _cookData() {
        ...
        if(in_array($info['htype'], array(2,3)) && $info['game_object'] == 2 && intval($info['subject_id']) < 1 ) $this->output(-1, '要填写专题ID'.$info['subject_id']);
        ...
        //专题ID
        if(in_array($info['htype'], array(2,3)) && ($info['game_object'] == 1) ){
        	$info['subject_id'] = 0;
        }
  
        
        
        if(self::CLIENT_LOGIN_ACTIVITY_TYPE == $info['htype']) {
        ...
        if(isset($info['rule_type']) && 3 == $info['rule_type'] && $info['condition_type'] == 3 && $info['htype']  == 3) {
        	$array = array();
            foreach ($info['ACoupon'] as $key => $value) {
            	...
            	$array[$key]['section_start'] = (int)$value['section_start'];
            	$array[$key]['section_end'] = (int)$value['section_end'];
            	$array[$key]['backAmount'] = (int)$value['backAmount'];
            	foreach ($value['denarr'] as $k => $v) {
            		...
            		$array[$key]['denarr'][$k]['Step'] = $v['Step'];
            		$array[$key]['denarr'][$k]['effect_start_time'] = $v['effect_start_time'];
            		$array[$key]['denarr'][$k]['effect_end_time'] = $v['effect_end_time'];
            	}
            }
            $info['rule_content'] = json_encode($array);
        } else {
            if(!$info['denomination']) $this->output(-1, '面额不能为空.');
            if(!$info['deadline']) $this->output(-1, 'A券有效期不能为空.');
            if($info['condition_type'] == 2 && $info['htype']  == 3 ){
            	if(!$info['restoration']){
            		$this->output(-1, '返还比例不能为空.');
            	}
            	if($info['restoration']>100 || $info['restoration'] < 1){
            		$this->output(-1, '返还比例范围在1-100之间.');
            	}
            	$ruleContent = array(
