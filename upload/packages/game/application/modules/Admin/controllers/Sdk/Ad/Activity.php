...
class Sdk_Ad_ActivityController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		$this->assign('ads', $ads);
		$this->assign('ad_type_name', $this->ad_type_name);
	...
	public function add_postAction() {
	    ...
	    if($info['ad_type'] == 1){
			$info['ad_content'] = Common::replaceHtmlAndJs($info['ad_content']);
			if(strpos($info['ad_content'], 'http://') === false || !strpos($info['ad_content'], 'https://') === false) {
	 ...
	 * @param unknown $info
	 */
	private function checkTimeOverlap($info, $editId='') {
	    if ($info['status'] == 1 && in_array('1', $info['show_type'])) {
	        $params = array(
	                        'status' => $info['status'],
	        ...
	        $result = Sdk_Service_Ad::getsBy($params, $orderBy=array('id'=>''));
	        if ($result) {
	        	$ids = ' ';
	        	...
	        	if (trim($ids)) {
                    $this->output(-1, '与公告ID'.$ids.'生效时间重叠');
	        	}
	        }
	    }
	}
