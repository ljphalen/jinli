...
class PointController extends Api_BaseController {
    ...
    public function myDailyTaskListAction() {		
    	$info = $this->getInput(array('puuid', 'uname', 'imei','sp'));   
    ...
    private function getTaskData($info){
    	...
    	//连续登录任务的数据
    	$continueLoginData = $this->getContinueLoginData($info['puuid'], $online);
    	//其他日常任务的数据
     ...
     * 获取连续登录的组装数据
     */
    private function getContinueLoginData($uuid, $online){
    	$time = Common::getTime();
    ...
    public function prizeStartAction(){
    	$request = $this->getInput(array('prizeId', 'puuid', 'uname'));
    	$sp = $this->getInput('sp');
