	...
	private function getJsonGoods($data, $parmes) {
		...
		foreach($data as $key=>$value) {
			if($value['type'] == 1 && $value['total_num']) {
				$detailUrl = urldecode($webroot.$this->actions['exchangeEntityUrl']. '?goodId=' . $value['id'].'&puuid='.$parmes['puuid'].'&uname='.$parmes['uname'].'&sp='.$parmes['sp']);
			} else if($value['type'] == 2 && $value['total_num']){
				$detailUrl = urldecode($webroot.$this->actions['exchangeAcouponUrl']. '?goodId=' . $value['id'].'&puuid='.$parmes['puuid'].'&uname='.$parmes['uname'].'&sp='.$parmes['sp']);
			}
			$goods[] = array(
					'id'=>$value['id'],
	...
	private function queryExchangeList($uuid, $reqPage) {
	    ...
	    do{
	       ...
	       foreach ( $srcData as $value ) {
	           ...
	           $dataItem['type'] = intval($info['type']);
	           $list[] = $dataItem;
	       }
