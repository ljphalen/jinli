...
class Util_Activity_Consume extends Util_Activity_Common implements Util_Activity_Coin{  
  ...
  private $mFileName =null;
   ...
   public function __construct($config){  
        ...
        $fileName = date('Y-m-d').'_consume.log';
        $this->mPath = $path;
   ...
   public function getCoin(){
	 ...
	 */
	 	...
	 	if($wealTaskConfig['subject_id'] && $this->checkApiKeyIsSuject($this->mConfig['api_key'], $wealTaskConfig['subject_id'])){
	 		...
	 		if(!$logRs){
	 			...
	 			if($this->updateSendTickets($responseData)){
	 				...
	 				//保存用户做的任务进度
	 				$cache = Common::getCache();
	 				$cacheKey = $this->mConfig['uuid'].'_user_info' ;
	 				//保存用户做的任务进度
	  ...
	  * 处理活动累计消费
	  */
	 public function processActivityTotalConsume($uuid, $items){
	 	//查找当前有效的累计返利消费活动
	 	$params =  $search = array();
	 	$params['status'] = 1;
	 	...
	 	foreach($items as $key=>$value){
	 		...
	 		if($value['game_object'] == 1){
	 			$game_rs = Resource_Service_Games::getBy(array('api_key'=>$this->mConfig['api_key']));
	 			if($game_rs){
	 				$rs= $this->processTotalConSumeCondition($uuid, $value);
	 				if($rs){
	 					$prizeConfig[]= $rs;
	 				}
	 			}else{
	 			...
	 			//是否在专题里面
	 			if($this->mConfig['api_key'] && $this->checkApiKeyIsSuject($this->mConfig['api_key'], $value['subject_id'] )){
	 				$rs = $this->processTotalConSumeCondition($uuid, $value);
	 				if($rs){
	 					$prizeConfig[]= $rs;
	 				}
	 	...
	 	if($this->updateSendTickets($responseData)){
	 		//赠送消息入队列
	 		$this->saveActivityTaskMsg($prizeConfig, $time);
	 	}
	 	return true;
	 }
	 ...
	 public function processActivitySingleConsume($uuid){ 	 
	 	...
	 	foreach($items as $key=>$value){
	 		...
	 		if($value['game_object'] == 1){
	 			$game_rs = Resource_Service_Games::getBy(array('api_key'=>$this->mConfig['api_key']));
	 			if($game_rs){
	 				$rs= $this->processSingleConSumeCondition($uuid, $value);
	 				if($rs){
	 					$prizeConfig[]= $rs;
	 				}
	 			}
