...
class Game_Service_Msg {	
	...
	static $msgType    = array(			
		...
		*/		
		...
		1  =>  array('key'=>'push','txt'=>'游戏内容'),
		2  =>  array('key'=>'push','txt'=>'专题'),
		3  =>  array('key'=>'push','txt'=>'分类'),
		4  =>  array('key'=>'push','txt'=>'活动'),
		5  =>  array('key'=>'push','txt'=>'链接')
	 ...
	 * @param int $type
	 */
	public static function checkAddMsg($type){
		$flag = false;
		switch(intval($type)){
			case 102:
				//A券过期
				$data = Game_Service_Config::getValue('msg_atickexpire');
                if(!empty($data)) $flag = ($data == 1) ? true : false;
				break;
			case 103:
				//A券赠送
				$data = Game_Service_Config::getValue('msg_atickgive');
				if(!empty($data)) $flag = ($data == 1) ? true: false;
				break;
			case 201:
			case 202:
			...
			case 205:
				//运营开关
				$data = Game_Service_Config::getValue('msg_yunying');
				if(!empty($data)) $flag = ($data == 1) ? true : false;
				break;
		}
		return $flag;
	 ...
	 * @return array
	 */
	public static function genMsgOutput($v) {
		switch($v['type']) {
			case 1 : //游戏
			case 201 : //游戏
				...
				$param    = array(
						...
						'gameId'   => $v['contentId'],
						'title'    => html_entity_decode(preg_replace("/&#39;/", "'", $v['title'])),
		...
		$t = array(
				'id'        => $v['id'],
				'type'      => self::$msgType[$v['type']]['key'],
				'viewType'  => $viewType,
				'title'		=> html_entity_decode(preg_replace("/&#39;/", "'", $v['title'])),
				'content'   => $v['msg'],
				'param'     => $param,
				'timeStamp' => $v['update_time'],
				//'unread'    => $s['status'] == 1 ? 0 : 1
		);
		return $t;	
	}
	 ...
	 * @return array
	 */
	public static function genSysMsgOutput($v,$puuid,$uname) {
		...
		switch($v['type']) {
			case 101 : //任务完成消息
				...
				$param    = array(
						'url'      => '',
						'title'    => '我的A券',//html_entity_decode($msg['title'])
						'source'      => 'acertificate',
				);
				break;
			case 102 : //A券过期消息
				...
				$param    = array(
						...
						'title'    => 'A券过期',//html_entity_decode($msg['title'])
						'source'      => 'acertificate',
				);
				break;
			case 103 : //A券赠送消息
			...
			case 107 : //积分赠送消息
				...
				$param    = array(
						'url'      => '',
						'title'    => '我的积分',//html_entity_decode($msg['title'])
						'source'      => '',
				);
				break;
			default :
		...
		$t = array(
				...
				'type'      => $type,
				'viewType'  => $viewType,
				'title'		=> html_entity_decode(preg_replace("/&#39;/", "'", $msg['title'])),
				'content'   => html_entity_decode(preg_replace("/&#39;/", "'", $msg['msg'])),
				'param'     => $param,
				'timeStamp' => $msg['update_time'],
				//'unread'    => ($s ? $s['status'] : $msg['status']) == 1 ? 0 : 1
		);
		return $t;
	}
