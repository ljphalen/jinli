(function(iCat){
	...
	GameApk.template =
	{
		...
		subject://专题列表页
		'<%if(data.list.length){%>\
			<div <%if(branch=="v3_1"){%> class="subject-list-v2" <%} else if(branch=="v3_2"){%> class="subject-list-v2 noShadow" <%} else {%> class="subject-list" <%}%> >\
				<ul>\
				<%for(var i=0,j=data.list.length;i<j;i++){%>\
					<li>\
						<a data-id="<%=data.list[i]["id"]%>" <%if(version!="v1"){%> data-type="0" data-viewType="<%=data.list[i]["viewType"]%>" <%}%> data-infpage="<%=data.list[i]["data-infpage"]%>" >\
		...
		eventSection://活动列表
		'<%if(data.list){%>\
			<div <%if(branch=="v3_2"){%> class="eventSection noShadow" <%} else{%> class="eventSection" <%}%>>\
				<ul class="event-list">\
