<?php
class XS_Document implements ArrayAccess, IteratorAggregate
{
    private $_data;
    ...
    {
        if ($this->_meta !== null) {
            throw new XS_Exception('Magick property of result document is read-only');
        }
    ...
    public function __call($name, $args)
    {
        ...
        throw new XS_Exception('Call to undefined method `' . get_class($this) . '::' . $name . '()\'');
    }
    ...
    public function getIterator()
    {
        if ($this->_charset !== null && $this->_charset !== 'UTF-8') {
            ...
            return new ArrayIterator(XS_Main::convert($this->_data, $to, $from));
        }
    ...
    public function beforeSubmit(XS_Index $index)
    {
    ...
    private function autoConvert($value)
    {
        ...
        $from = $this->_meta === null ? $this->_charset : 'UTF-8';
        $to = $this->_meta === null ? 'UTF-8' : $this->_charset;
        return XS_Main::convert($value, $to, $from);
    }
}
