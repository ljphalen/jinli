<?php
class XS_Server extends XS_Component
{
    const FILE = 0x01;
    const BROKEN = 0x02;
    public $xs;
    ...
    public function close($ioerr = false)
    {
        if ($this->_sock && !($this->_flag & self::BROKEN)) {
            ...
            if (!$ioerr && !($this->_flag & self::FILE)) {
                $cmd = new XS_Command(XS_Config::CMD_QUIT);
    ...
    public function setProject($name, $home = '')
    {
        if ($name !== $this->_project) {
            $cmd = array('cmd' => XS_Config::CMD_USE, 'buf' => $name, 'buf1' => $home);
            $this->execCommand($cmd, XS_Config::CMD_OK_PROJECT);
            $this->_project = $name;
        }
    }
    public function setTimeout($sec)
    {
        $cmd = array('cmd' => XS_Config::CMD_TIMEOUT, 'arg' => $sec);
        $this->execCommand($cmd, XS_Config::CMD_OK_TIMEOUT_SET);
    }
    public function execCommand($cmd, $res_arg = XS_Config::CMD_NONE, $res_cmd = XS_Config::CMD_OK)
    {
        if (!$cmd instanceof XS_Command) {
            $cmd = new XS_Command($cmd);
        }
        if ($cmd->cmd & 0x80) {
        ...
        $res = $this->getRespond();
        if ($res->cmd === XS_Config::CMD_ERR && $res_cmd != XS_Config::CMD_ERR) {
            throw new XS_Exception($res->buf, $res->arg);
        }
        if ($res->cmd != $res_cmd || ($res_arg != XS_Config::CMD_NONE && $res->arg != $res_arg)) {
            throw new XS_Exception('Unexpected respond {CMD:' . $res->cmd . ', ARG:' . $res->arg . '}');
        }
        return $res;
    }
    ...
    {
        if ($this->_sock === null) {
            throw new XS_Exception('No server connection');
        }
        if ($this->_flag & self::BROKEN) {
            throw new XS_Exception('Broken server connection');
        }
    }
    protected function connect()
    {
        ...
        if (is_int($conn) || is_numeric($conn)) {
            ...
            if (($sock = @fopen($conn, 'wb')) === false) {
                throw new XS_Exception('Failed to open local file for writing: `' . $conn . '\'');
            }
        ...
        if (($sock = @fsockopen($host, $port, $errno, $error, 5)) === false) {
            throw new XS_Exception($error . '(C#' . $errno . ', ' . $host . ':' . $port . ')');
        }
