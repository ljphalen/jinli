...
class WeiXin_Server_User extends WeiXin_Server_Base
{
    ...
    public function getOutInfo() {
        ...
        Common::log($userInfo, "weixin.log");
        //更新用户数据
        $fj_user = Fj_Service_User::getUserBy(array('open_id'=>$this->openid));
        $data = array(
            'open_id'=>$this->openid,
        ...
        if ($fj_user) {
            $ret = Fj_Service_User::updateUserBy($data, array(
                'open_id'=>$data['open_id'],
            ));
        } else {
            Common::log($data, "weixin.log");
            Fj_Service_User::addUser($data);
        }
