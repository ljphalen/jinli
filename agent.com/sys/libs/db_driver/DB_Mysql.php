...
class DB_Mysql extends DB_Base  {
    ...
    public function connect(){
		$this->conn = mysql_connect($this->_config['hostname'],$this->_config['username'],$this->_config['password']) or die("Sorry,can not connect to database");
	...
	public function errorMsg($msg = '', $error = '', $errorno = ''){
		$error     = !empty($error) ? $error : mysql_error();
		$errorno = !empty($errorno) ? $errorno : mysql_errno();
		//输出错误信息
		$str = __CLASS__." {$msg}<br>
		<b>SQL</b>: {$this->sql}<br>
		<b>Error details</b>: {$error}<br>
		<b>Error code</b>:{$errorno}<br>";
		echo $str;
	}
