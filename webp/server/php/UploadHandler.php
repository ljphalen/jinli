...
class UploadHandler
{
    ...
    protected function get_download_url($file_name, $version = null, $direct = false) {
        ...
        if (empty($version)) {
            ...
            if ($version_url) {
                return $version_url.$this->get_user_path().$file_name;
            }
    ...
    protected function get_file_object($file_name) {
        if ($this->is_valid_file_object($file_name)) {
            $file = new stdClass();
            $file->name = $file_name;
            $file->webpname = 'webp/'.$file_name.".webp";
            $file->webpsize = $this->get_file_size(
            			$this->get_upload_path($file->webpname)
            		);
            $file->size = $this->get_file_size(
                $this->get_upload_path($file_name)
            );
            $file->url = $this->get_download_url($file->name);
            $file->webpurl = $this->get_download_url($file->webpname);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file->webpname = "webp/".$name.".webp";
            $webp_file_path = $this->get_upload_path($file->webpname);
            image2webp($file_path, $webp_file_path);
            $file_size = $this->get_file_size($file_path, $append_file);
            $file->webpsize = $this->get_file_size($webp_file_path, true);
            $file->webpurl = $this->get_download_url($file->webpname);
    ...
    protected function get_file_name_param() {
        return isset($_GET['file']) ? stripslashes($_GET['file'])  : null;
    }
    ...
    public function delete($print_response = true) {
        $file_name = $this->get_file_name_param();
        $file_path = $this->get_upload_path($file_name);
        $webp_path = $this->get_upload_path('webp/'.$file_name.".webp");
        $success = is_file($file_path) && $file_name[0] !== '.' && unlink($file_path) && unlink($webp_path);
        if ($success) {
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
