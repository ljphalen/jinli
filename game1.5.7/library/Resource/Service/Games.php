...
class Resource_Service_Games extends Common_Service_Base{
	...
	public static function updateIdxGameResourceVersion($data, $id) {
		...
		if($data['status']){
			...
			//更新推荐列表中游戏的状态字段
			Game_Service_RecommendGames::updateGameStatus($data['game_id'], Resource_Service_Games::STATE_ONLINE);
			//更新每日一荐中游戏的状态字段
			...
			if(count($versions) == 1 || (in_array($data['status'],$tmp) && !in_array(1,$tmp))){
				...
				//更新推荐列表中游戏的状态字段
			    Game_Service_RecommendGames::updateGameStatus($data['game_id'], Resource_Service_Games::STATE_OFFLINE);
			    //更新每日一荐中游戏的状态字段
