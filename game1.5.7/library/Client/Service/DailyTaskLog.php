...
class Client_Service_DailyTaskLog{
	 ...
	 * @param unknown_type $limit
	 */
	public static function getByUuidList($page = 1, $limit = 10, $params = array(), $orderBy = array('id'=>'DESC')) {
		if ($page < 1) $page = 1;
		$start = ($page - 1) * $limit;
		$ret = self::getDao()->getByUuidList($start, $limit, $params, $orderBy);
		$total = self::getDao()->getByUuidCount($params);
		return array($total, $ret);
	}
