...
class Client_Ad_RecommendnewController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $list) = Game_Service_RecommendNew::getList($page, $this->perpage, $params, array('sort'=>'desc', 'id'=>'desc'));
	...
	private function setGameName(&$list) {
	    foreach ($list as $key => $value) {
	        $gameName = "";
	        $games = Game_Service_RecommendGames::getGames($value["id"]);
	...
	public function addPostAction() {
        ...
        $result = Game_Service_RecommendNew::addRecommendnew($info);
        if (! $result)
            $this->output(- 1, '操作失败');
	...
	public function editAction() {
        $id = $this->getInput('id');
        $info = Game_Service_RecommendNew::getRecommendnew(intval($id));
		$this->assignGroups();
	...
	public function editPostAction() {
		...
		$editInfo = Game_Service_RecommendNew::getRecommendnew($id);
		if (!$editInfo) $this->output(-1, '编辑的内容不存在');
		
		$updateParams = $this->getUpdateParams($postData, $editInfo);
		if (count($updateParams) >0) {
            $ret = Game_Service_RecommendNew::updateRecommendnew($updateParams, $id);
	...
	public function gamesAction() {
	    $id = $this->getInput('id');
	    $info = Game_Service_RecommendNew::getRecommendnew(intval($id));
	    $games = Game_Service_RecommendGames::getGames($id);
    ...
    public function gamesPostAction () {
        ...
        $info = Game_Service_RecommendNew::getRecommendnew($id);
        if (! $info)
            $this->output(- 1, '编辑的内容不存在');
        $ret = Game_Service_RecommendGames::updateItemsList($id, $games);
        if (! $ret)
            $this->output('-1', '操作失败.');
        Game_Service_RecommendNew::updateRecomendListCacheData();
        $this->output('0', '操作成功.');
	...
	public function gamesSortAction() {
	    $id = intval($this->getInput('id'));
	    $list = Game_Service_RecommendGames::getGames($id);
	    foreach($list as $key=>$value){
            $game = $this->getGameInfo($value['game_id']);
	...
	public function gamesSortPostAction() {
	    ...
	    $ret = Game_Service_RecommendGames::updateItemsSort($id, $sort);
	    if (!$ret) $this->output('-1', '操作失败.');
	    
	    if($status != $info['status']) {
	        $data['status'] = $status;
	        Game_Service_RecommendNew::updateRecommendnew($data, $id);
	    }
	...
	public function deleteAction() {
		...
		$result = Game_Service_RecommendNew::deleteRecommendnew($id);
	...
	public function batchUpdateAction() {
		...
		if($info['action'] =='delete'){
			 $ret = Game_Service_RecommendNew::deleteRecommendnewList($info['ids']);
		}elseif ($info['action'] == 'sort') {
		    $ret =Game_Service_RecommendNew::updateListSort($info['ids'], $info['sort']);
		}
		if (!$ret) $this->output('-1', '操作失败.');
        Game_Service_RecommendNew::updateRecomendListCacheData();
		$this->output('0', '操作成功.');
	...
	public function closeAction() {
        $this->assign('status', Game_Service_RecommendNew::closed_1_5_7_recommend() ? 1 : 0);
	}
