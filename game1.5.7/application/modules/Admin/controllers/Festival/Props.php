...
class Festival_PropsController extends Admin_BaseController {
	...
	public function addStep1Action(){
	    $clientConfig = Festival_Service_BaseInfo::getConfigClientVersion();
	...
	private function getCache(){
	    ...
	    $cacheObj = Common::getCache(); //线上用这个
	    return $cacheObj;
	}
	...
	private function isValidPropItem($group, $data){
	    foreach ($data as $key => $item){
	        ...
	        if(!$item['propImg']) $this->output(-1, "系列{$group} 道具{$key}元素图片不能为空.");
	        ...
	        if(!$item['interval']) $this->output(-1, "系列{$group} 道具{$key}元素时间间隔不能为空.");
	    }
	}
