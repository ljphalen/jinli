...
class TagLibCx extends TagLib {
    ...
    public function _import($attr,$content,$isFile=false,$type='') {
        ...
        $file		=   str_replace("?{:C('JS_VERSION')}", '', $file);
        $parseStr   =   '';
        $endStr     =   '';
        ...
        if($isFile) {
            ...
            foreach ($array as $val){
                ...
                switch($type) {
                case 'js':
                    $parseStr .= '<script type="text/javascript" src="'.$val.'?<?php echo C("JS_VERSION") ?>"></script>';
                    break;
                case 'css':
                    $parseStr .= '<link rel="stylesheet" type="text/css" href="'.$val.'?<?php echo C("JS_VERSION") ?>" />';
                    break;
                case 'php':
