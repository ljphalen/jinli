...
class ApiKeyHelper
{
	...
	static function query($Url, $data)
	{
		...
		if(APP_DEBUG)
		{
			...
			Log::write(sprintf("APPKAY_API_CURL_DATA : %s", var_export($data, true)), Log::DEBUG);
			Log::write(sprintf("APPKAY_API_CURL_RESULT : %s", var_export($result, true)), Log::DEBUG);
		}

		$decode = json_decode($result, true);
		if(empty($result) || empty($decode))
		{
			Log::write("URL fetch error: " . $Url, Log::EMERG);
			Log::write("URL fetch result: " . var_export($result, true), Log::EMERG);
			return false;
		}
