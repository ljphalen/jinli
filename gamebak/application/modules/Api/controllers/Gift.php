...
class GiftController extends Api_BaseController {
	...
	private function getGiftListCacheData($page, $gameId ){
		...
		$api = Util_CacheKey::getApi(Util_CacheKey::GIFT, Util_CacheKey::GIFT_LIST);	
		$dataKeyName = Util_CacheKey::getKey($api, func_get_args());
		$totalKeyName = Client_Service_Gift::getGiftListTotalKeyName ( $gameId );
		$cache = Cache_Factory::getCache();
		$giftListData = $cache->get($dataKeyName);
		$total        = $cache->get($totalKeyName);
		
		if($giftListData === false || $total === false){
			list($total, $giftListData) = $this->getGiftListFromDb ($page, $gameId);
			if($total){
				$cache->set($dataKeyName,  $giftListData, Client_Service_Gift::GIFT_LIST_EXPIRE);
				$cache->set($totalKeyName, $total,  Client_Service_Gift::GIFT_LIST_EXPIRE);
			}	
		}
		return  array($total, $giftListData);
	}
	...
	private function getMyGiftIdFromCache($user, $giftId, $type='uname'){
		...
		if(self::getMyGiftLog($user, $giftId, $type)){
			//首次没有缓存数据
			if($returnData === false){
				$returnData = array($giftId);
			}else{
				$returnData = array_push($returnData, $giftId);
			}
	...
	private function updateGiftIdToCache($user, $giftId){
		...
		if($giftIdData === false){
			...
			$giftIdData = array_push($giftIdData, $giftId);
		}
    ...
    public function grabAction() {
    	...
    	//抢礼包开始,获取激活码
    	$giftAcitivityCode = Client_Service_Giftlog::getBy(array('status'=>0,
    			                                                  'gift_id'=>$data['gift_id']),
    			                                                   array('send_order'=>'ASC','id'=>'ASC'));
