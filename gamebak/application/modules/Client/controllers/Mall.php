...
class MallController extends Client_BaseController {
	...
	public function moreAction(){
		...
		list($total, $goods) = $this->listGoods($this->page);
		$hasnext = (ceil((int) $total / $this->perpage) - $this->page) > 0 ? true : false;
		$jsonData = $this->getIndexJsonGoods($goods, $parmes);
	...
	private function getIndexJsonGoods($data, $parmes) {
		if(!$data) return '';
		$goods = array();
		$webroot = Common::getWebRoot();
		foreach($data as $key=>$value) {
			if($value['type'] == 1 && $value['total_num']) {
				$detailUrl = urldecode($webroot.$this->actions['exchangeEntityUrl']. '?goodId=' . $value['id'].'&puuid='.$parmes['puuid'].'&uname='.$parmes['uname'].'&imei='.$parmes['imei'].'&sp='.$parmes['sp'].'&serverId='.$parmes['serverId']);
			} else if($value['type'] == 2 && $value['total_num']){
				$detailUrl = urldecode($webroot.$this->actions['exchangeAcouponUrl']. '?goodId=' . $value['id'].'&puuid='.$parmes['puuid'].'&uname='.$parmes['uname'].'&imei='.$parmes['imei'].'&sp='.$parmes['sp'].'&serverId='.$parmes['serverId']);
			}
			$goods[] = array(
					'id'=>$value['id'],
	...
	public function exchangeAcouponPostAction(){
		...
		$this->checkExchangeAcouponPostInput();
		$good = $this->getGoodsInfo();
		$this->exchangeData['exchange_num'] = $this->exchangeNums;
	...
	//检查用户兑换数量是否正确
	private function checkExchangeAcouponPostInput() {
		if (intval($this->exchangeNums) <= 0) {
			$this->output(-1, '',array('exchangeNums'=>false));
		}
	}
