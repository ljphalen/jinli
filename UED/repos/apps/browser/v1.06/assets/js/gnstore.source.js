(function(iCat, $){
	...
	iCat.app('Store', function(){
		iCat.mix(iCat.mods, {
			'jm': ['/jquery/jquerymobile/jm.css','/jquery/jquerymobile//jm.js']
		});
		return {
			version: '0.6'
		}
	...
	iCat.mix(Store, {
		...
		scrollPic: function(){
			...
			iCat.incfile(['/jquery/touchSwipe.js','.~/slideImg.js'], function(){
				$('#mainfocus').SlideImg({
					auto: true,
					loop: true
				});
		...
		showMore: function(){
			...
			btnMore.find('.link').toggle(function(evt){
				evt.preventDefault();
		...
		loadMore: function(){
			...
			btnLoad.click(function(evt){
				...
				if(btnLoad.hasClass('disable')){
					...
					btnLoad.addClass('disable').find('em').html('<img src="'+iCat.appRef+'/pic/load.gif" alt="" />');
					$.get(url, {page: num}, function(data){
						...
						iCat.foreach(objData.data.list, function(i,v){
							strli += '<li>'
									+	'<a tj-cr="true" href="'+v.alink+'" class="intro clearfix">'
						...
						btnLoad.removeClass('disable').find('em').html('<span>点击加载更多</span>');
	...
	$(function(){
		...
		if(cr){
			$('a[tj-cr=true]').live('click', function(evt){
				evt.preventDefault();
				location.href = cr+'?url='+encodeURIComponent(this.href);
			});
			
			$('a[tj-cr=ajax]').live('click', function(evt){
				//evt.preventDefault();
				$.ajax({
					url: cr+'?url='+encodeURIComponent(this.href),
					dataType:'jsonp'
				});
			});
		}
