...
;(function($, window, document){
	$.fn.waterfall = function(options){
		...
		if(/rem/i.test(opts.colWidth)){
			var fontSize = $('html').css('font-size').slice(0, -2),
				colWidth = opts.colWidth.slice(0, -3);
				fontSize = new Number(fontSize);
			opts.colWidth = colWidth*fontSize;
		}
								...
								function(jsonData){
									try{
										...
										if($.isEmptyObject(jsonData) ){
											...
											jsonCache = jsonCache.concat(
												jsonData.data? (jsonData.data.list || []) : jsonData
											).reverse();
											dealData();
											// if(jsonData.data && !jsonData.data.hasnext){
											// 	showMsg('finish');
											// }
										}
									}
		...
		function dealData(){
			...
			loadImg(jsonCache, opts.imgUrlName, function(){
				...
				// if(hasnext===undefined||hasnext){
					$wf_result.hide();
				// }
				if((hasnext===false||hasnext==0)&&jsonCache.length==0){
					showMsg('finish');
				}
