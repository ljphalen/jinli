		...
		iCat.Class('Tools',
		{
			init: function(){
				...
				return {
					...
					clearHtml: function(pNode, uncla){
						var nodes = pNode.childNodes;
						if(!!uncla){
							iCat.foreach(iCat.toArray(nodes), function(i, node){
								if(node.getAttribute('data-unclass')===uncla){
									node.innerHTML = '';
...
(function(iCat, root, doc){
	...
	iCat.Class('Tools',
	{
		...
		blockLoad: function(file){
			var oSelf = this, PC = iCat.PathConfig,
				file = option.file, _url;
		...
		unblockLoad: function(option){
			...
			var node, type = file.replace(/.*\.(css|js).*/g,'$1');
			if(type==='css'){
