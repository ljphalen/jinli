...
class SubjectController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $subjects) = Gc_Service_Subject::getList($page, $perpage);
	...
	public function editAction() {
		$id = $this->getInput('id');
		$info = Gc_Service_Subject::getSubject(intval($id));
	...
	public function add_postAction() {
		$info = $this->getPost(array('sort', 'title','icon', 'img','status','start_time','end_time','descrip'));
		$info = $this->_cookData($info);
		$result = Gc_Service_Subject::addSubject($info);
	...
	public function edit_postAction() {
		$info = $this->getPost(array('id', 'sort', 'title','icon', 'img','status','start_time','end_time','descrip'));
		$info = $this->_cookData($info);
		$ret = Gc_Service_Subject::updateSubject($info, intval($info['id']));
	...
	public function deleteAction() {
		...
		$result = Gc_Service_Subject::deleteSubject($id);
		if (!$result) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
