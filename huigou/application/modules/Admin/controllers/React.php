...
class ReactController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $reacts) = Gc_Service_React::getList($page, $perpage, $params);
	...
	public function editAction() {
		$id = $this->getInput('id');
		$info = Gc_Service_React::getReact(intval($id));
	...
	public function edit_postAction() {
		...
		$info['status'] = 2;
		$ret = Gc_Service_React::updateReact($info, intval($info['id']));
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$ret = Gc_Service_React::deleteReact(intval($id));
		if (!$ret) $this->output(-1, '操作失败.');
		$this->output(0, '操作成功.');
	}
}
