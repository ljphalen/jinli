...
class Api_Top_Base extends Api_Top_Client {
	...
	protected function findTaobaokes(array $params) {
		...
		if ($params['saller_info'] || $params['shop_info']) {
			$sellerNicks = array();
			for ($i = 0, $j = count($items['taobaoke_items']['taobaoke_item']); $i < $j; $i++) {
				if ($params['shop_info']) {
					$items['taobaoke_items']['taobaoke_item'][$i]['shop_detail'] = $this->getShopInfo($items['taobaoke_items']['taobaoke_item'][$i]['nick']);
				}
	...
	protected function findItems(array $params) {
		...
		for ($i = 0, $j = count($items['item_search']['items']['item']); $i < $j; $i++) {
			$items['item_search']['items']['item'][$i]['shop_detail'] = $this->getShopInfo($items['item_search']['items']['item'][$i]['nick']);
	...
	protected function getItemInfoDetail($numIid) {
		...
		$item['shop_detail'] = $this->getShopInfo($item['nick']);
