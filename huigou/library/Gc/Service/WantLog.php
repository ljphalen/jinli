...
class Gc_Service_WantLog{
	 ...
	 * @param int $goodsid
	 */
	public static function getByUidAndGoodsId($uid, $goods_id) {
		if (!$uid || !$goods_id) return false;
		return self::_getDao()->getBy(array('uid'=>intval($uid), 'goods_id'=>intval($goods_id)));
	}
	/**
	...
	public static function getByGoodsId($goods_id) {
		...
		$ret = self::_getDao()->getWantLogsByGoodsId($goods_id);
		$total = self::_getDao()->count(array('goods_id'=>intval($goods_id)));
		return array($total, $ret);
	}
	...
	/**
	 * 
	 * @return Gc_Dao_WantLog
	 */
	private static function _getDao() {
		return Common::getDao("Gc_Dao_WantLog");
	}
}
