var SlideAuto = function() {
		this.init = function(oTouch) {
			...
			this.touch = $.type(oTouch.touchId)=='string'? $(oTouch.touchId) : oTouch.touchId;
...
SlideAuto.prototype = {
	...
	slide: function(index) {
		...
		if (this.vertical) {
			...
			var fontSize = -(-$('body').css('font-size').replace(/px/g,''));
			this.scroll.animate({
				left: -this.touch.find(".ui-slide-item").width()/fontSize * index + "em"
			}, 500, function(e) {
				if (index >= _this.numIndex) {
					_this.curIndex = 0;
					_this.scroll.css({
