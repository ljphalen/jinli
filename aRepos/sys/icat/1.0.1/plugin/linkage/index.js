...
ICAT.add('linkage', function(iCat){
	iCat.namespace('linkage');
	var C = iCat.cQuery, $ = jQuery,
		_getSelected = function(d, v){
		...
		_insertOption = function(oS, k, v, n3){
			var op = C('<option>').text(k).val(v).attr('title', v);
			if(v==n3) op.attr('selected','selected');
			op.appendTo(oS);
		},
		
		_threeSelect = function(cfg){
			var oS1 = iCat.isString(cfg.s1)? C(cfg.s1) : cfg.s1,
				oS2 = iCat.isString(cfg.s2)? C(cfg.s2) : cfg.s2,
				oS3 = iCat.isString(cfg.s3)? C(cfg.s3) : cfg.s3,
				v1 = cfg.v1 || null,
				v2 = cfg.v2 || null,
				v3 = cfg.v3 || null;
	...
	var linkage = {
		init: function(cfg){
			...
			if(cfg.isArea){
				...
				$.getScript(iCat.sysPlugin+'linkage/dataArea.js', function(){
			...
			if(cfg.isIndustry){
				...
				$.getScript(iCat.sysPlugin+'linkage/dataIndustry.js', function(){
