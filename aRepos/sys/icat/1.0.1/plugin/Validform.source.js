...
(function ($) {
                ...
                if (!flag) {
                    if ($(this).attr("ignore") === "ignore" && ($(this).val() === "" || $(this).val() === $(this).attr("tip"))) {
                        if (settings.tiptype == 2) {
                            ...
                            $(this).siblings(".Validform_checktip").removeClass().addClass("Validform_checktip").text("");
                        ...
                        $.ajax({
                            ...
                            success: function (s) {
                                ...
                                if (dataTips.result) {
                                    ...
                                    $.fn.Validform.sn.showmsg(dataTips.message, settings.tiptype, { obj: inputobj, type: 2, sweep: settings.tipSweep }, "hide");
                                } else {
                                    //inputobj.attr("valid",s);
                                    inputobj.attr("valid", '');
                                    errorobj = inputobj;
                                    $.fn.Validform.sn.showmsg(dataTips.message, settings.tiptype, { obj: inputobj, sweep: settings.tipSweep });
                                }
                            },
                            error: function () {
    ...
    $.Datatype = {
        ...
        "e": /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
        "url": /^[a-zA-z]+:\/\/[^\s]*$/,
        "phone": /^\d{3,4}-\d{7,8}-\d+$|^\d{3,4}-\d{7,8}$/,
        "en": /^[a-zA-Z]+$/
    };
})(jQuery);