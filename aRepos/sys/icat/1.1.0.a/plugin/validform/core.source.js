...
(function($){
	...
	$.fn.Validform=function(settings){
		...
		if($('input[type=password][tip]')[0]){
			
			$(document).bind('click', function(){
				var pwd = $('input[type=password][tip]'),
					defaultvalue = pwd.attr('tip'),
					tip = pwd.siblings('s');
				pwd.val()==defaultvalue? tip.show() : tip.hide();
			});
		...
		this.each(function(index){
			...
			$this.find("[tip]").each(function(){//tip是表单元素的默认提示信息,这是点击清空效果;
				var defaultvalue=$(this).attr("tip"),
					altercss=$(this).attr("altercss"),
					isPWD = $(this).attr('type')==='password';//密码提示时的优化 --- jndream 2012.4.28
				$(this).focus(function(){
					if($(this).val()==defaultvalue){
						if(isPWD) $(this).siblings('s').hide();
						$(this).val('');
						if(altercss){$(this).removeClass(altercss);}
					}
				}).blur(function(){
					if($.trim($(this).val())===''){
						if(isPWD) $(this).siblings('s').show();
			...
			//bind the blur event;
			$this.find("[datatype]").bind(settings.etype? settings.etype : 'blur', function(){
				var flag=true;
				...
				if(/,/.test(dtype)){
					dtype = dtype.split(',');
					flag = $.fn.Validform.sn.regcheck(dtype[0],$(this).val(),$(this)) && $.fn.Validform.sn.regcheck(dtype[1],$(this).val(),$(this));
					var ii = !$.fn.Validform.sn.regcheck(dtype[0],$(this).val(),$(this))? 0:1;
				} else {
					flag = $.fn.Validform.sn.regcheck(dtype,$(this).val(),$(this))
				}
				if(!flag){
					if($(this).attr("ignore")==="ignore" && ( $(this).val()==="" || $(this).val()===$(this).attr("tip") )){
						if(settings.tiptype==2){
							var tempobj=$(this).siblings(".Validform_checktip");//.parent().next().find(".Validform_checktip")
					...
					var err_msg = ii? $(this).attr("errormsg").split('|')[ii] : $(this).attr("errormsg");
					$.fn.Validform.sn.showmsg(err_msg||tipmsg.w,settings.tiptype,{obj:$(this),type:3,sweep:settings.tipSweep},"hide"); //当tiptype=1的情况下，传入"hide"则让提示框不弹出,tiptype=2的情况下附加参数"hide"不起作用;
				}else{
					if($(this).attr("ajaxurl")){
						...
						$.ajax({
							type: "POST",
							url: inputobj.attr("ajaxurl")+(/\?/.test(inputobj.attr("ajaxurl"))? "&t=":"?t=")+(new Date()).getTime(),
							//data: $(this).attr("name")+"="+$(this).val(),//"param="+$(this).val()+"&name="+$(this).attr("name") ----update by wwl 5-15
							data: $(this).attr('allForm') && $(this).attr('allForm')=='true' ? $(this).parents('form').serialize(): $(this).attr("name")+"="+$(this).val(), //----update by wwl 5-21
							dataType: "text",
							success: function(s){
								if($.trim(s)=="y"){
					...
					if($(this).attr("ajaxurl")){
						if($(this).attr("valid")!="true"){
							...
							if(!msghidden || settings.tiptype!=1){
								setTimeout(function(){
									thisobj.trigger(settings.etype? settings.etype : 'blur', ["postform"]);//continue the form post;
								},1500);
							}
	...
	$.fn.Validform.sn={
		...
		showmsg:function(msg,type,o,show){//o:{obj:当前对象, type:1=>正在检测 | 2=>通过}, show用来判断tiptype=1的情况下是否弹出信息框;
			...
			if(type==2 && o.obj){
				o.obj.siblings(".Validform_checktip").text(msg);//parent().next().find
				this.cssctl(o.obj.siblings(".Validform_checktip"),o.type);
			}
	...
	$.Datatype={
		...
		"p":/^[+]{0,1}(\d){1,4}[ ]{0,1}([-]{0,1}((\d)|[ ]){1,12})+$/,
		"m":/^((\+86)|86|0)?(13\d|15[012356789]|18[0236789]|14[57])\d{8}$/,
		"e":/^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
		"url":/^(\w+:\/\/)?\w+(\.\w+)+$/
	};
})(jQuery);