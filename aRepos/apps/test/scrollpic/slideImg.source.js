		...
		SlideImg: function(options){
			...
			o = {
				...
				reset: function(index){
					tabs.removeClass('ui-slide-tabcur').eq(index).addClass('ui-slide-tabcur');
				}
			};
			...
			$(window).bind('resize', function(){
				scroll.width(scrollItem.width()*numIndex*(opt.css3? 1:2));
				//opt.css3? scroll.get(0).style.webkitTransform = 'translateX('+(-curIndex*scrollItem.width())+'px)':
					scroll.css({left: -curIndex*scrollItem.width() + 'px'});
			});
		}
	});
})(Quo);