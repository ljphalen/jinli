...
(function(iCat, $){
	...
	iCat.mix(MH, {
		...
		slidePic: function(){
			...
			if(!$('.banner, .cla-menu .wrap, .proPic .wrap, .pic-show .wrap')[0]) return;
			iCat.incfile(['/zepto/touchSwipe.js','../slidePic.js'], function(){
				$('.banner').slidePic({slideItem:'.pic>span', circle:true, auto:true});
				...
				$('.proPic .wrap').slidePic({
					slidePanel: '.pic',
					slideItem: '.pic span',
					handlePanel: '.proPic .handle',
					handleItem: '.proPic .handle span'
				});
		...
		addItem: function(){
			var oWrap = $('.J_itemWrap'), url = oWrap.attr('data-ajaxUrl');
