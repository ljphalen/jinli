...
var webUI = {
    ...
    initTopNav: function () {
        ...
        if (hasNavTop) {
            var navTopList = $(".nav_top_menu").find("li");
			...
			navTopList.on('mouseover',function(){
				navTopList.removeClass("on");
                $(this).addClass("on");
				$(this).addClass('li_img');
				$('.nav_bottom_img').hide();
			})
			navTopList.on('mouseout',function(){
				$(this).removeClass('li_img');
				$('.nav_bottom_img').show();
			})
        }
	...
	initZtListSonSort:function(){
		if($('#zt').hasClass('on')){
			this.imgSimpleRearrange();
		}
	},
	initBzListSort:function(){
		if($('#taotuList').hasClass('on')){
			this.imageRearrange();
		}
	},
    //壁纸选择基本交互效果
...
$(function () {
    ...
    webUI.addRegistPage(
            ...
            bz_bzlwpsubject_bzlwpsubjectadd,
           
            bz_bzlwpsubject_bzlwpsubjectadd02
            );
