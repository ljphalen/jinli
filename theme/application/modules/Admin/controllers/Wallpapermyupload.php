 ...
 *
 */
class WallpapermyUploadController extends Admin_BaseController {

    protected $options;
    protected $urlName = "/wallpaper/attachs/wallpaper";
    protected $error_messages = array(
        ...
        7 => '没有写权限',
        8 => 'php停止上传',
        'post_max_size' => '文件大小 in php.ini',
        'max_file_size' => '文件太大',
        'min_file_size' => '文件太小',
        'accept_file_types' => '不应许的文件格式',
        'max_number_of_files' => '最大数量',
        'max_width' => '最大图片宽度',
        'min_width' => '最小图片宽度',
        'max_height' => '最大图片高度',
        'min_height' => '最小图片高度',
    ...
    public function indexAction() {
        $this->assign("meunOn", "bz_bzUpload");
    }

    public function indextwoAction() {
        $userInfo = $this->userInfo;
        $where = "wallpaper_up_status=0 and wallpaper_user = ".$userInfo['uid'];
        $wallpapers = $this->mk_imgPath(Theme_Service_WallFileImage::getByWhere($where));

        $targ = Theme_Service_Wallpapertype::getAll();

        $this->assign("wallpaperup", $wallpapers);
        $this->assign("targs", $targ);
        $this->assign("meunOn", "bz_bzUpload");

        //Theme_Service_WallFileImage::updateFiled($value, $filed, $id)
    }

    /* private function mk_imgPath($wallpaper) {
      ...
      $vs["url"] = $this->webroot . '/attachs/theme/wallpaper' . $tmp_url . 'thumbnail' . '/' . $tmp_name;
      // $res[$ks]["big_url"] = $this->webroot . '/attachs/theme/wallpaper' . $tmp_url . $big_url_resulution . '/' . $tmp_name;
      $vs["wallpaper_path"] = $this->webroot . '/attachs/theme/wallpaper' . $vs["wallpaper_path"];
      }
...
//--------------------------------------------------------------------------//
    ...
    public function uploadfilesAction($initialize = true, $error_messages = null) {

        $this->__inits();
        $year = date("Y", time());
        $mouth = date("m", time());
        $fileurl = DIRECTORY_SEPARATOR . $year . DIRECTORY_SEPARATOR . $mouth . DIRECTORY_SEPARATOR;

        $uploads = $this->urlName . $fileurl;
        $this->mkdirs($uploads);

        $this->options = array(
            'script_url' => $this->get_full_url($this->webroot . "/" . $uploads),
            'upload_dir' => BASE_PATH . '../attachs' . $uploads,
            'upload_url' => $fileurl,
            'user_dirs' => true,
            'mkdir_mode' => 0775,
            //表单的名字（重要参数）
...
// Automatically rotate images based on EXIF meta data:
        ...
        $files = array("files" => $this->post());
        $userName = $this->userInfo['uid'];
        $file_id = Theme_Service_WallFileImage::set_into_data($files, $userName);
        $files["files"][0]->fileId = $file_id;
     ...
     * @return string
     */
    protected function get_full_url($urlName = '') {
        $https = !empty($_SERVER['HTTPS']) && strcasecmp($_SERVER['HTTPS'], 'on') === 0;
        $urlHeader = $https ? 'https://' : 'http://';
        $urlHost = !empty($_SERVER['REMOTE_USER']) ? $_SERVER['REMOTE_USER'] . '@' : '';
        $urlRouter = (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : ($_SERVER['SERVER_NAME'] .
                        ($https && $_SERVER['SERVER_PORT'] === 443 ||
                        $_SERVER['SERVER_PORT'] === 80 ? '' : ':' . $_SERVER['SERVER_PORT'])));
        if (!$urlName) {
            $urlName = substr($_SERVER['SCRIPT_NAME'], 0, strrpos($_SERVER['SCRIPT_NAME'], '/'));
        }
        ...
        /* echo "urlHeader = ";
          ...
          echo "<br/>"; */
        $allUrl = $urlHeader . $urlHost . $urlRouter . $urlName;
...
// print_r($allUrl);
    ...
    /**
     * 生成文件名字
     * @param type $file_name 原文件名字;
     * @return string
     */
    protected function mk_image_name($file_name = 'tmp.jpeg') {
//$typeName = substr($file_name, strrpos($file_name, ".") + 1);
// $file_name = mb_convert_encoding($file_name, 'GBK', 'UTF-8');
        ...
        $uuid = substr($charid, 0, 8) .
                ...
                substr($charid, 16, 4) .
                substr($charid, 20, 12);
        $strName = $uuid . "_$file_name";

        //$strName = $uuid . $ext_name;
        return $strName;
    }
    ...
    protected function get_upload_path($file_name = null, $version = null) {
        ...
        $res = $this->options['upload_dir'] . $this->get_user_path()
...
//
    ...
    protected function get_server_var($id) {
        ...
        $res = isset($_SERVER[$id]) ? $_SERVER[$id] : '';
...
//$file_name = $this->mk_image_name($upload['name'][$index]);
...
// $_FILES is a one-dimensional array:
            $files[] = $this->handle_file_upload(
                    isset($upload['tmp_name']) ? $upload['tmp_name'] : null, $file_name ? $file_name : (isset($upload['name']) ? $upload['name'] : null), $size ? $size : (isset($upload['size']) ?
