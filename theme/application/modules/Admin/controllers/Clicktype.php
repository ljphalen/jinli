...
class ClicktypeController extends Admin_BaseController{
	...
	public function init() {
		parent::init();
		$this->parents = Theme_Service_ClickType::getParentsList();
	}
	...
	public function indexAction() {
		...
		$clcategorys =  Theme_Service_Clicktype::getListByParentIds($pids);
		$list = $this->_cookCategorys($this->parents, $clcategorys);
		
		$crs = Theme_Service_Click::searchClickList(array('sdate'=>$sDate, 'edate'=>$eDate));
	...
	public function add_postAction(){
		...
		$ret = Theme_Service_Clicktype::addClicktype($info);
	...
	public function editAction(){
		$id = $this->getInput('id');
		$info = Theme_Service_Clicktype::getClicktype(intval($id)); 
		$this->assign('parents', $this->parents);
	...
	public function edit_postAction(){
		...
		if (!$info['name']) $this->output(-1, '名称不能为空.');
		$ret = Theme_Service_Clicktype::updateClicktype($info, $info['id']);
		if (!$ret) $this->output(-1, '操作失败.');
		$this->output(0, '操作成功.');
	}
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Theme_Service_Clicktype::getClicktype($id);
		if ($info && $info['id'] == 0) $this->output(-1, '信息不存在无法删除');
		$ret = Theme_Service_Clicktype::deleteClicktype($id);
		if (!$ret) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
