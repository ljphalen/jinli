...
class AdController extends Admin_BaseController {
	...
	public function indexAction() {
		...
		list($total, $ads) = Theme_Service_Ad::getList($page, $perpage, $search);
	...
	public function editAction() {
		$id = $this->getInput('id');
		$info = Theme_Service_Ad::getAd(intval($id));
		$this->assign('ad_types', $this->ad_types);
	...
	public function add_postAction() {
		$info = $this->getPost(array('sort', 'title', 'ad_type', 'link', 'img', 'status'));
		$info = $this->_cookData($info);
		$result = Theme_Service_Ad::addAd($info);
	...
	public function edit_postAction() {
		$info = $this->getPost(array('id', 'sort', 'title', 'ad_type', 'link', 'img', 'status'));
		$info = $this->_cookData($info);
		$ret = Theme_Service_Ad::updateAd($info, intval($info['id']));
	...
	public function deleteAction() {
		$id = $this->getInput('id');
		$info = Theme_Service_Ad::getAd($id);
		if ($info && $info['id'] == 0) $this->output(-1, '无法删除');
		Util_File::del(Common::getConfig('siteConfig', 'attachPath') . $info['img']);
		$result = Theme_Service_Ad::deleteAd($id);
		if (!$result) $this->output(-1, '操作失败');
		$this->output(0, '操作成功');
	}
