...
class Util_File {
	 ...
	 * @param boolean $ifTheme           是否对文件加锁，默认为true即加锁

	 */
	public static function savePhpData($fileName, $data, $isBuildReturn = true, $method = self::READWRITE, $ifTheme = true) {
		$temp = "<?php\r\n ";
		...
		return self::write($fileName, $temp, $method, $ifTheme);
	}
	 ...
	 * @param bool $ifTheme 是否锁文件，默认为true即加锁
	 ...
	 */
	public static function write($fileName, $data, $method = self::READWRITE, $ifTheme = true, $ifCheckPath = true, $ifChmod = true) {
		touch($fileName);
		if (!$handle = fopen($fileName, $method)) return false;
		$ifTheme && flock($handle, LOCK_EX);
