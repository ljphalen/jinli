 ...
 *
 */
class Theme_Service_Stat{
	
	/**
	 * 
	 */
	public static function incrementPv($pv, $type) {
		$ret = self::_getPvDao()->get(array('dateline'=>date('Y-m-d'), 'tj_type'=>intval($type)));
		if ($ret) {
	...
	public static function incrementIp($arr_ip) {
		if (is_array($arr_ip)){
			...
			foreach ( $arr_ip as $key => $val) {
				$ret = self::_getIpDao()->get(array('dateline'=>date('Y-m-d'), 'ip'=>$val));
				if (!$ret) {
					$arr[$key]['id'] = '';
					$arr[$key]['ip'] = $val;
	...
	private static function _getIpDao() {
		return Common::getDao("Theme_Dao_Ip");
	}
	...
	private static function _getPvDao() {
		return Common::getDao("Theme_Dao_Pv");
	}
}
